...
void movingCircle(float x, float y, float size, int offset) {
  ...
  strokeWeight(2);
  stroke(random(150),random(100,200),random(150,255), lerp(255, 0, circleAge));
...
void mousePressed()
{
  ...
  if (mousePressed)
  {
    ...
    else if ( mouseX > width/2 && frame_rate_value < 120)
       {
         frame_rate_value++;
       }
    else if ((frame_rate_value == 1)||(frame_rate_value >= 120))
       {
          if (frame_rate_value == 1)
          {
             println("Frame rate cannot be decreased below 1");
          }
          
          else if (frame_rate_value >= 120)
          {
             println("Frame rate cannot be increase above 120"); 
          }

       }
       
    println("Frame Rate is: " + frame_rate_value);  
