...
float xspeed = 2.5, yspeed = 2.5;  // Speed of the shape
...
void size_init() {
  // Width of the shape
  size = 100*noise(xpos*.009, ypos*.009);       
  diff = 10*(size/50);
}
...
void bounce() {
  ...
  if (xpos > width-size-60 || xpos < 25) {
    xdirection *= -1;
  }
  if (ypos > height-size-25 || ypos < 25) {
    ydirection *= -1;
  }
}
...
void writeVictory() {
  PFont font;
  font = loadFont("Chalkduster-50.vlw"); 
  textFont(font); 
  text("VICTORY!", 110, 20, 700, 500);
}
