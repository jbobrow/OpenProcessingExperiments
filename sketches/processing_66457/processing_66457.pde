...
void draw() {
  ...
  noStroke();
  fill(255-colorCounter,255-colorCounter,200);
  ...
  if(mouseX<width/4 && mouseY<height/4){
    fill(200+colorCounter,200+colorCounter,200+colorCounter);
    ellipse(xPos, yPos, d/2,d/2);
  }
  
  if(xPos < (d/2) || xPos > width -(d/2)) {
    xVel = xVel*(-1);
    fill(255-colorCounter,255-colorCounter,100);
    if (mouseX>=width/2&&mouseOnRight==false);
    mouseOnRight = true; 
     colorCounter+=5;    
  }
  
  else if(yPos < (d/2)||yPos > height-(d/2)){    
  yVel = yVel*(-1);
...
if(mouseX<width/2 && mouseY <= height/2 || mouseY>= height/2){
...
if (mousePressed && mouseX>=width/2 && mouseY<height/2) {
    textFont(mySecondFont);
    fill(100,0,0);
    text("hahahahahahahahahahahaaha", 0, height-25);
