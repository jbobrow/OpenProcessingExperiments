...
class GUI {
  ...
  Draggon addDraggon(String text, int x, int y, int w, int h) {
    draggons.add(new Draggon(x, y, w, h, text));
    return draggons.get(draggons.size()-1);
  }

  Draggon addDraggon(String text, int x, int y, int w, int h, int orientation) {
  ...
  void run() {
    ...
    for (int i=0; i<scrolls.size(); i++) {
      ...
      if (!mouseLocked && mouseX>s.x+elementBase && mouseY>s.y && mouseX<s.x+s.w-elementBase && mouseY<s.y+s.h) {
        s.hover=true;
        if (mousePressed) {
          s.value=(mouseX-s.x-elementBase)*(s.max-s.min)/(s.w-2*elementBase);
          s.drg.hover=true;
        }
      }
      else {
      ...
      if (s.drg.pressed) {
        s.value=(s.drg.x-s.x-1.5*elementBase)*(s.max-s.min)/(s.w-3*elementBase);
      }
      s.value = s.value<s.min? s.min : s.value;
      s.value = s.value>s.max? s.max : s.value;
      s.drg.x = int(s.value*(s.w-3*elementBase)/(s.max-s.min)+s.x+1.5*elementBase);
    }
  ...
  class Scroll extends Element {
    ...
    Scroll(int x, int y, int w, int h, float min, float max, float start) {
      ...
      hover   = false;
      pressed = false;
      lowBtn = addButton("<", x, y, elementBase, h);
      hiBtn  = addButton(">", x+w-elementBase, y, elementBase, h);
      drg    = addDraggon("", int(x+w*value/(max-min)), y+h/2, elementBase, h, ONLYX);
  ...
  void drawButton(Button b) {
    ...
    case BASIC:
      ...
      rect(b.x, b.y, b.w, b.h, elementRound, elementRound, elementRound, elementRound);
      fill(0);
