...
float deltaT = 6; // 1.2 min - 6 max
float reactionRate = .49; // 0.62 - 0.49
float aRate = 0.038; // 0.04 - 0.038
float bRate = 0.01; // 0.008 - 0.01
float F = .04; // 0.04 - 0.015
...
void draw() {
  ...
  for(int i=0;i<w;++i) {
    for(int j=0;j<h;++j) {
      //pixels[j*w+i] = color(a[i][j]*255,255,255,10);
      pixels[j*w+i] = color(a[i][j]*255);
    }
  }
  updatePixels();
}
