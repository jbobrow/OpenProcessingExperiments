...
void draw() {
  ...
  fill(0);
  t = tlimit *60*1000 - millis();
// 60 stands for seconds in a minute, 1000 stands for milliseconds in a second
tmin = (t/(60*1000));
tsec = (t/(1000));
 textSize(20);
text("Time Left: "+ tsec, 30,35);
 
  

  if(state == true){
  for(int i = 0 ; i < deck.size(); i ++){
    if(deck.get(i).getFliped()==false){
        PImage ima = loadImage("back.jpg");
  image(ima,deck.get(i).getX(),deck.get(i).getY());
    }else{
       deck.get(i).render();
    }
  }
