...
final float repulsionMaxDist = 3.5;
final float repulsionStiffness = 20;
final float mouseMaxDist = 50;
final float mouseStiffness = 20;
final float particleRadius = 5;
...
void setup()
{
  ...
  bg = loadImage("couvprocessing.jpg");
...
void draw()
{
  ...
  for(int i=0; i<l; i++)
    ellipse(particles[i].x, particles[i].y, particleRadius, particleRadius);
}
