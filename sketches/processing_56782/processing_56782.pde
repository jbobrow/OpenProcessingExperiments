...
int w = 30;
int v= 60;
 // ----- another primitive value for the rectangle size. An integer type.
...
void setup() {
  ...
  for (int i = w/2; i < width; i = i + 20) {
    for (int j = w/2; j < height; j = j + 20) {
      ...
      wOff += 0.015;                      //  ----- this one increases by 0.02 for each sequence into the loop
  ...
  //saveFrame("cadre"+timestamp()+".jpg"); //  ----- I save my screen into a png file
}
