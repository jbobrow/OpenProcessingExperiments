...
Title Screen image credit to Team Shanghai Alice; image originally from 
Ten Desires 
...
In this Beta build, the hit box of the character has been fixed and more precise 
movement can be achieved.  The Boss health has also been raised; the character's
sprite moves right and left depending on which directional key the player is pressing.
The title screen also has a tentative art image.   
*/
...
//load images
  ...
  youmuTitleScreen = loadImage ("Th13Youmu.png"); 
...
//this chunk of code allows for a title screen   
  if (gamestarted == false) { 
    ...
    image (youmuTitleScreen, width/2 - 50, height/2 - 50); 
...
//if the player touches the hitbox of the enemy, they lose health
    if (rectangle_collision(girl.x+2, girl.y+22, 3, 3, width/2, 5, 50, 50) == true)
    {
      girl.health --;
    }
...
//when it is hit 
    for (Iterator iterator = particles.iterator(); iterator.hasNext();) {
      ...
      if (rectangle_collision(enemybulletX, enemybulletY, 10, 10, girl.x+2, girl.y+22, 3, 3) == true)
      {
        girl.health = girl.health - 5;
        if (rectangle_collision(enemybulletX, enemybulletY, 10, 10, girl.x+2, girl.y+22, 3, 3) == true)   iterator.remove();
      }
    }  
