...
void setup()
{
  ...
  // Load the table 
  String lines[]=loadStrings("https://spreadsheets.google.com/pub?key=0AovDPxTmQGH7dHB6R1NiOGJnaUhVZXA1ZVNJcnJGeEE&hl=en&output=txt");
  ...
  if ((mouseDragX==0) && (mouseDragY==0)){
    camera();
    image(img,30,40,img_displayw,img_displayh);
    textMode(SCREEN);
    text (caption1, 60+img_displayw, 40, 1000, 40);
    text (caption2, 60+img_displayw, 60, 1000, 40);
    text ("Scene : " + scene_no_str, 60+img_displayw, 80, 1000, 40);
    text ("Image at", 60+img_displayw, 100, 400, 40);
    text (hour_str + "hr", 60+img_displayw, 120,100,20);
    text (minute_str + "min.", 130+img_displayw, 120, 100, 20);
    text (second_str + "sec.", 200+img_displayw, 120, 100, 20);
 }
...
void display() {
  ...
  translate(-rows/2, -plotratio*cols/2,0);
  head();
  popMatrix();
}
...
void image_display(){
  ...
  String url = "https://dl.dropbox.com/u/2614917/CCA6/CCA_"+ scene_no_str +".jpg";
