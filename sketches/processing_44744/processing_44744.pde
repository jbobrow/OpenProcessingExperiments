...
void draw(){
  ...
  if (key == CODED) {       //When KEY DOWN is hit, the image moves up to reveal the image underneath
    if (keyCode == DOWN) {
      y-=4;
    } else if (keyCode == UP) {  //When  KEY UP is hit, the image moves down to show previous images
      y+=4;
    }
    if(y>0){    //If image is scrolled all the way to the top, it stops
        y=0;
    }
    if(y<-4200){    //If image is scrolled all the way to the bottom, it stops
        y=-4200;
    }
  }
 
     if (keyCode == DOWN) {    //If KEY DOWN is hit, leaves move down from out of frame and cotinue to fall
      y1+=3; 
      y2+=2.5;
      y3+=4;
      y4+=5.5;
      ...
      y7+=5; 
      y8+=4.5;
    } else if (keyCode == UP) {    //If KEY UP is hit, leaves move up from their current positions
      y1-=3; 
      y2-=2.5;
      y3-=4;
      y4-=5.5;
      ...
      if (y1>300){    //If the leaves fall beneath the page, they reappear at the top and fall again
        y1=-30;
      }
      if (y2>300){
        y2=-30;
      ...
      if (y1<-60){   //If leaves rise above the page, they will start from the bottom again, until other direction is pushed
        y1=300;
      }
      if (y2<-60){
        y2=300;
