...
int give = 25; //this is how forgiving the game is with the player's mouse position
int timeLast = 0; //this is the variable where we store the last known second
 
void setup() {
  size(500, 500); //this sets the size of the sketch
  smooth(); //this makes all 2D primitives smooth
  timeLast=second(); //this syncronizes the timeLast with the current second
  enemyX=random(500); //this randomly creates the horizontal position of the enemy
  enemyY=random(500); //this randomly creates the vertical position of the enemy
...
void game() { //this is a method that holds much of the game
  ...
  background(timer*25, timer*25, 0); //this redraws the background
  fill(255,0 ,0);
  rect(0,0,500,50);
  fill(0, 0, 255);
  text("Score: "+score, 250, 50); //this draws the score text and the score variable
  text("Timer: "+timer, 100, 50); //this draws the timer text and the timer variable
  fill(random(100), random(100), random(100)); //this sets the fill randomly
...
void mouseClicked() { //this is called any time the mouse is clicked
  if (mouseX<enemyX+give && mouseX>enemyX-give) { //this sees if the mouse is close to the enemy horizontally
    if (mouseY<enemyY+give && mouseY>enemyY-give) { //this sees if the mouse is close to the enemy vertically
      ...
      enemyX=random(500); //this places the enemy in a new position horizontally
      enemyY=random(500); //this places the enemy in a new position vertically
    }
  }
}
