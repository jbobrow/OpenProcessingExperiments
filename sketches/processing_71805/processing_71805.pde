...
void drawObject() {
  updateColor();
  ellipse(objectX, objectY, objectWidth, objectWidth);
}
...
void checkForCollision() {
  ...
  if (objectY > height - objectWidth) {
    objectY = height - objectWidth;
    speedY = -1 * speedY;
    speedY = DAMPING * speedY;
    speedX = FRICTION * speedX;
  }
  if (objectY < objectWidth/2.0) {
    objectY = objectWidth/2.0;
    speedY = -speedY;
    DAMPING = .9;
    speedY = DAMPING * speedY;
    speedX = FRICTION * speedX;
