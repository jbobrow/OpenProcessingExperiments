...
void setup(){
size(700,900);
background(255); // if you bring background to void draw, you wont be able to drwa a line from previous location
smooth();
frameRate(30);
}
...
//eyes
fill(mouseX,0,mouseY); // thr eyes will change colors acording to the mouse location
...
stroke (200);
fill (180, 200);
triangle(340,300,mouseX, mouseY,340,358);

}

void mousePressed(){ // handles mouse clickings, whenever you mouse click, a square comes out 
    stroke(0);
...
void keyPressed (){  // every time you press a key, the backround will change 
  background (mouseX, 0, mouseY, 100); // it will change the background color acording the the mouse location
  draw ();
    ellipse (mouseX, mouseY, 18, 18);  // and it will draw an ellipse 
