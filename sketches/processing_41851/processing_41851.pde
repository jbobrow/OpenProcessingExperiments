...
void draw(){
  ...
  //p2
  if(generalCounter==0){ //I'm using the lame way by having a image being rendered out of sight so then that image can be transformed and translated correctly!
    for(int k=width/2+10; k<width-gutter-xImgScaler*4; k=k+1*xImgScaler){ //using a repeat of 2 UNITS
      purpleTile=loadImage("imageblack.gif", "gif");
