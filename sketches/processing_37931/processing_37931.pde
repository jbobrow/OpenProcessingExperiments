...
void setup() {
    ...
    smooth();
    noLoop();
    PImage woodBG=loadImage("woodBG.jpg");
    image(woodBG,0,0,width,height);
}
    
void draw() {
    moodyJoe(100,100,width/3,height/3,1,true);
    moodyJoe(250,250,width/3,height/3,2,true);
    moodyJoe(400,400,width/3,height/3,3,true);
    }
...
void moodyJoe(int px, int py, int w, int h, int emotionNumber, boolean inEmotion) {
  ...
  int faceTemplate = (int)random(1,3);
  int faceX = x-w/2;
  int faceY = y-h/2;
  if(faceTemplate == 1) image(lightFace,faceX,faceY,w,h);
  else if(faceTemplate == 2) image(darkFace,faceX,faceY,w,h);
  else if(faceTemplate == 3) image(brownFace,x,y,w,h);
    
  if(inEmotion == false) {
    emotionNumber = (int)random(1,3);
  }
  //Eye coordinates
  int eW = w/3;
  int eH = h/6;
