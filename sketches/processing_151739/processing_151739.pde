 ...
 * Pressing the left and right arrow keys or drag mouse left-right changes the lifetime of all the circles 
 * globally.
 * 
 */
...
void setup() {
  ...
  size(1000, 800); // (displayWidth, displayHeight);
...
void movingCircle(float x, float y, float size, int offset) {
  ...
  float circleSize = size * 2.0 * sin(circleAge * HALF_PI) * 0.45;

  strokeWeight(2);
  stroke(0, 255, lerp(255, 0, circleAge), lerp(255, 0, circleAge) * 0.70);
  fill(lerp(128, 0, circleAge), lerp(120, 0, circleAge));
  ellipse(x-size/2+random(-5,5), y-size/2+random(-5,5), circleSize, circleSize);
}
