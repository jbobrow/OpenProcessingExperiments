...
void calcCurvature(int n, color c,int xNum){
  ...
  for(float t = 0;t<1.0;t+=0.01){
    ...
    float d2x = 6 * Ax * t + 2 * Bx;
    float d2y = 6 * Ay * t + 2 * By;
    
    float k = (d1x * d2y - d1y * d2x)
    / pow(sqrt( pow(d1x,2) + pow(d1y,2) ),3);
...
void draw(){
  ...
  calcCurvature(0,color(255,150,0),0); 
  calcCurvature(3,color(255,50,0),1); 
}
  ...
  if(selected != -1){
    ...
    if(shifted){
      if(selected == 3){
        ...
        pX[selected+2] = pX[selected];
        pY[selected+2] = pY[selected];
        ...
        pX[selected+4] = pX[selected+3] + (pX[selected]-pX[selected+1]);
        pY[selected+4] = pY[selected+3] + (pY[selected]-pY[selected+1]);
      }
      else if(selected == 5){
        pX[selected] = mouseX;
        pY[selected] = mouseY;
  ...
  if(selected != -1){
    ...
    if(shifted){
      if(selected == 3){
        ...
        pX[selected+2] += 2*(mouseX-pmouseX);
        pY[selected+2] += 2*(mouseY-pmouseY);
