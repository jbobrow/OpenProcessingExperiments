...
void page1() {
  ...
  popMatrix();
  //button size
  text("Click the box to alone",x-200, y);
  text("Click the box to wet",x-200, y+40);
  text("Click the box to drop",x-200, y+80);
  fill(255,0,0);
  rect (x,y,w,h);
  rect (x,y2,w,h);
  rect (x,y3,w,h);
...
void page3() {
  ...
  //button size
  text("Next wet event",x-300, y);
  text("Next drop event",x-300, y2);
  text("Next master's hand",x-300, y3);
  fill(0, 0, 255);
  ellipse(x, y, 40, 40);
  ellipse(x, y2, 40, 40);
  ellipse(x, y3, 40, 40);
...
void page5() {
  ...
  //button size
  text("Next drop event",x-300, y);
  text("Next master's hand",x-300, y2);
  fill(0, 0, 155);
  ellipse(x, y, 40, 40);
  ellipse(x, y2, 40, 40);
  //button size
...
void mousePressed() {
  ...
  switch(card)
  {
    ...
    if((x < mouseX) && (mouseX < x + w) && (mouseY > y) && (mouseY < y+ h)) {
      card = 1;
    }
    else if((x < mouseX) && (mouseX < x + w) && (mouseY > y2) && (mouseY < y2+ h)) {
      card = 3;
    }
    else if((x < mouseX) && (mouseX < x + w) && (mouseY > y3) && (mouseY < y3+ h)) {
      card = 5;
    }
    ...
    else if (dist(mouseX, mouseY, x, y2) < 20) {
      card = 5 ;
    }
    else if (dist(mouseX, mouseY, x, y3) < 20) {
      card = 0 ;
    }
