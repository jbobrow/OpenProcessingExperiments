...
iOS adjustments:http://stackoverflow.com/questions/5995210/disabling-user-selection-in-uiwebview
actual object
collision detection
shooting
shields
...
/* @pjs preload="starfield.jpg"; */
/* @pjs preload="starfield-6x6.jpg"; */

void setup(){
  size(600, 700); 
  verticalBound = height - controls; 
  starfield = loadImage("starfield-6x6.jpg"); 
  shipA = new Ship(100, 100); 
  buttons = new ArrayList<Button>(); 
  int offset = width/2 - 55; 
  btnAShoot = new RectButton(offset + 40, verticalBound +10, 20, buttoncolor, highlight); 
  btnALeft = new RectButton(offset + 20, verticalBound +30, 20, buttoncolor, highlight); 
  btnAShield = new RectButton(offset + 40, verticalBound +50, 20, buttoncolor, highlight); 
  btnARight = new RectButton(offset + 60, verticalBound +30, 20, buttoncolor, highlight); 
  btnAThruster = new RectButton(offset + 40, verticalBound +30, 20, buttoncolor, highlight);   
...
void draw(){
  ...
  image(starfield, 0, 0, width, verticalBound); 
...
class Button
{
  ...
  boolean over()
  {
    return false; // base implementation is always false
  }
