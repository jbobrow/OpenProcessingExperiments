...
void setup()  {
  ...
  line0 = new SketchLine(1);
  line1 = new SketchLine(1);
  line2 = new SketchLine(stoixeia + 5);
...
//  noCursor();
  ...
  for (int i=0; i<stoixeia; i++){
    elastikotita[i] = 0.15*(.27*(i+1));// 0.05  kai 0.005
...
void draw()  {
  ...
  if (mousePressed == true)  {  
    ...
    line1.render(0,0,0, lineAlpha);
    line2.calcPoints(mouseX, mouseY);
    line2.render(0,255,0, lineAlpha);
  } else {
      line0.calcPoints(mouseX, mouseY);
