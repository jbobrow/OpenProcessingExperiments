...
void draw() {
  ...
  float dataPoint=map(dogs[idx].deaths, 0, allDogMax, 0, plotHeight);
    float dataPointTwo=map(dogs[idx].attacks, 0, allDogMax, 0, plotHeight);

  fill(0);   

  rect(x, plotHeight-dataPoint, barWidth, dataPoint);
  text("Deaths", x, plotHeight-dataPoint+20);
  fill(#FF0000);
  rect(x+50, plotHeight-dataPointTwo, barWidth, dataPointTwo);
  text("Attacks", x+50, plotHeight-dataPointTwo+20);
