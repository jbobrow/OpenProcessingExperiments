...
void draw(){
  ...
  int min = minute();
  int hou = hour();
  int secCol = (sec*100)/60;
  int minCol = (min*100)/60;
  int houCol = (hou*100)/60;
  ...
  text("Colock: HSB",2,36);
  text("H: "+secCol,2,36*2);
  text("S: "+minCol,2,36*3);
  text("B: "+houCol,2,36*4);
  text(time[hou]+":"+time[min]+":"+time[sec],2,572);
}
