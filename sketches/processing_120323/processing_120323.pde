...
void draw() {
  ...
  noStroke();
  beginShape();
  vertex( floaters[0].location.x, floaters[0].location.y );
  for( int i = 0; i < floaters.length; i++ ) {
    ...
    floaters[i].checkEdges();
     if( show && i%2 != 0 ) {
        bezierVertex( floaters[i].location.x, floaters[i].location.y, floaters[constrain(i + 1, 0, 99)].location.x, floaters[constrain(i + 1, 0, 99)].location.y, floaters[constrain(i + 2, 0, 99)].location.x, floaters[constrain(i + 2, 0, 99)].location.y );
        //floaters[i].render();
     }
  } 
  endShape( CLOSE );
