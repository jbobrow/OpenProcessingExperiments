...
void draw(){
  background(50,50,55);
    ...
    for (int i=0; i<AnzahlRechtecke-1;i++){
      Reschtecke[i].draw();
     for(int j=i+1;j<AnzahlRechtecke;j++){
      if(Reschtecke[i].counter==Reschtecke[j].counter){
        Reschtecke[i].setColor(200,0,10);
        Reschtecke[j].setColor(200,5,10);
       }else{
        Reschtecke[i].setColor(0,200,10);
        Reschtecke[j].setColor(5,200,10);
      }
    }
  }
}
