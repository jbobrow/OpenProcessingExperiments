...
void drawAxis(){
  ...
  for(int i = 0; i < max(data); i=i+1000000){
...
void drawCircles() {
 ...
 for(int i = 0; i <= data.length-1;i++) {
   float diameter = sqrt(data[i]/PI)/20;
   noStroke();
   float h = 50;
   float s = 100-(i*5);
   float b = i*10;
   fill(h,s,b,50); //color circles
   ellipse(scaleX(width/2), height-scaleY(data[i]), diameter, diameter); //draw circle
   println(b);
   // text for city labels without opacity
   fill(h,s,b);
    
   if(i%2 <= 0){ //alternate label position
    textAlign(LEFT);
    text(labels[i], scaleX(width/2) + 30, height-scaleY(data[i]));
    } else {
      textAlign(RIGHT);
    text(labels[i], scaleX(width/2) - 30, height-scaleY(data[i]));
    }
...
//TITLE THE CHART
void title() {
  String title ="Top 10 Most Populated US Cities";
  fill(0);
  textAlign(LEFT);
  textSize(14);
  text(title, 25,50);
