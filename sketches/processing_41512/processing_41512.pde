...
float[] colour = {
  62, 87, 91, 100, 149, 199, 232, 237, 255, 186, 161,199,186,161};
float[] xc1 = {
...
void draw() {
  ...
  for (int i =0; i <8; i++) {
    if (i < 4) {
      ellipse(xc1[i]-42+temp1, yc1[i], csize, csize);
      ellipse(xc1[i]+42-temp1, yc1[i], csize, csize);
    }
    
    else {
      fill(colour[11],colour[12],colour[13]);
      ellipse(xc1[i]-42 +temp2, yc1[i], csize, csize);
      ellipse(xc1[i]+42 -temp2, yc1[i], csize, csize);
    }
  }
  ...
  fill(colour[7], colour[4], colour[0]);
