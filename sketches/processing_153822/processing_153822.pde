...
// my version - music and wings
//changed music/speed settings and brush
...
void setup()
{
  ...
  player = maxim.loadFile("prettypiano.wav");
  player.setLooping(true);
  player2 = maxim.loadFile("birdie.wav");
...
void mouseDragged()
{
  ...
  float alpha = map(speed, 0, 100, 0, 200);
  //println(alpha);
  ...
  brush12(mouseX, mouseY,speed, speed,lineWidth);//mirroredwings
...
void brush12(float x,float y, float px, float py, float lineWidth) {
  strokeWeight(1); //mine wings
  px=px+random(110);//makes ellipse longer  70
  py=py+random(70); //make ellipse more round` 30
  fill(255,200,200,90);//pinkfill
  ellipse(x,y,px,py);
