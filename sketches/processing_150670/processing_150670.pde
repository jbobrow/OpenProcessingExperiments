...
void update(){
  ...
  */
  ...
  textSize(30);
    ...
    textSize(60);
    text("Velocidad:"+vel, width/2-750, height/50-500);
    
    fill(0);
    textSize(30);
    text(""+vel, location.x-70, location.y-70, location.z);
  vel.sub(PVector.mult(acc, dt));
  if( key== 'l' ){
     
    location.sub(PVector.mult(vel,dt));//Euler, primero posicion y luego velocidad(de las particulas)
