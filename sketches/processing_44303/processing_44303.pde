...
//EJ Posselius - Processing.3 - 2011.10.30 - v4.2
...
//     s1 = round(map(s1,-75.,75.,10.,209.))/10;
     ...
     hs2.display();     
     fill(0,10);
     text("Y Variable",10,2*c-20);
//     s2 = map(s2,75.,-75.,0+s3,height-s3);
     s2 = map(s2,-75.,75.,0.01,1.25);
...
//      vy = -1;      
      ...
      for(float i=0.; i<2*PI; i+=0.01) {
        ...
        float a = map(sin(i+vx), -1., 1.,0,width-(d));
        float b = map(sin(i+vy), -1., 1., 0, height-(d));
