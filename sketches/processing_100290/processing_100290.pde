...
for (int j = 0; j < 10; j = j + 1) { //all this stuff draws the blocky background with mouse location sensitivity
   ...
   for (int i = 0; i < 10; i = i + 1) {
     ...
     float mouseBrightness = 1 - mouseDistance/sqrt(200); //establish brightness factor, in terms of % of maximum possible distance from  a square.
     fill(255*(mouseBrightness)*(i/10.), 255*(mouseBrightness)*(j/10.),random(255)); //sets R&G values - increasing across window, but modified by mouse if nearby.
     stroke(0,0);
