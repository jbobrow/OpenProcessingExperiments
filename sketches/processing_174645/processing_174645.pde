...
PVector lTop, PVector mTop, PVector rTop) {
  ...
  float u1 = bernstein(lBottom.x, mBottom.x, rBottom.x, 10*u); 
  float u2 = bernstein(lMiddle.x, mMiddle.x, rMiddle.x, 10*u); 
  float u3 = bernstein(lTop.x, mTop.x, rTop.x, 10*u);
  pos.x = bernstein(u1, u2, u3, v);
  u1 = bernstein(lBottom.y, mBottom.y, rBottom.y, 10*u); 
  u2 = bernstein(lMiddle.y, mMiddle.y, rMiddle.y, 10*u);
  u3 = bernstein(lTop.y, mTop.y, rTop.y, 10*u);
