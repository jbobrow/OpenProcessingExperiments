...
/* OpenProcessing Tweak of *@*http://www.openprocessing.org/sketch/44558*@* */
...
void setup(){//set up
  size (980,600,P2D);
 //   size (1920,1080,P2D);
  Hexagon firstHexagon = new Hexagon (new PVector(width/2,height/2),10,null); //make start hexagon
  background(255);
  stroke(255);
  smooth();
}
...
void render(){//draw this hexagon
 age++;
 fill(abs(cos(age/250.0))*255,abs(sin(age/200.0))*255,136,0+(age*20));//change fill colour by age counter
...
PVector newpos = new PVector(pos.x + (cos(angle)*distance), pos.y + (sin(angle)*distance));//decide a position for growth hexagon
 ...
 for (Hexagon checkhexagon : hexagons){ // run through the hexagons list, and check each one
    ...
    Hexagon newhexagon = new Hexagon(newpos, rad, this  ); // make a new hexagon
