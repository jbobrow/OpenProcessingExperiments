...
void setup() {
  ...
  robotWally = new dodgePlayer(60, 50, 1, loadImage(robotUrl), 0);
...
void isGameOver() {
  ...
  if ( numRound-1 > maxRound-moreWins ) {
...
void ballCollision() {
  if ( ball.speed == 0 && ball.collision(one.x, one.y) && !playerTwoHasBall) {
    playerOneHasBall = true;
    ball.x = one.x+13;
    ball.y = one.y+18;
  }
  if ( ball.speed == 0 && ball.collision(two.x, two.y) && !playerOneHasBall ) {
...
void resetMatch() {
  ...
  if ( roundOver ) {
    if ( numRound != 0 ) { // initial game state ( Round 1 )
      if ( URF ) {
        rideRobot = false;
        robotWally = new dodgePlayer(width/2, 50, 1, loadImage(robotUrl), 0);
      }
...
void checkField() {
  ...
  if ( ball.x > width-65 && !(playerOneHasBall || playerTwoHasBall) ) {
    ball.speed = 0;
    ball.x -= 130;
  }
  if ( ball.x < 20  && !(playerOneHasBall || playerTwoHasBall) ) {
    ball.speed = 0;
    ball.x += 130;
  }
}
...
void keyPressed() {
  if ( key == 'w' || key == 'W') {
    ...
    else if ( one.collisionRobot(robotWally.x, robotWally.y) ) {
      one = robotWally;
      rideRobot = true;
    }
  } 
...
class dodgePlayer {
  ...
  boolean collisionRobot(int h, int v) {
    return ( h > x-imagePlayer.width && h < x + imagePlayer.width && v > y - imagePlayer.height && v < y + imagePlayer.height);
  }
