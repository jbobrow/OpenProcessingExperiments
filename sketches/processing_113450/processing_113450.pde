...
void setup() {
  ...
  elapsedTime = millis();
  bouncyThing = new Bouncy("http://29.media.tumblr.com/tumblr_m05t2z9a1k1roy4svo1_400.gif");
...
void draw() {
 ...
 translate(-width/2, -height/2);
  ...
  float dist = sqrt(mouseX * mouseX + mouseY * mouseY);
  float maxDist = compositionSideLength * sqrt(2); 
  ...
  if (bouncyThing2 != null) {
    ...
    if (millis() - elapsedTime > 5000) {
      bouncyThing2 = new Bouncy("http://29.media.tumblr.com/tumblr_m05t2z9a1k1roy4svo1_400.gif");
    }
  }
}  

void keyPressed() {
  angle = (angle + (PI / 12)) % (2 * PI); 
}
