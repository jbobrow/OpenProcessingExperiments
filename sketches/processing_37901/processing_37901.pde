...
void draw() {
  background(120, 120);
  for(int i = 60; i < width; i = i + 120){
    for(int j = 60; j < height; j = j + 120){
      float fill1 = map(i, 0, width, 0, 15.96);
      float fill2 = map(j, 0, height, 0, 15.96);
      placeRubberStamp(i, j, fill1 * fill2);
    }
  }
}
