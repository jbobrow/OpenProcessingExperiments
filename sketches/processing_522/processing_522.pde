  ...
  void drawTicks()
  {
    ...
    for( int i = 0; i <= numberOfSubdivisions; i++ )
    {
      ...
      if( drawLabels )
      {
        ...
        {
          // is there a test to see if we are upside down?
          // if there is, then put it here to make right side 
          // up all the time.
          pushMatrix();
          translate( 7, 7 ); // make off center so we can see nums
          scale( 1, -1 ); // hack to get rid of upside-down fonts
          // note here that the labels are now wrong from the
          // outside, because of the environment's translations.
