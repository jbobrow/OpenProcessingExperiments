...
class Humanatee {
  ...
  Humanatee(){
    ...
    speedX = int(random(width));
    speedY = int(random(height));
  ...
  void swim(){
   if(xPos < humanateeWeight/2 || xPos > width - humanateeWeight/2) {
     speedX = -speedX;
     xPos += speedX;
   }
     yPos += speedY;
  if (yPos < humanateeHeightMult/2 || yPos > height - humanateeHeightMult/2){
    speedY = -speedY;
    yPos += speedY;
  }
