...
Dog sketchDog; //declare dog object as a global variable
...
class Flies {
  ...
  void display() {
    for (int i=0; i<3; i++) {
      drawFly(random(-30, 30), random(-30, 30), random(0.0001, 0.95), 0, 0);
    }
  }

  void drawFly (float ttranslateX, float ttranslateY, float teasing, int tx, int ty) {
    pushMatrix();
