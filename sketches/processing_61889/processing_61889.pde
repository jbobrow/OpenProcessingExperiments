...
class Spot {
  ...
  float radius = 35.0;  // Radius of the circle
  float x = random(radius, width-radius);       // X-coordinate
  float y = random(radius, height-radius);       // Y-coordinate
  float speedX = random(0.4, 5.0);   // Speed of motion on the x-axis
  float speedY = random(0.4, 5.0);   // Speed of motion on the y-axis
