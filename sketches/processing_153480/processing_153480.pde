...
void setup() {
  ...
  for(int i = 0; i < 5; i++) {
    ...
    db.size.y = 30 + floor(random(4.0))*30.0 + random(20);
    boxes.add(db);
  }
  rectMode(CENTER);
}
...
class DragBox {
  ...
  void update() {
    ...
    if(!dragged) {
      ...
      // avoid others and the sides
      vel.x *= 0.9;
      if(pos.y > groundY - 10.0 - size.y*0.5) {
          for(DragBox db : boxes) {
            if(!db.dragged) {
              float difx = db.pos.x - pos.x;
              if(difx != 0.0 && abs(difx) < 20.0 + size.x*0.5 + db.size.x*0.5) {
                if(difx < 0) {
                  vel.x = 2.0;
                }
                if(difx > 0) {
      ...
      if(pos.x < size.x*0.5) vel.x = 2.0;
      if(pos.x > width - size.x*0.5) vel.x = -2.0;
      // move
      pos.add(vel);
    }
  }
