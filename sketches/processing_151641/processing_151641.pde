
/*
 * Creative Coding
 * Week 2, 06 - Moving Patterns 2
 * by Indae Hwang and Jon McCormack
 * Copyright (c) 2014 Monash University
 *
 * Similar to the previous sketch, this sketch draws a grid of oscillating circles. Each circle has a "lifetime"
 * over which it grows and changes intensity and opacity. At the end of each lifetime the circle begins again.
 * Pressing the left and right arrow keys changes the lifetime of all the circles globally.
 * 
 */

// variable used to store the current frame rate value
int frame_rate_value;

void setup() {

  // make the display window the full size of the screen
  size(600, 600); //changed for easier viewing

  frame_rate_value = 11; //any framerate above 12 seems to cancel out having the wavy across movement 
  rectMode(CENTER);
  background(0);
}


void draw() {

  background(#06BCB1);

  int num = 10;
  int margin = 10;
  float gutter = -10; //distance between each cell
  float cellsize = ( width - (2 * margin) - gutter * (num - 1) ) / (num - 1);

  int circleNumber = 2; // counter
  for (int i=0; i<num; i++) {
    for (int j=0; j<num; j++) {
      circleNumber = (i * num) + j; // different way to calculate the circle number from w2_04

      float tx = margin + cellsize * i + gutter * i;
      float ty = margin + cellsize * j + gutter * j;

      movingCircle(tx, ty, cellsize, circleNumber);
    }
  }
} //end of draw 


void movingCircle(float x, float y, float size, int offset) {

  float circlePeriod = (float)frame_rate_value;
  float circleAge = (float)((frameCount + offset) % (int)circlePeriod) / circlePeriod;

  float circleSize = size * 2.0 * sin(circleAge * HALF_PI);

  strokeWeight(20); //thickness of line that makes the circles
  stroke(#FFA703, lerp(255, 0, circleAge));//stroke circle outline colour 
  fill(#FCEABA); //fill for inside the circle
  ellipse (x-size/2, y-size/2, circleSize, circleSize);
}


/*
 * mousePressed function
 *
 * called automatically by Processing when the mouse is pressed either left or right
 */
void mousePressed() {
 
  // right of middle -- increase frame_rate_value
  if (mouseX > width/2  && frame_rate_value < 120) {
    frame_rate_value++;
  }
 
  // left of middle -- decrease frame_rate_value
  if ( mouseX < width/2 && frame_rate_value > 2) {
    frame_rate_value--;
  }
 
  // print the current value on the screen
 // println("Current frame Rate is: " + frame_rate_value); //mouse clicking seems to only move the frame rate up and then everything just looks like you're on acid in a bad motel bathroom 
}

