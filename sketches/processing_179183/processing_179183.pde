...
void addActiveSticker() {
  if (layers.size() < stickers.length) {
    ...
    } while (layers.hasValue(newStickerIndex));
    layers.append(newStickerIndex);
...
void moveStickerDown() {
  if (currentSelection > 0) {
    ...
    layers.remove(currentSelection);
    currentSelection--;
    layers.insert(currentSelection, thisStickerIndex);
  }
}
