...
void setup() {
  size(600, 400);
  lines = loadStrings("arewethereyet.txt");
  joinedText = join(lines, " ");


  font = loadFont  //("Courier", 40);
...
//------------------------------
  ...
  for (int i = 0; i < joinedText.length(); i++) {
    ...
    float sortY = index*10+10;
    float m = map(mouseX, 10,width-70, 0,1);
    m = constrain(m, 0, 1);
    ...
    if (drawLetters[index]) {
      if (drawLines) {
        if (oldX!=30 && oldY!=0) {
          stroke(random (216), random (0), random(28), 100);
              strokeWeight(2);
    ...
    if (posX >= width+10 && uppercaseChar == ' ') {
      posY += 50;
      posX = 10;
    }
  }
