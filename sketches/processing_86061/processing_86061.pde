...
//  load images
  ...
  background = loadImage("nightSky.png");
  blue = loadImage("blueScreen.jpg");
  enemy = loadImage("Bosconian_Enemy_Base.png");
  background(0);
// Set starting Image
//  imageMode(CENTER);
...
void enemy() {
//  image(background, width/ 2, height/2);here
  if(direction == 1) {
  image(enemy, enemyX, enemyY, enemy.width/ 1.7, enemy.height/ 1.7);
  enemyY= enemyY +1.5;
}
  if(direction == 2) {
  image(enemy, enemyX, enemyY, enemy.width/ 1.7, enemy.height/ 1.7);
...
void back(){
  imageMode(CENTER);
  image(background,widthX, heightY);
  if(direction == 1) {
    heightY = heightY +2;
  }
  if(direction == 2) {
