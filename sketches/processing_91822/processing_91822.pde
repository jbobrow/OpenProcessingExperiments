...
final int curtainWidth = 128;
final int yStart = 45; // where will the curtain start on the y axis?
final float restingDistances = 10;
final float stiffnesses = 1.5;
final float curtainTearSensitivity = 200; // distance the particles have to go before ripping
...
//for second row animated with sound
/*    
    if(sound == 1)
    {
      for(int i = curtainWidth; i < 2*curtainWidth; i++) 
      {                               
          Particle particle = (Particle) particles.get(i);                      
          particle.setPosition(in.left.get((i-curtainWidth)*8)*120);            
      }      
    }
*/  
    if(sound == 1)
    {
      for(int i = (curtainHeight/2)*curtainWidth; i < (curtainHeight/2)*curtainWidth+curtainWidth; i++) 
      {                                           
          Particle particle = (Particle) particles.get(i);                      
          particle.setSoundResponse(in.left.get((i-(curtainHeight/2)*curtainWidth)*8)*120);            
      }      
    }           
...
void createCurtain () {
  ...
  // not that it isn't possible using an array, it's just more convenient this wayyyy
...
*/
      ...
      if (y==curtainHeight)
      {        
        particle.pinTo(new PVector(particle.position.x, curtainHeight * (restingDistances + 3)));
      }
      
      // we pin the very top and bottom particles to where they are
      if (y == 0){
      ...
      particles.add(particle);
        if (y == curtainHeight/2) println(particles.size());
    }
  }
}
