...
int startX = 50;
int startY = 600;
int emissionStartX = 500;
int emissionStartY = 320;
int emissionW = 10;
int emissionRatio = 50;
PFont font;
PFont fontBold;
color c;
boolean over = false;
...
void setup(){
  ...
  String[] data_in2 = loadStrings("data2.csv");   // loading in the csv file
  emissionsData = new int[data_in2.length];
  for (int i = 1; i < data_in2.length; i++){// start at 1 - skip the first row
     String thisLine = data_in2[i]; // split the data apart into lines
     String[] bits = thisLine.split(","); // split this line at the commas
     
     String emissions = bits[1]; 
     emissionsData[i] = int(emissions);
  }
  emissionsBars = new EBar[19];
  for (int i = 1; i < emissionsBars.length; i++){
    emissionsBars[i] = new EBar((emissionStartX + emissionW*i + i*2) - emissionW, emissionStartY, emissionsData[i]/emissionRatio); 
  }
...
void draw(){
  ...
  for (int i = 1; i < emissionsBars.length; i++){
    c = color(0, 200);
    fill(c);
    emissionsBars[i].render();
    emissionsBars[i].update();
    if (eover == true){
      rect(startX + w*i + i*2, startY, w, -totalData[i]/widthRatio);
    }
  }
  ...
  for (int i = 1; i < totalBars.length; i++){
    totalBars[i].update();
    if (over == true){
      rect((emissionStartX + emissionW*i + i*2) - emissionW, emissionStartY, emissionW, -emissionsData[i]/emissionRatio/emissionRatio);
    }
  }
