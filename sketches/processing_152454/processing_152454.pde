...
void movingCircle(float x, float y, float size, int offset) {
  ...
  float circleSize = size * 2.0 * cos(circleAge * PI);

  strokeWeight(2);
  stroke(random (255), lerp(255, 0, circleAge));
  fill(200, 10, 10);//(lerp(128, 0, circleAge), lerp(120, 0, circleAge));
  ellipse(x-size/2, y-size/2, circleSize, circleSize);
}
void mousePressed(){
  
  // clicking on left side of display screen
if(mouseX <= width/2 && frame_rate_value <100){
    frame_rate_value++;
  }
  // clicking on right side of display screen
