...
void keyPressed() {
  if (key == ' ') {
    saveFrame("perlinKaleidoscope-####.png");
  }
  if (key == CODED) {
    if (keyCode == UP) {
      background(0);
...
class Particle {
  ...
  void update() {
    velocity.x = 0.5*cos(TWO_PI*noise(0.01*position.x,0.01*position.y,0.011*position.z));
    velocity.y = 0.5*sin(TWO_PI*noise(0.01*position.x,0.01*position.y,0.01*position.z));
