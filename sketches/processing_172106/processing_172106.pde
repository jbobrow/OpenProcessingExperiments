...
float depth = 300;// acompanies width and height
...
int iterationsPerFrame = 1;// speeds up the sim
...
//lights();
  ...
  PVector[] allVerts =new PVector[0];
  for(int i=0;i<cubes.length;i++){
...
//    beginShape(TRIANGLES);
    // there are 6 sides, 2 triangles per side and 3 corners per triangle
    for(int j=0;j<6*2*3;j++){  // there must be a better way to do this
...
// z axis is inverted for rendering purposes
  allVerts=(PVector[])(append(allVerts,new PVector(verts[vertA[j]][vertB[j]][vertC[j]].x,verts[vertA[j]][vertB[j]][vertC[j]].y,-verts[vertA[j]][vertB[j]][vertC[j]].z)));
    }//    endShape();
  }
...
//Render the shadow:
 background(255);
  ...
  fill(100);
  beginShape(TRIANGLES); for(int i=0;i<allVerts.length;i++){vertex(allVerts[i].x,allVerts[i].z+height);}  endShape();
...
if(javaMode){
 ...
 fill(#FF8800); beginShape(TRIANGLES); for(int i=0;i<allVerts.length;i++){vertex(allVerts[i].x,allVerts[i].y,allVerts[i].z);}  endShape();
  }
  else {   int fillCounter=96;//Special javascript workaround because lightning is not working on vertex in javascript
   beginShape(TRIANGLES);  
   int step=6;
   for(int i=0;i<allVerts.length;i+=step){   fill((fillCounter/3)*9,(fillCounter/3)*6,(fillCounter/3)*3);
    for(int j=0;j<step;j++){  vertex(allVerts[i+j].x,allVerts[i+j].y,allVerts[i+j].z); }
     fillCounter--;   }  
    endShape();  }
