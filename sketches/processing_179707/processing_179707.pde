...
feel free to play with physic constants (cf. physic functions)
...
void acceleration() {
  ...
  ay2 = 0;
  for (int i = 0; i < nAttractionPoints; i++) {
    ax1 += dx1[i] * .02 / pow(d1[i], .01) ;  
    ay1 += dy1[i] * .02 / pow(d1[i], .01) ;
    ax2 += dx2[i] * .02 / pow(d2[i], .01) ;  
    ay2 += dy2[i] * .02 / pow(d2[i], .01) ;
  }
}

void nextStep() {
  acceleration();
  vx1 = vx1 + ax1 - vx1 / 10000; 
  vy1 = vy1 + ay1 - vy1 / 10000;
  vx2 = vx2 + ax2 - vx2 / 10000; 
  vy2 = vy2 + ay2 - vy2 / 10000;
  y1 = y1 + vy1; 
  x1 = x1 + vx1;
  y2 = y2 + vy2; 
  x2 = x2 + vx2;
