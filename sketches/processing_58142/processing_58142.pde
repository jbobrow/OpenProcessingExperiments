...
float speed = 0.001; // 0.0 - 0.5 (the speed at which the perlin noise "goes by")
float noiseScale = 0.11; // 0.03 - 0.15 (the amount of perlin noisiness (or smoothness if you prefer))
float scaling = 3.0;
...
class hexagon {
  ...
  void show() {
    ...
    noiseScale+=0.05;
    float c1 = noise(pos.x*noiseScale, (pos.y+counter)*noiseScale); 
    noiseScale+=0.05;
    float c2 = noise(pos.x*noiseScale, (pos.y+counter)*noiseScale); 
    fill(c*255,c1*255,c2*255); //c is always somewhere between 0 and 1, so we just multiply it by 255 for the grayscale
    pushMatrix();
