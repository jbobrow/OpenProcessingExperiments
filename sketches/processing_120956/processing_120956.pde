...
static class EdgeDetector
{
    ...
    static void extractPoints(List <PVector> vertices, PImage img, int k, int treshold, int res) 
    {     
          int col = 0, colSum = 0, W = img.width-1, H = img.height-1;
            
          // For any pixel in the image excepting borders convolute surrounding pixels with desired operator         
          ...
          {  
              for (int y = -1; y <= 1; y++) for (int x = -1; x <= 1; x++, col = img.get((X+x), (Y+y))) 
                  colSum +=  K[k][x+1][y+1] * ((col>>16 & 0xFF)+(col>>8 & 0xFF)+(col & 0xFF));                           
