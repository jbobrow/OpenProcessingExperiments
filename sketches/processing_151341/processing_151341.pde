...
void draw() {
  ...
  background(#33B477); // clear the screen to grey
  //int intest = 4/3;  
    ...
    for (int j=0; j<num; j++) {  // rows
     ...
     //println("sometimes = " + sometimes);
      if (sometimes == 3) {
        ...
        fill(#481414, random(255)); // rectangle
       }
        rect(gap * (i+1) + cellsize * i + random(-10, 10) + blankBorder/2, gap * (j+1) + cellsize * j + blankBorder/2 + random(-10, 10), cellsize, cellsize, random(-250, 70), random(-180, 30), random(-240, 90), random(-150, 240));       
