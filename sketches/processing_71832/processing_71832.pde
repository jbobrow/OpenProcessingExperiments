 ...
 */
 ...
 //These variables allow for the changing of the ellipse color
 int colorRed;
 int colorGreen;
 int colorBlue;
...
void drawObject() {
  ...
  ellipse(objectX+5, objectY+10, objectWidth, objectHeight);
}
...
void checkForCollision() {
  ...
  if (objectY > height - objectHeight) {
    ...
    //objectHeight = height + objectHeight;
    speedY = -1*speedY;
    speedY = DAMPING * speedY;
    speedX = FRICTION * speedX;
    }
...
void updateColor() {
colorRed = (int)random(0,256);
colorGreen = (int)random(0,256);
colorBlue = (int)random(0,256);
