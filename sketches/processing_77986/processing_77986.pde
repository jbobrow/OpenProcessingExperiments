...
PImage CAT;// cat
PImage mouse;// mouse
PImage cheese;// cheese

DrawingLine l;
void setup() {
img = loadImage("Background_Rug_Tile.jpg"); //background
CAT = loadImage("CAT.png"); // cat
mouse = loadImage("mouse.png"); //mouse
cheese = loadImage("cheese.png"); //cheese
...
//draw the line first
  ...
  if (l.havePoints() == true) {
    ...
    fill(255);
   image(mouse, position.x, position.y, 52, 80); //MOUSE

    time_to_check = millis() + 400; //SPEED of it?
    position = new PVector();
    position = l.getPositionFromTime(time_to_check);
    fill(255, 244,0);
    image(cheese, position.x, position.y, 62, 56);//CHEESE 
     
    time_to_check = millis() - 500; //draw ellipse half a second behind the middle
    position = new PVector();
    position = l.getPositionFromTime(time_to_check);
    fill(255, 255, 255);
    image(CAT, position.x, position.y, 94, 110); //CAT
  }
}
void mousePressed() {
