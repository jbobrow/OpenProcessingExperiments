...
void setup()
{
  ...
  rcarray = new int[lines+1];
  gcarray = new int[lines+1];
  bcarray = new int[lines+1];
  background(255);
  size(500,500);
  stroke(0);
for(int i=lines; i>0; i--){
  ...
  rcarray[i]=int(random(0,255));
  gcarray[i]=int(random(0,255));
  bcarray[i]=int(random(0,255));
}
}
...
void move(int index)
{
  stroke(rcarray[index],gcarray[index],bcarray[index]);
