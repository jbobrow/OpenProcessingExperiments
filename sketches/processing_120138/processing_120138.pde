...
void draw()
{
  ...
  {
    image(collisionImage, 250, 250, width, height);
    if (displayImage.width > 0)
    {
      image(displayImage, 250, 250, width, height);
    }
    ...
    for (int y = 0; y < collisionImage.height; y += 1)
    {
      for (int x = 0; x < collisionImage.width ; x += 1)
      {
        ...
        float tileSize = 10;
        float playerSize = 10;
        if (isRectOverlapping(platformX, platformY, platformX + tileSize, platformY + tileSize, px - knight.w * 0.5, py - knight.h * 0.5, px + knight.w/2, py + knight.h/2) == true && red(pixelColor) == 0)
        {
        ...
        px = playerX;
        py = nextY;
        if (isRectOverlapping(platformX, platformY, platformX + tileSize, platformY + tileSize, px - knight.w * 0.5, py - knight.h * 0.5, px + knight.w/2, py + knight.h/2) && red(pixelColor) == 0)
        {
          fill(255, 0, 0);
