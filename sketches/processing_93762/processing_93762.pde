...
Generator uses text from JayZ, Biggie and Tupac lyrics based on the Keywords of the film The Master
...
if (screenState == homePage){
  intro();
  println("we are now home");
  }

 else if (screenState == jayzState) {
    jayz();
    println("jayZ running");
     
    }//end JayZ Button
    
  else if (screenState == biggieState) {
    biggie();
    }//end Biggie Button  
      
  else if (screenState == tupacState) {
    tupac();
    }// end Tupac Button
...
void mousePressed() {
  //when mouse is inside the Jayz Image and mouse is pressed, turnON JayZ button
  if (mouseX > 0 && mouseX < imgW && mouseY > 0 && mouseY < imgH) {
    screenState = jayzState;
    println("JayZ live");
  }  
  // clicking Biggie
  if (mouseX > 0 && mouseX < imgW && mouseY > imgH && mouseY < imgH*2) {
    screenState = biggieState;
  }  
    // clicking Tupac
 if (mouseX > 0 && mouseX < imgW && mouseY > imgH*2 && mouseY < imgH*3) {
    screenState= tupacState;
  }  
...
void jayz () {
     frameRate(speed); // frameRate speed (default 0.08 frames per second)
     pause(); // return to home screen function
...
String [][] tupacLyrics = {
 ...
 "You might be deep in this game, but you got the rules missin, Niggaz be actin like they savage, they out to get the cabbage",
