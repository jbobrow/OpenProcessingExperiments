...
void draw() {
  ...
  float sWidth=textWidth(mousePosition);
  float sHeight= textAscent()+textDescent();

  if (mouseX+sWidth > width && mouseY>sHeight) {
    text(mousePosition, mouseX-sWidth, mouseY);
  }
  if (mouseX+sWidth > width && mouseY<sHeight) {
    text(mousePosition, mouseX-sWidth, mouseY+sHeight);
  }
  if (mouseX+sWidth < width && mouseY>sHeight) {
    text(mousePosition, mouseX, mouseY);
  }
  if (mouseX+sWidth < width && mouseY<sHeight) {
    text(mousePosition, mouseX, mouseY+sHeight);
  }
