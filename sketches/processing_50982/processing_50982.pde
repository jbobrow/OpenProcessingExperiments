...
int longueurDesRayonsSoleil = 44;
...
while ( rads < TWO_PI ) 
{ 
  line(320, 75, 320+cos(rads) * longueurDesRayonsSoleil, 75 + sin(rads) * longueurDesRayonsSoleil);
  rads = rads + tranche;
}
...
while ( d > 5 ) {
  ...
  xDSoleil = w*scale + w*(1.0-scale) + offsetx; //
  yDSoleil = h*scale + h*(1.0-scale) + offsety;//
  fill ( red, green, blue );
  ellipse( xDSoleil, yDSoleil, r, r );
  d = d - difference;
}
