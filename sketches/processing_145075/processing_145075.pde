...
void draw() {
  ...
  for (int i = 0; i < bezcritters.length; i++) {
    ...
    bezcritters[i].swimUp();
  }

  for (int i = 0; i < bezonelegs.length; i++) {
    bezonelegs[i].display();
    bezonelegs[i].swimDown();
  }
  ...
  if (timer1.isFinished()) {
    ...
    bezonelegs = (BezOneLeg[]) append(bezonelegs, myBezOneLeg);
