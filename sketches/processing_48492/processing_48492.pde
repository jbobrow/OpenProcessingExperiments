...
void sineFunction()
{
  ...
  float mySec = second()*2;
  time = time +.01; //speed
  rectHeight = mySec * sin(time * PI); //amplitude, frequency


  if (rectHeight > 0)
  { 
    fill(130 );
    rect(180, 200, 40, -rectHeight);
  }
...
private void waterFilter() {
  for (int x = 0; x < width; x++) {
    for (int y = 0; y < height; y++) {
      ...
      // water filter. I used to think that this effect
