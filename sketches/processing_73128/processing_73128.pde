...
int nmax = 100;
int stampN = 0;
int[] stampX = new int[nmax]; //stamp X position
int[] stampY = new int[nmax]; //stamp Y position
float[] stampR = new float[nmax]; //stamp rotation
float[] stampSi = new float[nmax]; //stamp size
int[] stampSe = new int[nmax]; //stamp seed
...
void setuphandle() {
  size( dim, dim );        // size of the window
  smooth();                // antialiasing 
  rectMode(CENTER);
  ellipseMode(CENTER);
  strokeWeight(1);
...
void drawhandle() {
  ...
  makestamps();
  //Follow mouse
  makecreature(mouseX, mouseY, 128, radjust, sadjust, seed);
  biteon();
}
...
void addstamps() {
  ...
  stampR[stampN] = radjust;
  stampSi[stampN] = sadjust;
  stampSe[stampN] = seed;
  stampN ++;
}
