...
void draw() {
  ...
  int num = (int) random(3, 15);   // select a random number of squares each frame
  int gap = (int) random(5, 15); // select a random size of the gap between each square
  ...
  // shapeShadow = 1;  // Changing the shapeShadow-variable doesn't work, it keeps the same number 
    ...
    for (int i=0; i<num; i++) {
      for (int j=0; j<num; j++) {
        ...
        shapeShadow =  (int) random(1,4); // random number assigned to shapeShadow variable
