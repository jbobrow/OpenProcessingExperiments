...
float shade(radius, d) {
  return pow(radius-d, 1/8)*255;
}

float disc(x, y, center_radius, center_angle) {
  // compute the distance to the center
  float center_x = cos(center_angle)*center_radius;
  float center_y = sin(center_angle)*center_radius;
  float d = dist(x, y, center_x, center_y);

  float radius = 1;
  return d < radius ? shade(radius, d) : 0;
}

color rgb(x, y, t) {
  float cr = cos(t)*1.25;

  int r = disc(x, y, cr, t+TWO_PI);
  int g = disc(x, y, cr, t+TWO_PI/3);
  int b = disc(x, y, cr, t+TWO_PI/3*2);
