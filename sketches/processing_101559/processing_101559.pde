...
frameRate(30);
colorMode(HSB,100);
qs=random(-0.003,0.003);
ss=random(-0.003,0.003);
ds=random(-0.003,0.003);
fs=random(-0.003,0.003);
...
void draw()
{
 ...
 if(d>complexity || d<0.1)
  ...
  a+=step;
  PVector p2=new PVector(r*cos((q+1)*a)*cos((s+1)*a),r*sin((d+1)*a)*sin((f+1)*a));
  stroke(8,100,100,20);
  float greyness = dist(p1.x,p1.y,p2.x,p2.y)*0.5;
  strokeWeight(max((20/icount)-dist(p1.x,p1.y,p2.x,p2.y)*(1/moy),1));
