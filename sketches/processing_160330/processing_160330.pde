...
void Undo(){
  stones0[(turn-undones/2)%2][Moves.get(move-undones-2)][Moves.get(move-undones-1)] = 0;
  undones+=2;
}

void Redo(){
  stones0[(turn+undones/2)%2][Moves.get(move-undones-2)][Moves.get(move-undones-1)] = 1;
  ++turn;
  undones-=2;
}
