...
void setup(){
  ...
  salmon[i] = new Fish(random(width),random(height),35,120); 
  }
...
class Air{
  ...
  float posX, posY, fposX, rad, frad;
  
  int timer = 0;

 Air(float x, float y, float r){
...
void display(){
  ...
  stroke(255,5);
  fill(255,2);
  ellipse(fposX,posY,frad,frad);
  noStroke();
  fill(255,1);
  ellipse(fposX-rad/8,posY,frad-rad/2,frad-rad/4);
...
bubbles = new ArrayList<Air>();
  ...
  void update(float x, float y){
    ...
    Air bubble = bubbles.get(i);  //name and initialize the new object!
    bubble.update();
    
    if(bubbles.get(i).posY <= -25){
    bubbles.remove(i);
    }
    }
  ...
  if(counter >= ranStop){
    xx = random(width);
    yy = random(height);
    bubbles.add(new Air(fxx,fyy,fishSize/3));
