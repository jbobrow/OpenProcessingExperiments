...
class Intersection {
  ...
  void draw() {
    fill(SIDEWALK);
    rect(x - 3, y - 3, ROADWIDTH + 6, ROADWIDTH + 6);
    fill(ASPHALT);
  ...
  void draw() {
    if (isZombie) {
      if (sleepTime > 0) {
        ...
        fill(lerpColor(color(0, 255, 0), color(50, 200, 30), min(1.0, zombieTime / 200.0)));
        rect(x, y, 6, 6);        
      }
    } else {
      fill(hairColor);
