...
void setup()
{
  ...
  {
    bleep[ii] = new SineWave(200 - ii*10, 0.1, 5000);
  }
  ...
  mousemove = new SineWave(50, 0.1, Aout.sampleRate()); // create a sine wave at 100 Hz, 0.1 amp, sample rate from audio out
 ...
 void __drawReaderLine(int reader_position)
{
  ...
  noStroke();
  fill(255);
  ellipse(reader_position,250,7,7);
}
...
void __drawLines()
{
  ...
  fill(random(0,30));
  noStroke();
    ellipse(dotsX[ii], dotsY[ii],30,30);  
     fill(255);
  noStroke();
  ellipse(dotsX[ii], dotsY[ii],7,7);  
}
