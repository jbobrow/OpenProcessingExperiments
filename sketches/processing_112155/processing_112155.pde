    ...
    background(36, 83, 197);
     ...
     if (circles.size() > 1) {
        for (int i = 1; i < circles.size(); i++) {
            ...
            // Draw a line from the current circle to every other circle
            // by getting every other circle's x and y position and drawing
            // a line to it.
            for (int j = 0; j < int(random(0, circles.size()-1)); j++) {
                Circle cwaffle = circles.get(int(random(0,circles.size()-1)));
                stroke(255, 50);
                strokeWeight(2);
