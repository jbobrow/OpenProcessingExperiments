 ...
 * frame rate. 
 * My version on a dark background with transparency changing on moving parts
 */
...
void setup() {
  ...
  frame_rate_value = 6;// change from 6
...
void movingCircle(float x, float y, float size, int circleNum) {
  ...
  noStroke();
  fill(120,200,120, random( 0,100)); //change colour here
 ...
 // rect(tempX, tempY, 1, size*10);//  this line gives vertical strings
  ...
  noFill();
  stroke(random(120),200,random(200));// change colour here and opacity too
