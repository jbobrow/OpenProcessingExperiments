 ...
 * as arguments.
 * 
 * Modified by Stephanie Holcomb to draw square and circular clocks,dependent on odd or even status of circleNumber variable 
 * and reverse direction of hand movement.
 *
 */
...
void draw() {
  ...
  float gutter = 4; //distance between each cell
...
void movingCircle2(float x, float y, float size, float angle) {
  ...
  // calculate endpoint of the line
  float tempX = x + (size / 2) * cos(angle); //reversing sin and cos moves hand in opposite direction
