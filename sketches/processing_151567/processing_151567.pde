...
void setup() {
  size(600, 600);
  rectMode(CORNER);
  noStroke();  // like the outline 
  frameRate(1);  // set the frame rate to 1 draw() call per second
  randomSeed(hour() + minute() + second() + millis());
  //noLoop(); // need to keep running to save frame
}
...
if (random (100) > brownSquares) { //  about 80% of time
        fill(160, 32, 240, random(180)); }//  rectangles are purple
     else {
       fill( 152,251,152,random(180)); // rectangles are white
}       // draws 25 rectangles 
        rect(gap -1 * (i+1) + cellsize * i + random (25) , gap - 1 * (j+1) + cellsize * j + random (25), cellsize, cellsize);
      }
    }
 // save your drawing when you press keyboard 's'
  if (keyPressed == true && key=='s') {
    saveFrame("my_25_squares.jpg"); 
  }
} //end of draw 
