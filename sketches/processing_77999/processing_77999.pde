...
boolean pScared = true; // boolean variable that lets the white ellipse to remain scared
void setup() {
  size(500, 500); // sets the size of the sketch
  background(178, 10, 200); // draws the background black
...
void draw() {
  ...
  fill(180, 38, 110); //sets the fill to red so the red ellipse is drawn as red
  ...
  if (pScared==false) { // if the white ellipse is "scared" then it remains yellow
    fill(240, 99 ,155 );
  }
