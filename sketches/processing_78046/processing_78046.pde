...
void setup() {
  ...
  pts2.add(new MetaBall(new PVector(200, 300)));
  pts2.add(new MetaBall(new PVector(300, 300)));
  pts2.add(new MetaBall(new PVector(600, 300)));
  pts2.add(new MetaBall(new PVector(700, 300)));
  pts2.add(new MetaBall(new PVector(800, 300)));
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      for (MetaBall b:pts) {
        ...
        intensity+= 500/(b.intensY[y] + b.intensX[x]+.1);
      }  
      float intensity2 = 0;
  for (MetaBall b:pts2) {
        // Increase this number to make your blobs bigger
        intensity2+= 50/(b.intensY[y] + b.intensX[x]+.1);
      }      
