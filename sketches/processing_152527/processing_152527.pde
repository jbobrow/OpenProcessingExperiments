...
//variables for  5 color boxes
int currentBox = 0;
float boxSize;

float[] boxhueValue;
float[] boxsatValue;
float[] boxbrightValue;
float[] boxcompValue;
...
void setup() {
  ...
  rectMode(CORNER);  
  boxhueValue = new float[6];
  boxsatValue = new float[6];
  boxbrightValue = new float[6];
  boxcompValue = new float[6];
  
  //initialize boxcolor arrays
  for (int currentBox = 0; currentBox < 5; currentBox++) 
            {
        boxhueValue[currentBox] = hueValue;
        boxbrightValue[currentBox] = brightValue;
        boxsatValue[currentBox] = satValue;
        boxcompValue[currentBox] = complementryHue;
            }
...
void draw() {
  ...
  stroke(0, 40);
  line(width/2 - innerR, height/2 - 50 , width/2 - outerR2, height/2 - 50);
  ...
  for (int i=0; i<=10; i++) {
    ...
    //outside(top)
    vertex(width/2 + outerR*sin(angle), height/2 - 50 + outerR*cos(angle) );
    //inside(down)
    vertex(width/2 + innerR*sin(angle), height/2 - 50 + innerR*cos(angle) );
  }
  endShape(CLOSE);
  ...
  dotLine(width/2, height/2 - 50, colorHandleX, colorHandleY, 40); 
  ...
  //   H         S    B
  fill(boxhueValue[currentBox], boxsatValue[currentBox], boxbrightValue[currentBox]);
  ...
  float comHandY = height/2 - 50 + radiusComeHand * sin(angleComHand);

  //dotline from center to comHand
  dotLine(width/2, height/2 - 50, comHandX, comHandY, 20); 

  boxcompValue[currentBox]= calculateCompHue(boxhueValue[currentBox]);

  //println("hueValue: "+hueValue + " + "+"comhue: "+complementryHue);

  fill( complementryHue, boxsatValue[currentBox], boxbrightValue[currentBox] );
  ...
  for (int i = 0; i < 5; i++) 
       {
         //fill(boxColor[currentBox]);
         
         
           if (i != currentBox)
             {
                fill(boxhueValue[i], boxsatValue[i], boxbrightValue[i]);//change this to color
                stroke(0);
                rect(i * width/5, 500, boxSize, boxSize);
              }
              else
             {
              fill(boxhueValue[currentBox], boxsatValue[currentBox], boxbrightValue[currentBox]);//change this to color
              stroke(0);
               rect(i * width/5, 500, boxSize, boxSize);
             }
       }//endfor
  //Draw Complimentary Color Boxes inside
  for ( int i = 0; i < 5; i++)
         {
           
           if(i != currentBox)
             {                    
                fill(boxcompValue[i], boxsatValue[i], boxbrightValue[i]);//change this to color
                stroke(0);
                rect(i * width/5 + 0.3 * width/5, 500 + 0.3 * boxSize, 0.35 * boxSize, 0.35 * boxSize);
             }
             else
             {
               fill(boxcompValue[currentBox], boxsatValue[currentBox], boxbrightValue[currentBox]);//change this to color
               stroke(0);
               rect(i * width/5 + 0.3 * width/5, 500 + 0.3 * boxSize, 0.35 * boxSize, 0.35 * boxSize);
             }
...
float calculateCompHue(float hueValue) {
  ...
  if (boxhueValue[currentBox] >= 180 && boxhueValue[currentBox] < 360) {
    return boxhueValue[currentBox]-180;
  }
  else 
    return boxhueValue[currentBox]+180;
}
...
void colorHandleUpdate() {
  ...
  if (isLocked) {
    ...
    float angle = atan2(mouseY-(height/2 - 50), mouseX-width/2  );
    float distance = dist(mouseX, mouseY, width/2, height/2 - 50);
    float radius = constrain(distance, outerR, outerR2); 
    colorHandleX = width/2  + radius * cos(angle);
    colorHandleY = height/2 - 50 + radius * sin(angle);
    // satValue is Global Variable that will be remembered
    if (keyPressed == true && key==CODED) {
                 if (keyCode == SHIFT ){
            
               boxsatValue[currentBox] = map (radius, outerR, outerR2, 0, 100);
                                 }
                                            }
     else {
     // map distance from outer edge of the wheel to brightness
              boxbrightValue[currentBox] = map(radius, outerR, outerR2, 0, 100);
          }
             
    boxhueValue[currentBox] = map (degrees(angle), -180, 180, 360, 0);
