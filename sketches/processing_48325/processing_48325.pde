...
public class Ball {
  ...
  Rectangle rectangle;
  // BALL PROPERTIES --
  int width = 1; //changed ball width and height
  int height = 5;
  ...
  // velocity
  int velX = 1; //changed velocity to create different diagonal pattern
  int velY = 100;
  //
  int x;
  int y;
  ...
  //
   ...
   Ball(int X, int Y, color StrokeColor, color FillColor) { //added color objects to change ball fill and color
    x = X;
    y = Y;
  ...
  int getY(){ //made getY here for
    return y; //gives Y value of ball
  }
