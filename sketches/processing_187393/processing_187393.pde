...
float maxFractalAngle = 90; //angle in degrees (spacing of lines)
float minFractalLength = 50; //minimum length, proceed from, base upon canvas size
float maxFractalLength = 120; //maximunm length, will not exceed, base upon canvas size
...
void draw() {
  ...
  Fractal(width,height,-90,9); //branching attributes
  noLoop(); 
...
void Fractal(float X, float Y, float direction, float depth) {
  ...
  int fractals = int(random(1,maxFractal)); //int=(random(1,5));
