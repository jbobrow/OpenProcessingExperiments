...
{
  size(800, 800);
  BGfloor = requestImage("BGfloor.png");
...
//  myTank.lookAt(mouseX, mouseY);
  ...
  for (int i = 0; i < bulletCount; i += 1)
  {
    ...
    if(dist(100, 100, bullets[i].x, bullets[i].y) < 70)
    {
      enemy = true;
    }
    if (enemy)
...
void keyPressed()
{
    ...
    if (keyCode == UP)
    {
      ...
      myTank.sprite.setAnimation(0, 14, true);
    }
    if (keyCode == DOWN)
    {
...
class Tank 
{
  ...
  Tank()
  {
    ...
    this.sprite = new AnimatedSprite("WalkingMan.png", 88, 98);
  }
  ...
  void draw()
  {
    ...
    rotate(radians(this.rotation + 90));
    scale(this.s);
