...
beginShape();
 ...
 // rect(0,0,width/2,height);
  ...
  beginShape();
    ...
    {
     float theta = i*step;
     float offset = cos((sin(theta)+PI)*phases-phase_offset)*amp;
     float tx = x+ (sin(theta)*(rad+offset));
     float ty = y+(cos(theta)*(rad+offset));
       vertex(tx,ty);
    }
  ...
  beginShape();
    ...
    {
     float theta = i*step;
     float offset = cos((sin(theta)+(PI/2))*phases-phase_offset)*amp;
     float tx = x+ (sin(theta)*(rad+offset));
     float ty = y+(cos(theta)*(rad+offset));
       vertex(tx,ty);
    }
  ...
  beginShape();
    ...
    {
     float theta = i*step;
     float offset = cos((sin(theta)+(PI/4))*phases-phase_offset)*amp;
     float tx = x+ (sin(theta)*(rad+offset));
     float ty = y+(cos(theta)*(rad+offset));
       vertex(tx,ty);
    }
