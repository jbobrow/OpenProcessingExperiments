...
class Mover
{
  ...
  {
    color newC = img.pixels [constrain ((int) location.y * img.width + (int) location.x, 0, img.pixels.length-1)];
    c = lerpColor (c, newC, 0.1);
  }
  ...
  void flock (ArrayList <Mover> boids)
  {
    ...
    for (int i = 0; i < boids.size(); i++)
    {
      ...
      if (alignCount > 12 || seperationCount > 12) break;
    }
