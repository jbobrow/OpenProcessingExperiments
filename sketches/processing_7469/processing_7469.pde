	...
	void update() {
		...
		VectorType mouse = new VectorType(
				(float) mouseX, (float) mouseY);

		for (int x = 0; x < gridWidth; x++) {
			for (int y = 0; y < gridHeight; y++) {
				GridNode node = nodes[x][y];
				VectorType vector = scale(node.getPosition(), mouse, -0.01f);
				vector.add(zoom(node.getPosition(), mouse, 0.97f, 1.0f));
				vector.add(rotate(node.getPosition(), mouse, -(mouseX-width/2)*0.0004));
				node.warp = vector;
			}
		}
	}
	...
	void drawPoints() {
		for (int x = 0; x < gridWidth; x++) {
			for (int y = 0; y < gridHeight; y++) {
				int col = lerpColor(lerpColor(color(255), color(255, 0, 0),
						(float) x * gridWithInverse), lerpColor(
						color(0, 255, 0), color(0, 0, 255), (float) x
								* gridHeightInverse), y * gridHeightInverse);
				stroke(col);
				GridNode node = nodes[x][y];
	...
	private VectorType rotate(VectorType position, VectorType center, float angle){
		...
		float d = sqrt(rotationVector.x*rotationVector.x+rotationVector.y*rotationVector.y);
