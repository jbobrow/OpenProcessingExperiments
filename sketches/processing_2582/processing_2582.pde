...
class SquareManager
{
  ...
  public boolean createSquare(int cx, int cy, int s)
  {
    ...
    if( ! findIntersection(cx, cy, s) )
    {
      ...
      //translate(-(s/2),-s/2);
       float squarehue;
      if (s>width/6) {
      squarehue=random(30,60);
      } else {
        squarehue=random(-10,10);;
      }
      
      int orient=int(random(2));

      for (int i=0+s/20; i<s-(s/20); i++) {
        for (int j=0; j<=s/30; j++) {
          stroke((randomhue+squarehue+random(4))%100,random(60,80),random(80,100),8);
          strokeWeight(random(4));
          if (orient==1) {
            line(cx+random(s/10),cy+i+random(-s/20,s/20),cx+s+random(-s/10),cy+i+random(-s/20,s/20));
          } 
          else {
            line(cx+i+random(-s/20,s/20),cy+random(s/10),cx+i+random(-s/20,s/20),cy+s+random(-s/10));            
          }
        }
      }
  ...
  private boolean findIntersection( int cx, int cy, int s) 
  {
    ...
    for ( int i = 0; i < m_count; i++ )
    {
      ...
      // Rect One
      int left1 = tx;// - (ts/2);
      int right1 = tx+ts;// + (ts/2);
      int top1 = ty;// - (ts/2);
      int bottom1 = ty+ts;// + (ts/2);

      // Rect Two
      int left2 = cx;// - (s/2);
      int right2 = cx+s;// + (s/2);
      int top2 = cy;// - (s/2);
      int bottom2 = cy+s;// + (s/2);
