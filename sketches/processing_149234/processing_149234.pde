  ...
  if ( roundOver ) {
    if ( numRound != 0 ) { //  ( Round 2 )
      if ( URF ) {
        oneRideRobot = false;
        twoRideRobot = false;
        robotWally = new dodgePlayer(width/2, 130, 1, loadImage(robotUrl), 0);
      } else { // one versus one
...
void keyPressed() {
  if ( (key == 'j' || key == 'J') && ( (playerOneHasBall && ONEVSONE) || (!playerOneHasBall && URF) ) ) {
    if ( robotWally.collisionRobot(one.x, one.y) ) {
      ball.speed = 20;
      rapidFire = true;  
      one = robotWally;
      oneRideRobot = true;
...
if ( key == '1' && ( ( playerTwoHasBall && ONEVSONE) || (!playerTwoHasBall && URF) )) {
  if ( robotWally.collisionRobot(two.x, two.y) ) {
    ball.speed = -20;
    rapidFire = true;
    two = robotWally;
    twoRideRobot = true;
