...
void draw() {
  ...
  float cellsizeB = (height - (num + 1) * gap * 0.75) / (float)num; //height calculation
  ...
  float offsetY = cellsizeB/16.0;
    ...
    for (int i=0; i<num; i++) {
      for (int j=0; j<num; j++) {
        ...
        fill(140, 180); // shadow
        rect(gap * (i+1) + cellsizeA * i + offsetX, 0.75 * gap * (j+1) + cellsizeB * j + offsetY, cellsizeA, cellsizeB);

        fill(247, 57, 57, 180); // rectangle
        rect(gap * (i+1) + cellsizeA * i, 0.75 * gap * (j+1) + cellsizeB * j, cellsizeA, cellsizeB);
      }
    }
} //end of draw 
