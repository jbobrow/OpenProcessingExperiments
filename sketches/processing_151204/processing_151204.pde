...
void draw() {
  ...
  for (int i=0; i<num; i++) {
    for (int j=0; j<num; j++) {
      ...
      if (circleNumber%2 > 0){
      movingCircle(tx, ty, cellsize, circleNumber * TWO_PI * millis() / 40000.0);
      }
      else{
       movingCircle2(tx, ty, cellsize, circleNumber * TWO_PI * millis() / 40000.0);
      }
  }
  }
}//end of draw 
...
void movingCircle(float x, float y, float size, float angle) {
  ...
  stroke(0,0,255);
  strokeWeight(1);
  rect(x, y, tempX*.2, tempY*.4); // red line
}
void movingCircle2(float x, float y, float size, float angle) {
  ...
  stroke(0,0,255);
  strokeWeight(1)
  ellipse(x, y, tempX*.2, tempY*.4); // red line
}
