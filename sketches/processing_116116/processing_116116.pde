...
void createMoveNumber(){
 ...
 void collide(int size){
  if (w == size){
    // big if = movement turns around at the edge 
...
http://www.jeffreythompson.org/blog/2011/11/30/collision-detection-functions-for-processing-released/
...
void checkCollide(){
  for (int i = 0; i < rectZ.size(); i++){
    if (i != id){
      if (x + w/2 >= rectZ.get(i).x - w/2    && x - w/2 <= rectZ.get(i).x + w/2    && y + h/2 >= rectZ.get(i).y - w/2    && y - h/2 <= rectZ.get(i).y + w/2) {
        createMoveNumber();
      }
    }
  } 
