...
void render(float a, float b, float c, float d, float xmin, float xmax, float ymin, float ymax) {
  ...
  for (int i=0;i<nbePoints;i++) {
    ...
    if (useColors)stroke(256*noise(x1), 256*noise(y1), 256*noise(x1, y1), 120);     
  ...
  if (Jong) {
    ...
    text("x1= sin("+c+"*x0)-cos("+d+"*y0)", 220, 40);
  }
  else {
    text("Cliff Pickover attractor:", 10, 20);
    text("x1= sin("+a+"*y0)+"+c+"*cos("+a+"*x0)", 220, 20);
    text("x1= sin("+b+"*x0)"+d+"*cos("+b+"*y0)", 220, 40);
  }
}
void draw() {
