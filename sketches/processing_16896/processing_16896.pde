...
void draw() {  //draw function loops
  ...
  //line( 0,210,150,210);
   ...
   /*line( width/2, height/2, width/2 + 20*cos((float)y/10), height/2 + 20*sin((float)y/10));*/
    ...
    stroke(249,20,45);
    noFill();
   funct( width/2, height/2,30, PI/2,height-y/50,5);
   stroke(114,200,200);
   ellipse(width/2,height/2,20,20);
   stroke(238,230,133);
    
   while( i < numRays ){
      line( width/2 + 12*cos(angle + angleAround) , height/2 + 12*sin(angle + angleAround) , width/2 + 23*cos(angle + angleAround), height/2 + 23*sin(angle + angleAround));
       
      angleAround += 2*PI/numRays;
      i++;
   }
