...
float INIT_THICKNESS = 22;
float INIT_DAMP = 10;
int MAX_LENGTH = 600;

class Gesture 
{
  float  damp = INIT_DAMP;
  float  dampInv = 1.0 / damp;
  float  damp1 = damp - 1;
  float  thickness = INIT_THICKNESS;
  ...
  Gesture(int mw, int mh) {
    ...
    h = mh;
    capacity = MAX_LENGTH;
    col = color(255, 100, 245, 100);
    path = new PVector[capacity];
  ...
  {
    if (nPoints == 0)
      col = color(100+x*155/w, 255-y*155/h, 245, 100);
    
    if (nPoints >= capacity-1) 
  ...
  void compile() {
    // compute the polygons from the path of PVector's
    if (exists) {
      clearPolys();
