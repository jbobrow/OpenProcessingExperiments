...
void draw() {
  ...
  textSize(15);
  text("Key: ",10,18);
  text("Click a Planet to view Information", 500,18);
  text("Date: " + day() + ":" + month() + ":" + year(), 1162, 120);
  text("Time: " + hour() + ":" + minute() + ":" + second(), 1162, 145);
...
void setupSphere(float R, int xDetail, int yDetail, int n){
  ...
  for(int i = 0; i <= xDetail; i++){
    for(int j = 0; j <= yDetail; j++){ 
      float phi = xGrid[i][n]*2*PI;
      float theta = PI-yGrid[j][n]*PI;
      allPoints[i][j][0][n]=R*sin(theta)*cos(phi);
      allPoints[i][j][1][n]=R*sin(theta)*sin(phi);
      allPoints[i][j][2][n]=R*cos(theta);
     }
  }
}
...
void drawSphere(PImage Map,int n){
  ...
  for(int j = 0; j < yDetail; j++){
    ...
    texture(Map);
    for(int i = 0; i <= xDetail; i++){
      vertex(allPoints[i][j+1][0][n],   allPoints[i][j+1][1][n],   allPoints[i][j+1][2][n],   xGrid[i][n],   yGrid[j+1][n]);
      vertex(allPoints[i][j][0][n],     allPoints[i][j][1][n],     allPoints[i][j][2][n],     xGrid[i][n],   yGrid[j][n]);
    }
    endShape(CLOSE);
  }
}
