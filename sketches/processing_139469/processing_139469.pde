...
X_MIN = -1; X_MAX = 1;
Y_MIN = -1; Y_MAX = 1;
TIME_INCREMENT = PI/50;
OUT_PAUSE = false;
FRAMES = PI/TIME_INCREMENT;
 
float flower(x, y, t, n, k1, k2, k3) {
  float radius = dist(x, y, 0, 0)/k1;   // cartesian to polar
  float angle = atan2(x, y) + t*k2 +radius*k3;     // cartesian to polar; turns with time
  float value = sin(angle*n)-radius + sin(radius)*1.5;

return pow(sin(value), 10)*255;
  return min(64, max(0, (1+value) * 64));
}
 
color hsb(x,y,t) {
  float r = sin(t/2);
  float s = pow(r,2)+ .5;
  float v = flower(x, y, t+s, 9, s/5, -1, r);
 
  return color(s*90, 255, 255-v/2); 
} 
// End of user formula @@@
