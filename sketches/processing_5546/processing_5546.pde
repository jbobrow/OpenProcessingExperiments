...
String[] nouns = { "bracket", "hyphen", "comma", "asterisk", "slash", "dash",  "paranthesis", "colon", "exclamation", "period" };
...
void draw() {
  ...
  */
  ...
  textFont(f);
  fill(136,174,175);
  text(sentence,75,height/2);
}

void mousePressed() {  sentence =  getNounPhrase() + " " + getVerbPhrase(); }
/*
void keyPressed() {
  if( key == ' '  ) {  
    sentence =  getNounPhrase() + " " + getVerbPhrase();
