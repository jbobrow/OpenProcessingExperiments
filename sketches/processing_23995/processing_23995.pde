...
void setup() {
  ...
  File dir = new File(sketchPath,"../crops");
...
float theScaleStart, float theScaleEnd, float theRotationStart, float theRotationEnd) {
  ...
  for (int i = 0 ; i < items.length; i++) {
    ...
    items[i].a = theAngle + random(-theRangeA/2,theRangeA/5);
...
void drawCollageItems(CollageItem[] theItems) {
  for (int i = 0 ; i < theItems.length; i++) {
    ...
    float newY  = height/5 + sin(theItems[i].a) * theItems[i].l; 
    translate(newX, newY);
    rotate(theItems[i].rotation);
    scale(theItems[i].scaling);
    image(images[theItems[i].indexToImage], 10,0);
    popMatrix();
  }
}
