...
int num = 5; 
int currentSwatch; // store number of current square
color[] colSwatch; // main colour for outer square
color[] compSwatch; // complimentary colour for inner square
boolean[] defaultSwatch; 
...
void setup() {
  ...
  colSwatch = new color[num];
  compSwatch = new color[num];
  defaultSwatch = new boolean[num];
  
  for (int i=0; i<num; i++) {
    defaultSwatch[i] = true;
  }
  currentSwatch = 0;
}
...
void draw() {
  ...
  endShape(CLOSE); // END of itten's color wheel
...
//  println("hueValue: "+hueValue + " + "+"comhue: "+complementryHue);
  ...
  // draw rects to store chosen colours  
  for (int i=0; i<num; i++) {
    rectMode(RADIUS);
    noStroke();
    if(defaultSwatch[i]) {
    ...
    if(currentSwatch == i && isLocked) {
      colSwatch[i] = color(hueValue, 100, brightValue);
      fill(colSwatch[i]);
      defaultSwatch[i] = false;
    }
    rect(120+140*i, height-90, 70, 70);
    ...
    if(currentSwatch == i && isLocked) {
      compSwatch[i] = color(complementryHue, 100, brightValue);
      fill(compSwatch[i]);
      defaultSwatch[i] = false;
    }
    rect(120+140*i, height-90, 70, 70);
