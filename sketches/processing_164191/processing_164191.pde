...
void setup() {
  ...
  for (int i = 0; i < numBalls; i++) {
    balls[i] = new Ball(random(width), random(height), random(3,6), i, balls);
  ...
  void collide() {
    for (int i = id + 1; i < numBalls; i++) {
      ...
      if (distance < minDist * 1.5) {
        ...
        float U = (cos(3*a_1*PI/180) + 1) * (cos(3*a_2*PI/180) + 1);
        float gradU1 = -3 * sin(3*a_1*PI/180) * (cos(3*a_2*PI/180)+1);
        float gradU2 = -3 * sin(3*a_2*PI/180) * (cos(3*a_1*PI/180)+1);
    ...
    impulse( 0.1 * (1-fr2) * rndQ(), 0.1 * (1-fr2) * rndQ() );
    float dx = mouseX - x;
    float dy = mouseY - y;
    float distance = sqrt(dx*dx + dy*dy);
    if ( mousePressed && (distance < 50) ) {
      //impulse( 1.5 * rndQ(), 1.5 * rndQ() ); 
