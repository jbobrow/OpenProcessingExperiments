...
// Number of swatches and arrays to store the colours for each swatch
int swatches = 6;
float [][] swatchColour;
...
void setup() {
  ...
  // set up the initial swatches
  swatchColour = new float [swatches][3];
  swatchCompColour = new float [swatches][3] ;
 
 // subtle modifcation to a grey neutral swatch
...
void draw() {
  ...
  for (int i=0; i<=20; i++) {
    float angle = radians(18*i-90); // 20 x 18 degree steps
  ...
  for (int i=0; i<swatches; i++){
     //squareSwatch(100+(i*100),650,100,100,i);
     squareSwatch(100+(i*100),650,100,100,i);
  }
}
 ...
 * squareSwatch
 *
 * Draw two squares and fill colour and its compliment
 *
 */
