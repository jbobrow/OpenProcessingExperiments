...
class Frog {
  ...
  float bound_x1, bound_x2, bound_y1, bound_y2;
  ...
  Frog(int x, int y, int frogNumber, float frogSize) {
  ...
  void body() {
    s_frog = map(scale_frog, 1, 10, 0.08, 0.3);
    pushMatrix();
  ...
  void right_leg() {
    if(!jump){ 
      shape(right_leg, c_xpos + 98, c_ypos + 45);
    } else {
  ...
  void left_leg() {
    if(!jump){
      shape(left_leg, c_xpos - 45, c_ypos + 42);
    } else {
  ...
  void right_hand() {
    s = map(mouseY, 0, 320, 1, 1.2);
    pushMatrix();
    translate(c_xpos + 100, c_ypos - 10);
    scale(s);
    shape(right_hand,0, -6);
    popMatrix();
  }
  
  void left_hand() {
    s = map(mouseY, 0, 320, 1.2, 1);
    pushMatrix();
    translate(c_xpos - 50, c_ypos - 5);
    scale(s);
    shape(left_hand, -55, -18);
    popMatrix();
  }
...
//    }
  ...
  void determineWhoJumps() {
    float diff = map(s_frog, 0.08, 0.3, 10, 80);
    if(mouseX < width/2) {
      bound_x1 = c_xpos;
      bound_x2 = c_xpos + diff;
    } else if(mouseX > width/2) {
      bound_x1 = c_xpos + diff/2.7;
      bound_x2 = c_xpos + diff + diff/2.7;
    }      
      bound_y1 = c_ypos - (diff + diff/3);
      bound_y2 = c_ypos + diff;
    
    if(mouseX > bound_x1 && mouseX < bound_x2 && mouseY > bound_y1 && mouseY < bound_y2) {
      jump = true;
    }
