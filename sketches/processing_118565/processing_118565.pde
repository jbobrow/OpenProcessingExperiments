...
void setup()
{
  ...
  bg = requestImage("BG.jpg");

  
  
  
  mouse = new Mouse("THawk.jpg");
  mouse.y = 200;
  mouse.directionX = 10;
  mouse.directionY = 10;
  mouse.speed = .5;
    
  troll = new SpriteObject("mouse.jpg");
  troll.y = 200;
  troll.directionX = 10;
  troll.directionY = 10;
  troll.speed = .5;
    
  predator = new Predator("Snake.jpg");
  predator.y = 200;
...
void draw()
{
  ...
  if(!predator.isOverlapping(troll))
  {
    troll.update();
    troll.render();
  }
  ...
  if(timer < 10)
  {
  mouse.targetX = predator.x;
  mouse.targetY = predator.y;
  }
}
