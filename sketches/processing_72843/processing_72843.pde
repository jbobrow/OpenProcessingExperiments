...
int walktime = 10; //Frames per walk
...
Creature[] stamps = new Creature[nmax]; //stamp array
...
void makefeet(int localX, int localY, int localalp, int walking) {
  ...
  if (walking < walktime) {
    ellipse(localX-64,localY+128-64*walking/walktime,64,170);
    ellipse(localX+64,localY+128,64,170);
  }
  else {
    ellipse(localX-64,localY+128,64,170);
    ellipse(localX+64,localY+128-64*(walking-walktime)/walktime,64,170);
  }
  //Choose feet
  fill(midgreenchoose(),localalp);
  if (walking < walktime) {
    ellipse(localX-64,localY+192-64*walking/walktime,85,51);
    ellipse(localX+64,localY+192,85,51);
  }
  else {
    ellipse(localX-64,localY+192,85,51);
    ellipse(localX+64,localY+192-64*(walking-walktime)/walktime,85,51);
  }
  //Make toes
  fill(midgreenchoose(),localalp);
  for (int i = 0; i < 3; i++) {
    if (walking < walktime) {
    ellipse(localX-96+i*32,localY+224-64*walking/walktime,51,42);
    ellipse(localX+32+i*32,localY+224,51,42);
    }
    else {
      ellipse(localX-96+i*32,localY+224,51,42);
    ellipse(localX+32+i*32,localY+224-64*(walking-walktime)/walktime,51,42);
    }
  }
} 
