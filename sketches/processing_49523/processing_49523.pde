...
void draw(){
  ...
  pushMatrix();
    ...
    for(int i = 0; i < gridSize; i++){
      stroke(0,0,0,0);
      fill(grid[0][i][currentRow]*255,grid[1][i][currentRow]*255, grid[2][i][currentRow]*255, 32);
      ellipse(i, currentRow, 4*grid[0][i][currentRow]+grid[1][i][currentRow], 4*grid[1][i][currentRow]+grid[2][i][currentRow]);   
    }
  popMatrix();
