...
int ballSpeedX = 12;     // speed of the ball horizontally
int ballSpeedY = 12;     // speed of the ball vertically
...
void checkCollide() {
  ...
  if (ballY > height-paddleHeight) { // past 'goal line'
    if (ballX > mouseX && ballX < mouseX + 300) { // within paddle
    ballSpeedY = ballSpeedY * -1;
      ballSpeedY = ballSpeedY + -1.5; // switch Y direction
...
void mousePressed() {
  ...
  paddleColor = int((map(mouseX, 0, width, 0, 300)));
}
