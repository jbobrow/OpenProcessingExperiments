...
void setup()
{
  ...
  ground = requestImage("ground2.png");
...
void draw()
{
  ...
  {
    bullets[i].draw();
    if (dist(100, 100, bullets[i].x, bullets[i].y) < 50)
    {
      hitMonster = true;
    }
  }
...
class Tank 
{
  ...
  Tank()
  {
    ...
    this.sprite = new AnimatedSprite("yellowcarnicecl2.png", 66, 45);
  }
...
class Bullet
{
  ...
  Bullet()
  {
    this.img = requestImage("ProjectileSpriteImages-270x270.png");
  }
