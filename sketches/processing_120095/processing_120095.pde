...
class Asteroid extends Object {
  ...
  void collision(Object thing, int i, int j) {
    if (dist(this.x, this.y, thing.x, thing.y) < this.s+thing.s) {
      ...
      pewpew.remove(j);
      if (random(0, 64) < 2){
        helpme.add(new JesusChrist(x, y, 0));
      }
      if (s>4) {
        switch(c) {
        case 0: 
          rocks.add(new Asteroid(x, y, a-PI/8, v*2, w*2, s/2, c));
          rocks.add(new Asteroid(x, y, a+PI/8, v*2, w*2, s/2, c));
          break;
        case 1: 
          rocks.add(new Asteroid(x, y, a-PI/8, v*1.4, w*2, s/2, c));
          rocks.add(new Asteroid(x, y, a+PI/8, v*1.4, w*2, s/2, c));
          break;
        case 2: 
      ...
      if (score%20 == 0) {
        rocks.add(new Asteroid(mercury.x+random(128, width-128), mercury.y+random(128, height-128), random(-PI, PI), random(.8, 1.2), random(-PI/100, PI/100), 24, 1));
      }
      if (score%30 == 0) {
