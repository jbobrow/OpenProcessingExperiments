...
public class Scribble extends Object {
  ...
  void drawGrid(float xD_, float yD_) {
    for (int i=0; i<height; i+=height/yD_) {
      for (int j=0; j<width; j+=width/xD_) {
        pushMatrix();
        translate(i, j);
        scale(2.0/xD_,2.0/yD_);
        display();
        popMatrix();
      }
    }
...
public void draw() {
  ...
  if (scribbleCount>0) {
    for (int i=0; i<scribbleCount; i++) {
      scribbles[i].update();
      scribbles[i].display();
      scribbles[i].drawGrid(xdiv, ydiv);
    }
  }
}
