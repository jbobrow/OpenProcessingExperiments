...
void setup(){
  ...
  slideTypes = new RadioButtons(widt/2+15,widt+10,100,20,options,buttonsColor);
  bgSlider = new GSlider("Grout Value",20,widt+30,150,10,.5,slidersColor);
  checkSlider1 = new GSlider("Tile Value A",20,widt+60,150,10,0,slidersColor);
  checkSlider2 = new GSlider("Tile Value B",20,widt+90,150,10,1,slidersColor);
  tileWidthSlider = new GSlider("Tile Width",210,widt+35,150,10,.25,slidersColor);
  tileHeightSlider = new GSlider("Tile Height",210,widt+70,150,10,.25,slidersColor);
  groutSlider = new GSlider("Grout Width",widt/2+150,widt+35,200,10,1,slidersColor);
  tileSlider = new GSlider("Tile Displacement",widt/2+150,widt+70,200,10,1,slidersColor);
}
...
void draw(){
  if(mouseY<widt){
    MouseX = mouseX;
    MouseY = mouseY;
    groutSlider.setVal(mouseY/widt);
    tileSlider.setVal(1-mouseX/widt);
  }
  sqw = tileWidthSlider.getPos()*200;
  sqh = tileHeightSlider.getPos()*200;
  groutWidth = groutSlider.getPos()*40;
  background(color(bgSlider.getPos()*255));
  int numRows = ceil(widt/(sqh+groutWidth))+1;
