...
class Bullet
{
  PVector position;
  float w, h;
  float velocity;  // Speed also determines whether the bullet belongs to the player or to an enemy (Negative = player, Positive = enemy)
  
  Bullet(float x_, float y_, float velocity_)
  {
  ...
  void display()
  {
    // Bullet is just a vertically long ellipse for now
    noStroke();
    if (velocity < 0) fill(255, 255, 0);  // If the bullet is the player's, color it yellow
    if (velocity > 0) fill(255, 0, 0);    // If the bullet is the enemy's, color it red
...
class Player
{
  ...
  void move()
  {
    ...
    if (keyRight) player.velocity.x = (speed/20)*-1;
    else if (keyLeft) player.velocity.x = (speed/20);
    else player.velocity.x = 0;
    if (keyUp) player.velocity.y = -2;  // This and the next value need to be changed so that they are affected by the speed stat
    else if (keyDown) player.velocity.y =  4;
  ...
  void display()
  {
    ...
    vertex(position.x+30, position.y+15);
    vertex(position.x-30, position.y+15);
    endShape();
  }
  ...
  void shoot()
  {
    if (keySpace && shotTimer.finished)
    {
      bullets.add(new Bullet(player.position.x, player.position.y, -8));
      shotTimer = new Alarm(500 - attack*5);
    }
  }
...
class statGraph
{
  ...
  void run()
  {
    ...
    defense = map(attack, diameter, 0, 0, 100);
...
void keyPressed()
{
//  if (key == 'a') player.velocity.x = (speed/20)*-1;
//  if (key == 'd') player.velocity.x = (speed/20);
//  if (key == 'w') player.velocity.y = -2;  // This and the next value need to be changed so that they are affected by the speed stat
//  if (key == 's') player.velocity.y =  4;
