...
void draw() {
  ...
  float mapAngle = map(xpos,0.,800, 0., PI*2);
  rotate( mapAngle);
  if (ypos >= ypos2) {
    fill(#242B74);
    noStroke();
...
line(0,-20,0,-140);
  fill(255);noStroke();
  text("days left: "+(int)daysFL[index1], 0, -150);popMatrix();
  popMatrix();
  pushMatrix();
...
line(0,-20,0,-140);
  ...
  text("days left: "+(int)daysOH[index1], 0, -150);popMatrix();
  popMatrix();
  ...
  text("mouse press : pause", 2*width/4, height-20);
}
