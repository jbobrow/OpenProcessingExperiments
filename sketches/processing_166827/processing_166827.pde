...
void draw() {
  ...
  float s = map(mouseY, 0, height, 1, 2);
  Scale( tMatrix, s, s);
...
float[][] scaleMatrix = {{x,0,0},{0,y,0},{0,0,1}}; float[][] matIn = new float[3][3]; MatCopy(tMatrix, matIn);
...
{ float[][] rotMatrix = {{cos(r),-sin(r),0},
{sin(r),cos(r),0},
{0,0,1}}; float[][] matIn = new float[3][3];
MatCopy(tMatrix, matIn);
...
void MatMult(float[][] a, float[][] b, float[][] 
out)
