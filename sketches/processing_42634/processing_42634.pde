...
void setup() {
  ...
  data = new FloatTable("deathcauses.tsv");
...
void drawTitleTabs() {
  ...
  for (int col = 0; col < columnCount; col++) {
    ...
    // If the current tab, set its background dark purple, otherwise use light purple
    
    fill(col == currentColumn ? #e4d6ea : #b099bb);
    rect(tabLeft[col], tabTop, tabRight[col], tabBottom);
    
    // If the current tab, use black for the text, otherwise use dark gray
    fill(col == currentColumn ? 0 : #ffffff);
