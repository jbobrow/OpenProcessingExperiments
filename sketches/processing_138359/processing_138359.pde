...
void draw()
{
  ...
  for (int i = 0; i < bulletCount; i += 1)
  {
    if(dist(myMonster.x, myMonster.y, bullets[i].x, bullets[i].y)<50)
    {
      monsterHit = true;
...
class Monster{
  ...
  Monster(){
    this.img = requestImage("monster.gif"); 
  }
...
class Tank 
{
  ...
  Tank()
  {
    ...
    this.sprite = new AnimatedSprite("guy.gif", 64, 64);
  }
...
class Bullet
{
  ...
  Bullet()
  {
    this.img = requestImage("ball.gif");
  }
