...
//a class, basically a boid, but with a cool display method.
...
class worm{
  ...
  public void draw(){
    //Draws each worm segment. I got this idea from frank te riets vin van veel sketch.
  ...
  //movement algorithms
  public void wander(){
    //provides pseudo-random, realistic movement for bored worms.
  ...
  public void squiggle(int SW, PVector dir){
    //makes a worm squiggle around, using sine waves.
    counter1 = counter1 + 1;
    rot = sin(counter1) * SW;
  ...
  public PVector r(float range){
    //returns a random PVector from within a range.
  ...
  public void flee(PVector target){
    //runs away from a location.
    acc.set(loc.x - target.x, loc.x - target.x, 0);
    acc.normalize();
    acc.mult(speed);
    checkDist(target); //makes chases boring
    vel.add(acc);
    vel.limit(MAXspeed);
    loc.add(vel);
    check();
  ...
  public void checkDist(PVector target){
    //checks if it would be faster for a worm to go through a wall to get to its quarry, rather than going straight.
  ...
  public void frict(float frict){
    //in case you wanted to include friction.
    vel.div(frict);
  }
  
  public PVector FlockTarg(worm[] flock){
    //given an array of worms, returns the average location.
    PVector Avgloc;
    float xa = 0, ya  = 0;
