...
int star(x, y, t, x0, y0, kt, ks, kr, s0) {
  float radius = dist(x-x0, y-y0, 0, 0);
  float angle = -atan2(x-x0, y-y0);
  angle = angle + t*kt;
  float angle2 = angle * (int)(s0+sin(t*10*ks)*10);
  float x2 = radius * cos(angle2);
  float y2 = radius * sin(angle2);
  float stripe = cos(x2) * kr/radius/radius;
  return (stripe + 1) * 127/3;
}

color rgb(x, y, t) {
  //                       x0    y0  kt  ks  kr  s0
  float a = star(x, y, t,  1,    1,  1,  1,  1,  13);
  float b = star(x, y, t, -1+cos(t),   -1+sin(t), -2, .1, .1,  13);
  float c = star(x, y, t, .5+cos(t/2.1), -1.5+sin(t/2.1), -1, .0, 1+sin(t*5)/5, 5);

  return color(a+b, a, a+c+84);
}
