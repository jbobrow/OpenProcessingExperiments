...
void draw() {
  ...
  radius =  10; 
  angle = 0; 
  cellSize = map(mouseY,0,height, 5,40); 
  int counter = 0;   
   ...
   while (radius < radiusMax) {
    ...
    movingCircle(x, y, size, counter,radius, cellSize); 
    ...
    angleStep= map(round(mouseX),0,width, PI/40, PI/4)
    movingCircle(x, y, size, counter, radius,cellSize); 
    angle += angleStep; 
    radius += radiusStep;
    cellSize += sizeStep;
...
void movingCircle(float x, float y, float size, int offset, int radius, int cellSize) {
  float circleAge;
...
float h=map(radius*size,0,width*cellSize,0,360)
