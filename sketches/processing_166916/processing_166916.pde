...
void draw()
{
  ...
  if(!lose)
  {
    ...
    checkLoss();
    textFont(font, 20);
    text("Keeds Respawnedde: " + score, 10,15);
    text("Speed: " + spd, 10,31);
    text("Kids Respawning: " + h, 10, 47);
    if(shots != 0)
    {
      text("Hitmarkerez Misseded vs Hitmarkerz earned: " + int(hits) + "/" + int(shots), 10, 63);
    }
    time++;
  }
  else
  {
    textFont(font, 40);
    text("You lost boyy",10,40);
    textFont(font,20);
    text("Hitmarkerz: " + score,10,70);
  }
