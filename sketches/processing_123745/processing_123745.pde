...
/*
1.0: Release. Corregit l'error que feia que la pilota sortís disparada sense motiu
1.1: Afegides les porteries. Corregit un error pel qual la pilota es podia quedar encallada.
Pla 1.2: Afegir Porters. Afegir calcul de pilota-x-futura i pilota-y-futura per fer una especie de previsió d'on estara la pilota.
*/
...
//Tindran una força aleatoria entre 1 i 5, que sera l'impuls al xutar, i una velocitat de 0.75 a 3, que indica com de rapid es mouran pel camp. Aixo simbolitza els jugadors bons i els dolents
void setup(){
  size (1000, 500);
...
void draw(){
  ...
  noStroke();
  //Procediment: Es dibuixen els dos equips, la pilota, i es calcula si hi ha algú disponible per al xut. De ser aixi, xuta!
...
class Pilota {
  ...
  //xvel i yvel és la velocitat de la pilota. El modul de ambdues determina la seva velocitat final
  float xpil;
  float ypil;
  float xvel;
  float yvel;
  ...
  /*RODA
  Post: La pilota ha "rodat" en la direcció cap on es movia, i la seva velocitat es decrementa degut al fregament
  */
  void roda(){
    xpil+=xvel;
    ypil+=yvel;
  ...
  /*Xut
  Post: La pilota ha agafat un impuls igual al doble de la força del jugador 
  */
  void xut(float yjug, float forsa, boolean casa){
    ...
    else if (yjug > height / 2) yvel=-(forsa*2);
  }
...
//Clase Futbolista: Els seus parametres són, en aquest ordre:
//Coordenades, Velocitat del jugador, Força al xutar, i el seu equip
class Futbolista {
  float xPos;
  float yPos;
  float velocitat;
  ...
  /*Mou
  Post: El jugador persegueix la pilota a raó de x unitats, on x es la seva velocitat
  */
