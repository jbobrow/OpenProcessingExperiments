...
public class Ball 
{
  Rectangle rectangle;
  // BALL PROPERTIES --
  int width = 5 + (int)random(0, 3);
  int height = 5 + (int)random(0, 3);
  boolean hasStroke = false;
  color strokeColor = #9bdab7;
  boolean hasFill = true;
  color fillColor = #9bdab7;
  // velocity
  int velX = 4; //3;
  int velY = 4; //3;
  //
  ...
  {
    updatePosition();
    rectangle.setPosition(x + (int)random(0, 3), y + (int)random(0, 3));
    if (fireMode)
    {
      rectangle.flameYourself();
    }
    else
    {
      rectangle.drawYourselfBall();
    }
  }
  ...
  void updatePosition() 
  {
    ...
    if (result == 1)
    {
      ...
      if (result == 0) 
      {
        ...
        {
          if (bricks[i].imAlive && bricks[i].blockCol == ballMode && !fireMode) 
          {
          ...
          else if (bricks[i].imAlive && fireMode) 
          {
            int res = checkCollisionWithRectangle(bricks[i].rectangle);
            if (res != 0) 
            {
       ...
       for (int i=0; i<playerBricks.length; i++) 
        {
          if (playerBricks[i].imAlive) 
          {
            int res = checkCollisionWithRectangle(playerBricks[i].rectangle);
            if (res != 0) 
            {
              playerBricks[i].die();
              break;
