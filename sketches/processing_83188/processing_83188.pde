...
void draw() { for (int i=0; i<numBlobs-1; ++i) { b[i].update(i); }
 ...
 m += b[i].getbs()*2000/(b[i].bx[x] + b[i].by[y] + 1); }
 pg.pixels[x+y*w] = color(255-dist(x,y,width*.8,height*.8), m+dist(x,y,width*.8,height*.8)*.3, (x+m+y)*scale); //in HSB mode: color((m+x+y),255,255);
 } } pg.updatePixels();
