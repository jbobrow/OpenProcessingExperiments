...
//Press spacebar to save an .mov file of your interaction with the sketch
//Imported Video Library
//Removed some original lines of code that seemed to have issues with Minim/Video
...
//Declare new objects Minim and MovieMaker
MovieMaker mm;
...
//Increased canvas size. Added P3D to the canvas parameters, this mode is helpful when using a large number of images in the processing sketch, or video or audio playback. 
...
void setup() {
    size(1280, 720, P3D);//adjust size of canvas; P3D is different renderer then the default, good for images, video, 3d, etc.
      smooth();
//minim code begins here
minim = new Minim(this);
  out = minim.getLineOut(Minim.STEREO);
  // creating a sine wave Oscillator: 440 Hz, 0.5 defines amplitude
...
public int getColor(VectorType subPixelPosition, int[] sourcePixels) {
    ...
    try {
        ...
        r = constrain(r - 7, 0, 335); // adjusted settings here to change color fade
