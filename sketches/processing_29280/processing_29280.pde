  ...
  if (pv.size() > 0) {
    while (Stack) {
      ...
      for (PVector a: pv) {
        if (a.dist(item) < 700) 
          Stack = false;
      // if item is made within the range of the snake, find a different coordination
        if (mySnake.getVect().dist(a) < 500)
          Stack = false;
      }
    }
  }
...
void draw() {
  ...
  if (gameOver == true) {
    ...
    text("Score: " + howMany , width/2-100, height/2);
    text("Rebecca is merciless.", width/2-200, height/2 +30);
    text("Press 'r' to revive  ", width/2-130, height/2 +60);
