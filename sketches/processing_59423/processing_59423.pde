...
class Allies
{
  ...
  void mousePressed()
  {
    if (mouseX<45 && mouseX>=0 && mouseY>=0 && mouseY<40) {
      ...
      {
        if (touch==assignNum)PlayerFighter.add(new Allies(mouseX, mouseY, touch));
      }
      for (int a=0; a<startupAmount; a++) {
        if (PlayerFighter.size()>startupAmount) {
          Allies allies = (Allies) PlayerFighter.get(a);
          Allies allies2 = (Allies) PlayerFighter.get(PlayerFighter.size()-1);
          if (allies.coordinateX==allies2.coordinateX && allies.coordinateY==allies2.coordinateY) {
  ...
  void CleanUp()
  {
    ...
    for (int p=0; p<OpposingFighter.size();p++) {
      if (startupAmount < PlayerFighter.size()) {
        Enemies enemy= (Enemies) OpposingFighter.get(p);
        Allies ClearAgain = (Allies) PlayerFighter.get(startupAmount);
        if (ClearAgain.mouseXpoint==enemy.coordinateX && ClearAgain.mouseYpoint==enemy.coordinateY) {
          PlayerFighter.remove(startupAmount);
        }
      }
    }
