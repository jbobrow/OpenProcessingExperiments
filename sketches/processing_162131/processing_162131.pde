...
float temperature = 25; //outside temperature [Ã�Â°C]
float superheat = 0; //difference of lifting gas to outside temperatuer [Ã�Â°C]
...
void setup () {
  ...
  B1 = new HSButton(20, 560, 160, 40, "He", true);
  B2 = new HSButton(220, 560, 160, 40, "H2", false);
  B3 = new HSButton(0, 100, width, 40, "fix vs. volume", false);
...
void draw () {
  ...
  //Texts
  textAlign(CENTER);
  text("Windreiter Lift Calc 0.1",200,32);
  textAlign(LEFT);
  text("LIFT [g]", 4, 70);
  text("VOLUME [l]", 4, 170);
  ...
  check_lifting_gas();
    ...
    float lift_gas_molar_mass = 0.0041; //molar mass for helium [kg/mol]
    ...
    case 1:
      lift_gas_molar_mass = 0.0021; //molar mass H2 [kg/mol]
      break;
    }
...
void check_lifting_gas() {
  ...
  if ((B1.Bchecked())&&(lifting_gas==1)) {
    lifting_gas = 0;
    B2.Bcheck(false);
  }
  if ((B2.Bchecked())&&(lifting_gas==0)) {
    lifting_gas = 1;
    B1.Bcheck(false);
  }
  if ((!B1.Bchecked())&&(lifting_gas==0)) {
    lifting_gas = 1;
    B2.Bcheck(true);
  }
  if ((!B2.Bchecked())&&(lifting_gas==1)) {
    lifting_gas = 0;
    B1.Bcheck(true);
  }
}
