...
void movingCircle(float x, float y, float size, int offset) {
  ...
  float circleSize = size * 1.5 * sin(circleAge * HALF_PI);

  strokeWeight(2);
  stroke(255, lerp(255, 0, circleAge));
  
  float colors = (mouseX - mouseY) / 4;
  
  float opacity = random(80, 150);
  fill(colors, lerp(128, 0, circleAge), lerp(120, 0, circleAge), opacity);
  ellipse(x-size/2, y-size/2, circleSize, circleSize);
}


void mousePressed() {
  if(mouseX <= 600 / 2 && frame_rate_value < 39)
  {
    frame_rate_value++;
  }
  
  if(mouseX > 600 / 2 && frame_rate_value > 30)
  {
    frame_rate_value--;
  }
