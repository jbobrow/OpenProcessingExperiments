...
int interactionDistance = 1; //distance in pixels between two object to interact
...
void updateEachObjectPosition() {
  ...
  for (int i = 3; i < numberOfObjects; i++) {
    ...
    if (objectIsOffscreenLeft(i)) {
      ...
      objectXPositions[i] = random(0,100);
    }
    ...
    if (objectIsOffscreenTop(i)) {
      //move it to the right edge
      objectYPositions[i] = random(10,50);
    }
