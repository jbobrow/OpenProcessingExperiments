...
void setup() {
 back=back+100;
  size(300, 300); // sets the size of the sketch
...
void draw() {
  ...
  if (dist(rX, rY, pX, pY)<radius*3 || dist(gX, gY, pX, pY)<radius*3) { // this if statement checks to see if the white ellipse is near either of the predator ellipses
    if (pScared==false) {
  ...
  B=random(0,255);
  fill(R,G,B);
   stroke (random(255, 200, 128));
  strokeWeight (2);
  ellipse (random(10,300), random(10,300), random (1, 50), random (10, 100));
  }
}
