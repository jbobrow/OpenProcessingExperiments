...
//  blendMode(ADD);
  df = new DotFont();
  df.dotFontColor = color(255,255,255,127);
  mc = new MoveChar[512];
  for(i=0;i<512;i++){
    mc[i] = new MoveChar(random(width),random(height));
  }
...
class MoveChar
{
    int charNum,charMode,w;
    color charColor;
    float x,y,v,theta,vx,vy;
    float charW,charH;
    int[] var;
    MoveChar(float a,float b)
    {
      x = a;
      y = b;
      v = random(1)+random(1)+random(1);
      theta = TWO_PI*random(360)/360;
      vx = v*cos(theta);
      vy = v*sin(theta);
      charW = (3-v) * 8;
      w = int (int(0.5+charW/5));
      charH = charW * 7 / 5;
      
      charNum = int(random(33,127));
      charMode = int(random(4));
      colorMode(HSB, 256);
//      charColor = color(random(256),95,255,127);
      charColor = color(random(256),95,255);
      var = new int[8];
      var[0] = int(-charW - 3);
      var[1] = int(width + charW + 3);
      var[2] = int(-charW + 3);
      var[3] = int(width + charW - 3);
      var[4] = int(-charH - 3);
      var[5] = int(height + charH + 3);
      var[6] = int(-charH + 3);
      var[7] = int(height + charH - 3);
    }
    ...
    void drowMoveChar()
    {
      ...
      fill(charColor);
      df.drawDotFont(charNum,4,x,y,w);
    }
}
...
class DotFont
{
  ...
  void drawDotFont(int charNum,int charMode,float x,float y,float charWidth)
  {
    ...
    switch(charMode){
      ...
      case 4:
        c57[charNum].drawCharBoxInt(int(x),int(y),int(charWidth));
        break;
...
class Char57
{
  ...
  void drawCharBoxInt(int x,int y,int w)
  {
    int i,j;
