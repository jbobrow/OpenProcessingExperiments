...
void dessinerTruc(int N, int x,int y,float angle,float radius) {
        ...
        stroke(0,0,0);
          ...
          float thetastep = 2*8.14/N*6;
          
          for(int i=0; i < 3+N; i++) {
            
            float x1 = x+radius*cos(angle+thetastep*i+16)*6.75;
            float y1 = y+cos(angle+thetastep*i+24)*4;
            float x2 = x+radius*sin(angle-thetastep+i+16)*6;
            float y2 = y+radius*sin(angle+thetastep-(i+44))*9;
...
void draw() 
{
  ...
  for(int j = 0 ; j < 8 ; j ++) {
        ...
        dessinerTruc(i+23,20+i*440,20+j*60,angle,9+radius*j/19);
  }    
  }
