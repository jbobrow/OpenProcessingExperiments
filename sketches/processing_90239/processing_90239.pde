...
public void draw() 
{
  ...
  stroke(0,50);
  fill(0,45);
  text("Alga: "+Algae.size()+"   Organs: "+Larven.size()+"   Factor: "+int(Algae.size()/1.8),10,30);
  noFill();
  ellipse(mouseX,mouseY,15,15);
  popStyle();
  for(int i=0;i<Algae.size() && Algae.size()<600;i++)
  {
...
public class ALGA
{
  ...
  ALGA()
  {
    ...
    VARIETY = int(random(0,1.4));
    GET_COLOR();
  }
  
  ALGA(float X,float Y, int V)
  {
    MASS = 1;
    ROOT = new VEC2D(X,Y);
    VARIETY = V;
  ...
  public void GROW(ArrayList tempS)
  {
    ...
    if(random(0,tempS.size()/1.8)<(MASS+2)/3)
    {
      ...
      {
        tempS.add(new ALGA(ROOT.X+modX,ROOT.Y+modY,VARIETY));
      }
    }
  }
  ...
  public void GET_COLOR()
  {
    ...
    {
      case 1:
        SHADE = color(int(random(85,145)),40,0);
        break;
      default:
        SHADE = color(40,int(random(85,145)),0);
        break;
    }
  }
}
public class LARVE 
{
  ...
  int render_count,LARVE_SIZE,FOOD,SIGHT,pupate;
  ...
  public void FIND_FOOD(ArrayList tempL,ArrayList tempS)
  {
    ...
    else if (LARVE_SIZE < 4 ) 
    {
      for (int i = 0; i < tempL.size() && !changed;i++) 
      {
        ...
        if(TL1 != this && TL1.LARVE_SIZE >= 4 && dist(TL1.HEAD.X,TL1.HEAD.Y,HEAD.X,HEAD.Y) < SIGHT ) 
        {
      ...
      if (!changed) 
      {
        int tempDIST = int(SIGHT*1.5);
        int closest = 0;
        for (int i = 0; i < tempS.size() && !changed;i++) 
        {
          TA1 = (ALGA) tempS.get(i);
          if(dist(TA1.ROOT.X,TA1.ROOT.Y,HEAD.X,HEAD.Y)<tempDIST) 
          {
            tempDIST = int(dist(TA1.ROOT.X,TA1.ROOT.Y,HEAD.X,HEAD.Y));
            closest = i;
          }
        }
        if(closest != 0)
        {
          TA1 = (ALGA) tempS.get(closest);
    ...
    else if (LARVE_SIZE == 4 ) 
    {
      ...
      {
        TL1 = (LARVE) tempL.get(i);
        if(TL1 != this && TL1.LARVE_SIZE > 6 && dist(TL1.HEAD.X,TL1.HEAD.Y,HEAD.X,HEAD.Y) < SIGHT ) 
        {
          DIRECTION.FROM_TO(TL1.HEAD,HEAD);
          DIRECTION.ADD_VEC(new VEC2D(random(-1,1),random(-1,1)));
          DIRECTION.NORMALIZE2D();
    ...
    else if (LARVE_SIZE > 4) 
    {
      ...
      {
        TL1 = (LARVE) tempL.get(i);
        if(TL1.LARVE_SIZE > 0 && TL1.LARVE_SIZE < 4 && dist(TL1.HEAD.X,TL1.HEAD.Y,HEAD.X,HEAD.Y) < SIGHT*1.5) 
        {
