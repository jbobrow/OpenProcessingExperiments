...
String male = "Admiral Ackbar";//ccl
String female = "Ada";
...
void setup() {
  ...
  a = new Doll(male, #FF5721, width/2, height/2, 100, 100);//ccl: First doll's attributes
...
class Doll { // this is the class name//ccl
  ...
  Doll(String sex, color tone, float xSpot, float ySpot, float diamX, float diamY) {//ccl
  ...
  void make() {
    ...
    if (gender == female) {
      ...
      //head
      ellipse(brainX, brainY, diameterX, diameterY);//ccl
    }
    else {
...
fill(shade);
      ...
      ellipse(rightFootX, rightFootY, footSize, footSize);
      //head
      ellipse(brainX, brainY, diameterX, diameterY);//ccl head
