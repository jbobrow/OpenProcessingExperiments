...
int gameState=learn2play;

PFont titleFont = createFont("ComicSansMs", 24);
...
void learn_2play() {
  ...
  fill(255);
  textFont(titleFont);
  text("they say taco man died while mixing songs in a tin shack(love shack?) durin a bada$$ twister in newbraski",0,height/2);
  text("there aint nary a night you wont hear his somnambulent corpse a-singing or a-hummin some lame mournful song.",0,height/2+30);
  text("a song about pizza.",0,height/2+60);
  text("gooey peppery delicious pizza and how it hurts his soul that there aint no pizza in heaven-",0,height/2+90);
  text("ya see, he went to terrorist heaven, and instead of pizza, they just have to have sex all the time with virgins",0,height/2+120);
  text("& it sux btw.",0,height/2+150);
  text("well i hear that you got the guts to save taco man from martyrdom, so im gonna tell ya how!!!",0, height/2+180);
  text("just move that there mousey thinga yers around the screen to direct mystically recursive pizza towards his eerie hand",0, height/2+210);
  text("thats it!",0, height/2+240);
  fill(random(225,255));
  text("click anywhere to start",107,height/2+240);
  fill(255);
  text("and git to work city boy/girl!",380,height/2+240);
  text("oh i almost forgot to tell yall 2 turn up da volume",0,height/2+270);
  fill(random(200,255),random(90,100),random(30,40));
  text("Johnny Snatch by Oreaganomics. Video Game design by ComicSansCouture",width/5,height-4);
}
//----------------------------------------------------------------------------- 
void moveEnemy() {
...
//-----------------------------------------------------------------------------
void mousePressed() {
  if (gameState == gameover || gameState == learn2play) {
