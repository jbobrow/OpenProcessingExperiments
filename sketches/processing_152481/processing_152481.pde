  ...
  for (int i=0; i<num; i++) {
    for (int j=0; j<num; j++) {
      ...
      if (circleNumber%2==0){
        ...
        movingCuadrado (tx, ty, cellsize, circleNumber * TWO_PI * millis() / 60000.0);}
      }
...
void movingCuadrado (float x, float y, float size, float angle) {
  ...
  strokeWeight(0);
  fill(140, 180);
  rect(x, y, size, size, 5); // rect

  stroke(255, 0, 0);
  strokeWeight(2);
  line(y, x, tempY, tempX); // red line
}
