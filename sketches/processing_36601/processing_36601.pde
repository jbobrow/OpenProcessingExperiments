...
void draw() {
  ...
  while (i < numRect) {
    rectMode(CENTER);
    if (i<numRect-1) {
      rect((rectXps[i]+(mouseX-width/2)*(rectYps[i]/moved)), rectYps[i]+sin((frameCount+rectYps[i])/50.0)*100, (rectYps[i]/2.0), (rectYps[i]/2.0));
    }
    else {
      rect(rectXps[i], rectYps[i], (rectYps[i]/2.0), (rectYps[i]/2.0));
    }
      //    rectXps[i] = rectXps[i];
      i=i+1;
    }
