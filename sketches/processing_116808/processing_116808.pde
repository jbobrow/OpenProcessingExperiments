...
//Lin,Lien-Ya
//the ball become block when the time is over 12:00.
//the ball grows up according to the seconds.
//the line below shakes according to the minutes.
//the line of * rotate according to the minutes.
//閃爍的尾巴是詩人的憂鬱揮之不去，顫抖的線其實是詩人的腳，越抖越厲害呵呵
//Miro assigntment with movement&flash;.
void setup() {
  size(600, 750);
  background(230);
  smooth();
...
void draw() {
  ...
  bezierVertex(mouseX+31, mouseY-43, mouseX-6, mouseY+9, mouseX, mouseY);
  endShape();
  ...
  float m = minute();
   ...
   bezierVertex(39, 676, 141, 662, 221, 593);//point
   bezierVertex(259, 557, 261, 499, 223, 494);//point
   bezierVertex(mouseX+31+r2, mouseY-43+r2, mouseX-6+r2, mouseY+9+r2, mouseX+r2, mouseY+r2);
   endShape();
   */
  ...
  beginShape();
  vertex(294, 711);
  bezierVertex(327+r2, 734+r2, 421+r2, 744+r2, 453+r2, 721+r2);
  bezierVertex(460+r2, 715+r2, 467+r2, 644+r2, 464, 639);
  endShape();
...
void tail() {
  ...
  noStroke();
  beginShape();
  vertex(mouseX+27, mouseY-18);
  bezierVertex(mouseX+17, mouseY-42, mouseX-54, mouseY-9, mouseX-36, mouseY+21);
  bezierVertex(mouseX-24, mouseY+37, mouseX, mouseY+35, mouseX+15, mouseY+23);
  bezierVertex(mouseX+30, mouseY+13, mouseX+37, mouseY, mouseX+27, mouseY-18);
  endShape();
}
void seconds2() {
  float s =second();
  ...
  if (12<h) {
    ...
    beginShape();
    vertex(0, 0);
    bezierVertex(-46, -8, -59, 32, -24, 38);
    bezierVertex(-8, 38, 13, 38, 15, 23);
    bezierVertex(18, 12, 13, 2, 0, 0);
    scale(1.0+s/38);
    endShape();
    popMatrix();
  }
