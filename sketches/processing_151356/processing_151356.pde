...
void draw() {
  ...
  for (int i=0;i<num;i++) {
    for (int j=0;j<num;j++) {
      ...
      angle=radians(frameCount*(mouseX-width/2)/30);//rotation speed varies with distance from //mouse to center of the sketch
      movingCircle(tx, ty, cellsize, (1-2*int(useACircle))*angle, useACircle);
    }
  }
}
