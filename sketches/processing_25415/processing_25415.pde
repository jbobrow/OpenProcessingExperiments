...
void EvaluateKey() {
  ...
  if (key == CODED) {
    ...
    switch (key) { 
    case 'n':
      // new / in the sense of delete screen 
      background(0);
      break; 
    case ' ': 
      // Space key
      // start new different form / toggles the swarm mode (5 ways of swarming)
      keyPressedFAKE();
      break; 
    case 'i':
      // save frame 
...
void keyPressedFAKE() {
  ...
  for(int i = 0; i < Z.length; i++) {
    ...
    if ( choice > 15 ) {
      ...
      Z[i].reset( map (i,0,Z.length-1,200,width-200), 600, 300, 0, 0, 0, 1 );      
      stroke(222);       
      point     ( Z[i].x, Z[i].y, Z[i].z ) ; 
      // } // for
    } // if
