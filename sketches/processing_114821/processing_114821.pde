...
int switchCounter;
Slider speedSlider;
final static String slideLabel = "Speed Slider"; 
int w = 50;
...
void setup() 
{
  ...
  speedSlider = new Slider(180,450,300,75,slideLabel);
...
void draw() 
{
  ...
  { 
    ...
    if (mousePressed && myTrap.contains(thisAnt.intX, thisAnt.intY) && switchToggle.says("Burn Ants"))
    {         
      thisAnt.x = 9999;
      thisAnt.y = 9999;
    }
...
{    
  if (switchToggle.contains(mouseX, mouseY))
  {
    ...
    {
     switchToggle.setText("Click Slider"); 
    }
    else if (switchCounter == 4)
    {
      switchToggle.setText("Remove Food");
    }
    else if (switchCounter > 4)
    {
...
void mousePressed(){
  if (speedSlider.contains(mouseX,mouseY) && switchToggle.says("Click Slider")){
    speedSlider.setUpdate(true);
  }
...
void mouseDragged() {
  ...
  if (speedSlider.isUpdating()) {
    float prevValue = speedSlider.interp(mouseX, mouseY);
     speedSlider.setValue(prevValue);
     frameRate(60*prevValue + 5);
  }
  
   if (switchToggle.says("Remove Food")){
    food.bite(mouseX,mouseY);
  }
}    
  
void mouseReleased(){
     if (speedSlider.isUpdating()) speedSlider.setUpdate(false);
