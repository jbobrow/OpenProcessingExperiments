

maegru maegru1;
maegru maegru2;
maegru maegru3;

int y;
int spatie;
int stop;
int breedte;
int hoogte;

float i = 50;

void setup() {
  size(500, 500);
  background(100);
  y = 0;
  spatie = 30;
  stop = height;
  breedte = 10;
  hoogte = 10;
  frameRate(50);

  maegru1 = new maegru();
  maegru2 = new maegru();
  maegru3 = new maegru();
}
void draw() {

  background(255);
  partyrain();

  maegru1.monstercolour();
  maegru1.drawmonster();
  maegru1.monstermovement();

  maegru2.monstercolour();
  maegru2.drawmonster();
  maegru2.monstermovement();

  maegru3.monstercolour();
  maegru3.drawmonster();
  maegru3.monstermovement();

  strokeWeight(3);
  stroke(96, 0, 0);
  fill(100, 30, 20);
}



void partyrain() {

  strokeWeight(1);
  for (int i=0; i < 100; i ++) {
    stroke(random(255), random(255), random(255));
    ellipse(random(0, 500), random(0, 500), 1, 75);
  }

  while (i > 500) { 
    i = 0;
  }
}



class maegru {

  float PosX;
  
  float monsterposX;
  float monsterposY;

  int richting;
  int teller;

  boolean test;

  maegru() {
    
    PosX = random(-200,200);
    
    monsterposX = random(100, 400);
    monsterposY = random(100, 400);
    richting = 5;
    teller = 0;
    test = true;
  }

  void monstercolour() {

    if (test == true) {
      fill(0);
    }
    else if (test == false) {
      fill(55, 0, 0);
    }

    strokeWeight(2);
    if (test == true) {
      fill(0);
    }
    else if (test == false) {
      fill(100, 30, 20);
    }

    if (test == true) {
      fill(0);
    }
    else if (test == false) {
      fill(80, 0, 0);
    }


    fill(10);
    stroke(0);


    stroke(96, 0, 0); 
    if (test == true) {
      fill(0);
    }
    else if (test == false) {
      fill(60, 0, 0);
    }

    println(teller);

    if (teller == 10) {
      teller = 0;
      test = !test;
    }
  }

  void monstermovement() {

    monsterposX = mouseX + PosX;
    monsterposY = monsterposY - richting;

    if (monsterposY < 0 + 100) {
      richting = -richting;
      teller = teller +1;
    }
    if (monsterposY > 500 - 100) {
      richting = -richting;
    }
  }
  void drawmonster() {

    strokeWeight(3);

    stroke(96, 0, 0);
    triangle(monsterposX-100, monsterposY+280, monsterposX-50, monsterposY+60, monsterposX-30, monsterposY+60); //linkerbeen
    triangle(monsterposX+100, monsterposY+280, monsterposX+50, monsterposY+60, monsterposX+30, monsterposY+60); //rechterbeen
    ellipse(monsterposX-100, monsterposY+280, 20, 20); //linkervoet
    ellipse(monsterposX+100, monsterposY+280, 20, 20); //rechtervoet

    ellipse(monsterposX, monsterposY, 200, 200); //lichaam

    triangle(monsterposX-15, monsterposY-15, monsterposX-25, monsterposY-30, monsterposX-60, monsterposY-30); //linkerbeen
    triangle(monsterposX+15, monsterposY-15, monsterposX+25, monsterposY-30, monsterposX+60, monsterposY-30); //rechterbeen

    ellipse(monsterposX-30, monsterposY+30, 30, 30); //mond1 (van links)
    ellipse(monsterposX-10, monsterposY+30, 30, 30); //mond2
    ellipse(monsterposX+10, monsterposY+30, 30, 30); //mond3
    ellipse(monsterposX+30, monsterposY+30, 30, 30); //mond4

    triangle(monsterposX-40, monsterposY-12, monsterposX-30, monsterposY-30, monsterposX-25, monsterposY-50); //litteken oog
    triangle(monsterposX-40, monsterposY-85, monsterposX-35, monsterposY-130, monsterposX-25, monsterposY-95);  //linkerhoorn
    triangle(monsterposX+40, monsterposY-85, monsterposX+35, monsterposY-130, monsterposX+25, monsterposY-95); //rechterhoorn

    line(monsterposX-100, monsterposY-5, monsterposX-150, monsterposY-5); //linker'arm'
    line(monsterposX+100, monsterposY-5, monsterposX+150, monsterposY-5); //rechter'arm'
  }
}

