...
PImage p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12,p13,p14,p15,b1,b2,rs;
PFont font1;
/*
...
void draw() {
 ...
 float y9 = random(height);  
  ...
  for(int i=1; i<30;i++){
   ...
   rotate(PI/9.0);
    ...
    //for(int j=1; j<30; j++){
        tint(255,100-i);
        PImage crop= get();
  image(crop, 0, 0, width-10*i, height-10*i);
  if (mousePressed == true) {
   ...
   pushMatrix();
     image(rs, height/2, width/2, height/2, width/2); 
popMatrix();
    }
