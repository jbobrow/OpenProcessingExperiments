...
/*
 * Creative Coding
 * Based on Week 2, 05 - Moving Patterns 1
 *
 * Changed the gutter size and added colour
 * 
 * Todo: Fix colour - different from sketch in normal Processing... 
 * By Matthias Harvey
 * 
 */
...
float t = 0.0;
float t2 = random(2000);
float t3 = random(1234);
...
void draw() {
  ...
  background(200 * noise(t3) , 240 * noise(t3) + 15, 200 * noise(t3));
...
void movingCircle(float x, float y, float size, int circleNum) {
  ...
  noStroke();
  fill(noise(circleNum)*200, noise(t2)*100, noise(t)*255);
