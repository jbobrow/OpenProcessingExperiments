...
AudioPlayer musica; // leitor de musica
 ...
 void setup() {  
  ...
  textFont(texto);
  lab = loadImage("labirinto fundo.png");//load da imagem do labirinto 
...
// Colisao 
 ...
 fundo = loadImage ("sem obstáculo.png");
 rect(260,355,25,25);
 image (fundo,0,0);
 ...
 void venceu() {//menu de vitoria
  ...
  venceu = loadImage ("image 2 venceu.png");
  image (venceu,0,0);
  /*background(0,255,0); 
  //fill(0); 
  ...
  if(mousePressed){
     reset();
     lab = loadImage("labirinto fundo 2.png");//load da imagem do labirinto
     estado = 1;
