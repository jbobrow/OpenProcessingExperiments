...
void setup() {
  size(800, 600);
  background(#FFFFFF);
  vp = createGraphics(400,600-16);
  //vp.colorMode(HSB, 100, 100, 100, 100);
  initDepart(vp.width/2, vp.height/2, min( vp.width/2, vp.height/2));
...
void initDepart(float ix, float iy, float radius) {
  askForErase=true;
  for (int i = 0; i < nAttractionPoints; i++) {
    x0[i] = random(vp.width / 2) + vp.width / 4;
    y0[i] = random(vp.height / 2) + vp.height / 4;
    oldx0[i] = x0[i];
    oldy0[i] = y0[i];
  }
...
void draw() {
  vp.beginDraw();
  if (styleAnimation || askForErase) vp.background(colorFromHSBA(95*3.6,50*2.55,25*2.55,255.0));
  askForErase = false;
  for (int i = 0; i < nIter[nIterMode]; i++) {
    ...
    if (styleExtremite) {
      vp.noStroke();
      vp.fill(colorFromHSBA((float)(globalIteration%3600)/10, 255, 255, 255));
      vp.ellipse(xDisp1, yDisp1, 10, 10);
      vp.fill(colorFromHSBA((float)((globalIteration+100)%3600)/10, 255, 255, 255));
      vp.ellipse(xDisp2, yDisp2, 10, 10);
    }
    if (styleMilieu) {
      vp.noStroke();
      vp.fill(colorFromHSBA((float)((globalIteration+50)%3600)/10, 255, 255, 255));
      vp.ellipse((xDisp1+xDisp2)/2, (yDisp1+yDisp2)/2, 10, 10);
    }
    if (styleTrait) {
      vp.stroke(colorFromHSBA((float)((globalIteration+50)%3600)/10, 255, 255, 255));
      vp.line(xDisp1, yDisp1, xDisp2, yDisp2);
    }
    if (globalIteration >=2 && styleQuad) {
      vp.stroke(colorFromHSBA((float)((globalIteration+50)%3600)/10, 255, 255, 25));
      vp.fill(colorFromHSBA((float)((globalIteration+50)%3600)/10, 255, 255, 25));
      vp.quad(prevx1, prevy1, prevx2, prevy2, xDisp2, yDisp2, xDisp1, yDisp1);
    }
    if (globalIteration >=2 && styleContour) {
      vp.stroke(colorFromHSBA((float)((globalIteration+50)%3600)/10, 255, 255, 255));
      vp.fill(colorFromHSBA((float)((globalIteration+50)%3600)/10, 255, 255, 255));
      vp.line(prevx1, prevy1, xDisp1, yDisp1);
      vp.line(prevx2, prevy2, xDisp2, yDisp2);
    }
    prevx1 = xDisp1; 
    prevy1 = yDisp1;
    prevx2 = xDisp2; 
  ...
  vp.stroke(0);
  vp.fill(#FFFFFF);
  if (styleAttraction) for (int j = 0; j < nAttractionPoints; j++) vp.ellipse(xInViewPort(x0[j]), yInViewPort(y0[j]), 10, 10);
  vp.endDraw();
  image(vp, 0, 0);
  image(vp, 401,0);
