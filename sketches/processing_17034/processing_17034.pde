...
void setup() {  //setup function called initially, only once
 ...
 background(255);  //set background white
  ...
  ls[0] = {width/2,height/2,PI/2,2,0,0,0,0};
  numAnts = 1;
  ...
  //background(255);
    ...
    if(mousePressed && !alreadyClicked && depth < 7){
       while( i < numAnts ){
          ...
          ls[numAnts + i] = {ls[i][0],ls[i][1],ls[i][2] + PI/15,ls[i][3],ls[i][4] + 0.001,ls[i][5], 1,0.1 };
          ls[2*numAnts + i] = {ls[i][0],ls[i][1],ls[i][2] - PI/15,ls[i][3],ls[i][4] - 0.001,ls[i][5], -1,-0.1 };
