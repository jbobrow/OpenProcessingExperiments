...
void setup()
{
  ...
  for (int j = 0; j <= it-1; j++) {
    ...
    yheight[j]=abs(attractory[j]-detractory[j])*height;
  }
}
...

{
  ...
  for (int j = 0; j <= it-1; j++) {
    ...
    if (abs(mouseY-centrey[j])<yheight[j]/2) {
      dist = abs(mouseX-centrex[j]);
    }else{
      dist = 1000;
    }
    stroke(lerpColor(from, to, colorpos*int(10-dist/100)));
    strokeWeight(min(10,max(1.5,100/dist)));
