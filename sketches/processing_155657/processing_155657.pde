...
int giWidth=900;
int giHeight=600;
int giDieRad=min(5*giMaxRad, giHeight);  //600/2.2; //2.5*giMaxRad;
int maxInitialSpeed=100;
float gfLifExp=120000; //600000; //240000;
//float gfStaTme=10000; // starvation time before death
...
float gravity=-0.0;
float electricalStrength=200000; //50; //300000; //500000; //1000000;
...
//color bColor = color(random(50, 80), random(120, 200), random(160, 230));
color bColor = color(40,0,10);
...
void writePostcard() {
  ...
  textSize(30);
  fill(oppositeColor(x, y), 100);
   greetingText="I hope you have enjoyed this course as much as I have.";
  text(greetingText, x, y);

  y+=50;
  textSize(30);
  fill(oppositeColor(x, y), 100);
   greetingText="I leave you with my nanoBubbles which are the creatures";
  text(greetingText, x, y);

  y+=50;
  textSize(30);
  fill(oppositeColor(x, y), 100);
   greetingText="I have learnt to love (ha ha!)";
  text(greetingText, x, y);

  y+=60;
  textSize(40);
  fill(oppositeColor(x, y), 100);
   greetingText="Julio Montabes";
  text(greetingText, x, y);

  y+=70;
  textSize(20);
  fill(oppositeColor(x, y), 100);
   greetingText="From Parc GÃ¼ell in Barcelona for Creative Coding students. July 2014";
  text(greetingText, x, y);
  ...
  textSize(20);
  fill(oppositeColor(x, y), 100);
   greetingText=".. DblClick - Restart";
  text(greetingText, x, y);
  y+=30;
  textSize(20);
  fill(oppositeColor(x, y), 100);
   greetingText=".. Click - To catch and drag a bubble";
  text(greetingText, x, y);
  y+=30;
  textSize(20);
  fill(oppositeColor(x, y), 100);
   greetingText=".. You can also use letters b c/C d e/E f/F g/G l n/N r/R s and t";
  text(greetingText, x, y);
}
...
void reportObject(int i) {
  ...
  println("      G="+bubble[i].pvGraAcc.x+"/"+bubble[i].pvGraAcc.y+" Mc="+bubble[i].pvMecAcc.x+"/"+bubble[i].pvMecAcc.y+" E="+bubble[i].pvEleAcc.x+"/"+bubble[i].pvEleAcc.y+" ChF="+bubble[i].pvCheAcc.x+"/"+bubble[i].pvCheAcc.y+" NF="+bubble[i].pvNucAcc.x+"/"+bubble[i].pvNucAcc.y );
...
class Bubble {
  ...
  float pfLFTime; // Last Feeding Time
  float pfLRTime; // last time of sexual reproduction
  ...
  Bubble() { // DEFAULT CONSTRUCTOR
    ...
    ///pfSexRadius = random(0.95,1.01)*pfRadius;
  ...
  void die() {
    if (pbAlive) {
      ...
      //if (millis()-pfLFTime>gfStaTme) {
      //  println(" piNum="+piNum +" died from STARVATION. millis()-pfLFTime="+(millis()-pfLFTime) +"gfStaTme="+gfStaTme );
      ...
      //println(" piNum="+piNum +" pfRadius="+pfRadius +" r*giDieRad="+r*giDieRad +" age()="+age() +" pfLifExp="+pfLifExp );
    }
  }
  ...
  void eat(Bubble a, Bubble b) {
    ...
    ///int bite = (int) (sqrt(sq(a.pfRadius)+sq(maxBite)) - a.pfRadius);
  ...
  void reproduceBubble(Bubble b) {
      ...
      if (age()>pfMaturityAge && pfRadius>pfMaturityRadius) { 
        ...
        if (rR>pfBirthRadius) {
          ...
          if (iFreNum!=-1) {
            ...
            //bubble[iFreNum].pfLFTime=millis();
            bubble[iFreNum].pfLFTime=frameCount;
            bubble[iFreNum].pfLRTime=frameCount;

            int re=(int) constrain((red(pcBodyColor)+red(b.pcBodyColor))/2*random(0.50, 1.50), 1, 255);
            int gr=(int) constrain((green(pcBodyColor)+green(b.pcBodyColor))/2*random(0.50, 1.50)*random(0.90, 1.10), 1, 255);
            int bl=(int) constrain((blue(pcBodyColor)+blue(b.pcBodyColor))/2*random(0.50, 1.50), 1, 255);
    ...
    // if (pfRadius>random(0.80, 0.90)*giDieRad) { // asexual reproduction
      ...
      if (age()>pfMaturityAge && pfRadius>pfMaturityRadius) { 
        ...
        if (iFreNum!=-1) {
         ...
         // bubble[iFreNum].pfLFTime=millis();
          ...
          int re=(int) constrain(red(pcBodyColor)*random(0.50, 1.50), 1, 255);
          int gr=(int) constrain(green(pcBodyColor)*random(0.50, 1.50), 1, 255);
          int bl=(int) constrain(blue(pcBodyColor)*random(0.50, 1.50), 1, 255);
          bubble[iFreNum].pcBodyColor = color(re, gr, bl);
          ...
          ////bubble[iFreNum].pvPos.x = pvPos.x + cos(alpha) * pfSexRadius;
          ////bubble[iFreNum].pvPos.y = pvPos.y + sin(alpha) * pfSexRadius;
          //bubble[iFreNum].pvPos.x = pvPos.x + cos(alpha) * pfRadius;
          //bubble[iFreNum].pvPos.y = pvPos.y + sin(alpha) * pfRadius;
            bubble[iFreNum].pvPos.x = pvPos.x + cos(alpha) * pfRadius * random(0.95, 1.05);
            bubble[iFreNum].pvPos.y = pvPos.y + sin(alpha) * pfRadius * random(0.95, 1.05);
  ...
  void display() {
    ...
    if (drawObject) {
      ...
      ellipse ( sX(), sY(), sRadius()*2, sRadius()*2);
        ...
        noFill();
        stroke(red(pcBodyColor), 255, 255);
    ...
    if (drawCenter) {
      ...
      //stroke(pcBodyColor);
      stroke(255, green(pcBodyColor), blue(pcBodyColor));
