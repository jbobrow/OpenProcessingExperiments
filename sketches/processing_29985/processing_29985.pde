...
class Brush {
  ...
  void drawTheLine(){
    ...
    for (int j = 0; j < lions.size(); j++) {
      ...
      style = (Brush) styles.get(j);
      style.moveThePts(lion);
    }
  }
...
class Walker extends Brush {
  ...
  void moveThePts(ArrayList arraylistname) {
    ...
    for (i = 0; i < arraylistname.size(); i++) {
      PVector p = (PVector) arraylistname.get(i);
      PVector morph = new PVector(1, 1); //could be new PVector (frameCount, frameCount);
      p.add(morph);
      if (i > 1) {
        PVector q = (PVector) arraylistname.get(i-1);
        line (p.x, p.y, q.x, q.y);
...
class Branch extends Brush {
  ...
  void moveThePts(ArrayList arraylistname) {
    ...
    for (i = 0; i < arraylistname.size(); i++) {
      PVector p = (PVector) arraylistname.get(i);
      PVector morph = new PVector(random(-10, 10), sin(a)*r); //could be new PVector (frameCount, frameCount);
      p.add(morph);
      if (i > 1) {
        PVector q = (PVector) arraylistname.get(i-1);
        line (p.x, p.y, q.x, q.y);
