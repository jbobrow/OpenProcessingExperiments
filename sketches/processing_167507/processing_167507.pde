...
VectorField vField;
 int unitPerSide=128*1;
 int sizeCanvas=512*1;
 float counter=0;
          ...
          for(int i=0; i<gridSize.x; i++){
            for(int j=0; j<gridSize.y; j++){
              ...
              float myy=(j+.5)*height/gridSize.y;
                noStroke(); //(255);
                fill(1*field[i][j].x, 200*field[i][j].y, 50*field[i][j].x, 100);
                strokeWeight(.1);
                ellipse(myx, myy, field[i][j].x, field[i][j].x); //velocityAtLocation(i,j).x , velocityAtLocation(i,j).y
             // popMatrix();
            }
          }
       }
...
public void mouseDragged() {
    ...
    if(keyCode == DOWN) { convolveFactor=convolveFactor/0.8;   }
          float   limitMouse=  sq(sq(height) + sq(width));
              convolveFactor= map( sq(sq(mouseX) + sq(mouseY)) ,1, limitMouse, 0.00000001, 10); // map( log(mouseY),1,log(height), 300, 0.0000001);
}
