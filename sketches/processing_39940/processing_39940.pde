...
void draw() {
  ...
  background(random(mouseX+1), random(mouseY+1), random(mouseX+1));
  ...
  for (int lineY = 0; lineY < 820; lineY = lineY + 80) {
    ...
    for (int lineX = 0; lineX < 820; lineX = lineX + 80) {
      ...
      stroke(255);
      fill(0, 0, 0, 50);
      rect(lineX, lineY, random(mouseX+1), random(mouseY+1));
      //Ellipses with random forms and white lines
      stroke(255);
      fill(0, 0, 0, 50);
      ellipse(lineX-20, lineY-20, random(mouseX+1), random(mouseY+1));
    }
  }
}
