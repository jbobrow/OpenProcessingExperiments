...
/*Verwende Variablen. Zeichne eine einfache Form (z.B. Kreis, 
Rechteck). Die Form soll bei gedrückter Maustaste immer größer 
werden. Bei Tastendruck soll die Grösse auf den initialen Wert 
...
void draw() {
  ...
  fill(0);
  ellipse(238, 256, augenWidth, 74); //(238, 256, 15, 74);
  ellipse(362, 256, augenWidth, 74); //(362, 256, 15, 74);
  fill(106, 114, 58);
  ellipse(238, 220, 80, 22);
  ellipse(362, 220, 80, 22);

  if (mousePressed) {
    if (mouseButton == LEFT & augenWidth < 96) {
      augenWidth = augenWidth + 2;
    }
    if (mouseButton == RIGHT & augenWidth > 22) {
      augenWidth = augenWidth - 2;
    }
  }
  ...
  ellipse(0, 0, 16, 24);
  fill(124, 185, 170, 20);
  ellipse(18, 1, 32, 14); //(358, 380, 32 , 14);
  ellipse(-18, 1, 32, 14); //(394, 380, 32, 14);
