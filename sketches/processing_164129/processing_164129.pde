...
void setup() {
  ...
  frogLocation = new PVector(width/2-67, height-160);

  /* myCar1 = new Car(); //new object
   myCar2 = new Car();
   myCar3 = new Car();
   myCar4 = new Car();*/
}
...
void draw() {
  ...
  case 1:
    ...
    Univers = createFont("UniversLTStd-Ex.otf", 400);
    textFont(Univers, 20);
    ...
    if (score >= 20) {
      ...
      score = 0 ; 
      frogLocation = new PVector(width/2-55, height-120);
    }
...
//end foreground big planet */

PFont Univers; //nickname!!

Univers = createFont("UniversLTStd-Ex.otf", 400);
textFont(Univers, 50);
textAlign(CENTER, CENTER);
fill(#E4F2A8);
text("CLICK MOUSE TO START OVER", width/5, height/3,300,350);
...
void game() {
  ...
  for (Car myCar : myCars) {
    ...
    PVector offset = new PVector (65, 55);

    PVector fixed = PVector.add(frogLocation, offset);
    float d = fixed.dist(myCar.location);
 ...
 //   println(d) ; 
  ...
  for (int i = 0; i < myCars.size (); i++) {
    ...
    popMatrix();
    fill(#E4F2A8);
    text(score, width/2-1, 31);
    stroke(#E4F2A8);
    noFill();
    rect(width/2-25, 9, 47, 47);
...
PFont Univers; //nickname!!
    ...
    text("CLICK TO START", width/5, -7, 300, 50);
