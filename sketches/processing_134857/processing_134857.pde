...
{  
  ...
  if (gamePLAY == gameWIN)
  {
    ...
    text("You did not die!", width/2,100);
    text("Your score: " + (int)gamePoints, width/2,200);
    text("Now try to beat that!", width/2,250);
  }
  if (gamePLAY == gameLOSE)
  {
    ...
    fill(#FFFFFF);
    textAlign(CENTER);
    text("Failure!", width/2,50);
    text("You died!", width/2,100);
    text("You can do better than that!", width/2,150);
    text("...", width/2,250);
    text("Can't you?", width/2,300);
  }
}
