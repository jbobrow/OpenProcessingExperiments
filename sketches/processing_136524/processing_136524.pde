...
int n = 5;
/*Delta tempo (para calcular as mediÃ�Â§Ã�Âµes),obs:quanto menor melhor eh a aproximacao*/
...
void setup() {
  ...
  for (int i = 0;i<n;i++) {
    ...
    massas[i].vx=random(-1, 1);
    massas[i].vy=random(-1, 1);
  }
}
...
void desenhaMassas() {
  ...
  for (int i=0;i<n;i++) {
    fill(massas[i].r, massas[i].g, massas[i].b);
    stroke(200);
