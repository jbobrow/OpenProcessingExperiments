...
void setup()
{
  ...
  rhodes = minim.loadFile("RhodesBySinQ.wav");
  rhodes.play();
...
/*//Create a key trigger for audio
void keyPressed() {
  if (key == 'b') {
    rhodes.trigger();
    //this give me text feedback saying the file was triggered
    //also if you press the key multiple times it audio
    //will start again on top of and just behind the 1st instance
    if (key =='b') {
