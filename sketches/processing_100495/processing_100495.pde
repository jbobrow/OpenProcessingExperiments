...
void setup() {
  ...
  ball1 = new Ball(); 
  paddle1 = new Paddle();
  paddle2 = new Paddle(20.0, 0.0, 100.0, 2.0);
...
class Ball { 
  ...
  float ballX = 200;
  float ballY = 650;
  float ballSpeedX = random(7, 10);
  float ballSpeedY = random(-10, 5);
  ...
  void reset(Counter c) { // Resets things if the ball leaves the screen
    if (ballX > width + ballDiameter) {// ballX < -ballDiameter) {
      ballX = 800;  // edit to specify person
      ballY = 650;
      ballSpeedX = random(7, 10);
      ballSpeedY = random(-10, 5);
...
class Paddle { 
  ...
  void rightPaddleMove() {
    ...
    paddleX = constrain(paddleX, 900, 1000);
  }
  /*
  ...
  void collidePaddle1 (Ball b, Counter c, Setting s) { // If ball hits paddle, invert X direction
    if ( hitPaddle(b) == true ) {
      b.ballX = paddleX;
      b.ballSpeedX = random(7, 10);
      b.ballSpeedY = random(-10, 5);
