...
void mouseClicked()
{
     for(int i=0;i<num;i++)
     {
      ...
      if (clickDist <= 20)
        ...
        /*Set the x position for the stopped circle*/
        fixedXPos[i] = x;
       
        /* set stop to false if already stop is true */
        stop[i] = !stop[i];
       
        /* Set new speed if stop has been set to false */
        if (stop[i] == true)
          {
          speed[i] = random(10);
          }
