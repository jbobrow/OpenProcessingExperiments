...
/*
some of the parameters of these graphics are referred
 from other works on Open-processing.
...
draw the concentric circle with multi colors on the ring.
  ...
  // the parameters of the arc is referred from Open-Processing
  // I have written the source at the start of this practice.
...
void SECOND () {
  ...
  int m = minute();
  int x_coord = int(20*cos(s * unitAngle));
  int y_coord = int(20*sin(s * unitAngle));
  int startPoint = 243;
  int move;
  /*
  for(int i = 0; i < 60; i++){
  ...
  */
  ...
  // the ellipse moved as a circle
  strokeWeight(3);
  stroke(0);
  fill(186,192,128,99);
  ellipse(362 + x_coord,191 + y_coord,80,80); 
  ...
  noFill();
  strokeWeight(1);
  line(startPoint + s,0,startPoint + s,158);
  strokeWeight(2);
  line(startPoint + 4 + s,0,startPoint + 4 + s,158);
  //    two groups of horizontal lines, the upper one.
  strokeWeight(3);
  line(startPoint - 12 + s,28,startPoint + 25 + s,28);
  //    the lower one.
  strokeWeight(1);
  line(startPoint - 12 + s,142,startPoint + 25 + s,142);
  strokeWeight(2);
  line(startPoint - 12 + s,145,startPoint + 25 + s,145);
  //    the arc.
  strokeWeight(1.3);
  arc(startPoint + 14 + s,67,40,40,radians(85),radians(280));
  //the matrix with white and red on the right part of the frame.
  color c1;
  color c2;
  if(s%2 == 1) {
    c1 = color(191,80,67);
    c2 = color(223,186,142);
  }
  else {
    c1 = color(223,186,142);
    c2 = color(191,80,67);
  }
  noStroke();
  fill(243,236,226);
...
void HOUR() {
 ...
 int x_coord = int(50*cos(h * hourAngle + m * hourPlus + 1.5*PI));
 int y_coord = int(50*sin(h * hourAngle + m * hourPlus + 1.5*PI));
 int widthGrown = h + (m / 60);
 //    the concentric circles on the right side of the highest triangle.
  strokeWeight(2);
  fill(186,196,194);
  ellipse(207 + x_coord,123 + y_coord,42 + widthGrown,42 + widthGrown);
  noStroke();
  fill(72,102,153);
  ellipse(207 + x_coord,123 + y_coord,28 + widthGrown,28 + widthGrown);
