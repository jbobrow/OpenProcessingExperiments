  ...
  for (int i=0; i<num; i++) {
    ...
    if (i<num-1) {
      ...
      if (distance > rowHeight && distance < rowHeight + distanceMargin) {
        ...
        stroke(0, 20);
        drawCircle(x[i],y[i],distance/random(4),color(128,10),color(0,10));
...
void drawCircle(float x, float y, float wh, color cFill, color cStroke)
{
  //a little extra, this replaces the old fill/stroke colors back at the end
  float oldFill = g.fillColor;
  float oldStroke = g.strokeColor;
  fill(cFill);
  stroke(cStroke);
  ellipse(x,y,wh,wh);
  fill(oldFill);
