...
void draw() {
  ...
  for (int i=0; i<num; i++) {
    for (int j=0; j<num; j++) {
      ...
      if (circleNumber % 2 == 0) {           // check if circleNumber is even - remainder = 0
        movingCircle(tx, ty, cellsize, circleNumber * (-TWO_PI) * millis() / 60000.0);    // move to the opposite direction
      }
      else {
...
void movingCircle2(float x, float y, float size, float angle) {
  ...
  strokeWeight(1);
  fill(140, 180);
  ellipse(x, y, size, size); // ellipse

  stroke(0, 255, 0);
  line(x, y, tempX, tempY); // green line
}
