...
//            (myCells[i].location.y+myCells[j].location.y), (myCells[i].location.z+myCells[j].location.z));
//           
//           line(myCells[i].location.x, myCells[i].location.y,
//           myCells[i].location.z, cONEL[j].x, cONEL[j].y, cONEL[j].z); // draws a line from the center of the cell
...
//  boolean yhit = false;
  ...
  float w;
  float h;
  float depth;//different velocities for each axis, also i thought maybe it was JavaScripts random function that wasn't working as i kept getting diagonal lines...
  ...
  void display() {
    ...
    ellipse(location.x, location.y, dia-2, dia-2);
    }
    w = this.location.x;
    h = this.location.y;
    depth = this.location.z;
    map(w, 0, 0, width - 100, width-100);
    map(h, 0, 0, height-100, height-100);
    map(depth,  0, 0, width - 100, width-100); 
    println(max(w,h, depth));
