...
//AnimatedSpriteObject car;


class MouseRotatingCar extends AnimatedSpriteObject
{
  ...
  void update()
  {
    this.rotation = degrees(atan2(mouseY-this.y, mouseX-this.x)) + 90;
...
void setup()
{
  ...
  car = new MouseRotatingCar("top.jpg");
  car.x = width / 2;
  car.y = height / 2;
  car.speed = 5;
  car.frameWidth = 50;
  ...
  car.startFrame = 0;
  car.endFrame = 15;
  car.frameSpeed = 6.0; // 6fps

   
  for (int i = 0; i < bulletCount; i += 1)
  {
    b[i] = new Bullet("red.jpg");
    b[i].x = -1000;
    b[i].y = -1000;
    b[i].s = 0.25;
  }
...
void keyReleased()
{
    ...
    if (key == 's')
    {
      ...
      car.startFrame = 1;
      car.endFrame = 5;
      car.currentFrame = car.startFrame;
    }
