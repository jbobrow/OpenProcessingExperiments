...
void draw() {
  ...
  int num = 21;
  int margin = -10;
  float gutter = -7; //distance between each cell
...
void movingCircle(float x, float y, float size, int circleNum ) {
  ...
  float tempX1 = x + (size / 2) * sin(PI / frame_rate_value * finalAngle/2);
  float tempY1 = y + (size / 2) * cos(PI / frame_rate_value * finalAngle/2);

  stroke(50,100);
  fill(230,100);
  ellipse(tempX, tempY, size, size); 
  fill(230,230,100,200);
  ellipse(tempX1+x, tempY1, size/2, size/2);
}
