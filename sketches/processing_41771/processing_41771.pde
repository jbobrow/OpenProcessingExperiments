...
void setup() {
  ...
  for (int i = 0; i < p.length; i++) {
    ...
    // (Nick's comment) note the different origin locations, here
    // (Nick's comment) sparks are made near the top of the sketch, water comes up from the bottom
    if (random(10) > 5) {
...
class WaterParticle extends GenParticle {
  ...
  WaterParticle(float xIn, float yIn, float vxIn, float vyIn, float r, float ox, float oy) {
    ...
    age = 0; // (Nick's comment) age is used for color; water starts off blue and gradually turns to white and very transparent (vapor)
  }
  ...
  void display() {
    ...
    radius-=0.1; // radius gradually gets smaller
    age++;
  }

  void regenerate() {
    if ((x > width + radius) || (x < -radius)) { //conditions set up to draw within
      x = originX;
      y = originY;
      vx = random(-0.5, 0.5);
      vy = random(-3.0, -.0);
...
class SparkParticle extends GenParticle {
  ...
  void update() {
    if (y >= height - radius) { // (Nick's comment) added bounce here
      y = height/radius;
      vy = -0.5 * vy;
      radius = radius/0.25;
    }
  ...
  void regenerate() {
    if (age > 300) {    // (Nick's comment) sparks die at age 300
      x = originX;
      y = originY;
      vx = random(-1.5, 1.5);    // (Nick's comment) more x velocity than y
      vy = random(-1, -0.5);
      age = 0;
      radius = 5.0;
    }
...
class Particle {
  float x, y; // (NC) The x- and y-coordinates
  float vx, vy; // (NC) The x- and y-velocities
  float radius; // (NC) Particle radius
  float gravity = 0.1;
