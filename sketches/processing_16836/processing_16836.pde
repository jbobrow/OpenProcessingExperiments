...
PImage a,b,d,bg,plate,source,poke,no,plate2,poke2;
int card = 0; 
//strawberry
float c=25.0;
float e=25.0;
...
void draw() {
  ...
  if (card == 0) { 
    ...
    fill(#55A6E3);
    text("um... maybe need more..",250,400);
    ...
    fill(#55A6E3);
    ellipse(x, y, 40, 40);
    text("Looks more better!",360,400);

    //tetris
    tetris(1,160,3,180,0);
    tetris(0.8,160,3,200,0);
    tetris(1,160,3,-80,70);
    ...
    text("What a yammy Cake!",300,400);
    text("Thanks for it dear my friend!",250,430);
    
     textSize(12);
         fill(60);

    text("Bake again!",483,235);
    textSize(28);
    fill(#55A6E3);
...
//cheese
    ...
    //poke
    poke = loadImage("poke2.png");
    image(poke,400,230);
    
    
    fill(#F21616);
    text("DO U MAKE THIS?!!ITS SUCK!!!!!",250,400);
    textSize(12);
    text("Sorry.. I will make this again",420,260);
    textSize(28);
    fill(20);
    ellipse(x, y, 40, 40);
...
void mousePressed() {  
  ...
  if (card == 0) {  
    ...
    if (d < 20) {
      ...
      float d = dist(mouseX, mouseY, x-40, y-20);
      if (d < 20) {
        card = 1;
      }
      else card = 7;
...
void grape(int movex2, int movey2, float s) {
...
void tet2(float a2,float hi2, int movex2, int movey2) {
  scale(a2);
  translate(movex2,movey2);
