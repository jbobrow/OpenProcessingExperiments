...
//  background(255);
  ...
  for (int i=0; i<numArray; i++) {
    ...
    // bounce off a circle radius width -200
    if (((x[i]-width/2) * (x[i]-width/2)) + ((y[i]-height/2) * (y[i]-height/2)) > ((width/2 - 50) *(width/2 - 50)) ) {
       xInc[i] = xInc[i] > 0 ? -random(0.1,1) : random(0.1,1);
       yInc[i] = yInc[i] > 0 ? -random(0.1,1) : random(0.1,1);

    }
//    // bounce off the sides of the window
//    if (x[i] > width || x[i] < 0) {
//      xInc[i] = xInc[i] > 0 ? -random(0.01,1) : random(0.01,1);
//    }
//
//    if (y[i] > height || y[i] < 0 ) {
