...
final int kColorStep = 20;  // max amount by which rgb can random walk
...
void setup()
{
  ...
  // x/y are auto-inited to 0
  // init the rest of the arrays with rnd values
  for (int i = 0; i < kMaxBalls; i++) {
    gBallSize[i] = int(random(kSizeMax - kSizeMin)) + kSizeMin;  // random value between kSizeMin and kSizeMax - 1
    gBallRed[i] = int(random(128)) + 128;  // rnd vals between 128 and 255
    gBallGreen[i] = int(random(128)) + 128;
    gBallBlue[i] = int(random(128)) + 128;
  }
}
