...
void setup(){
  ...
  colorMode(HSB);
  for(int i=0; i<N ; i++){
     balls[i] = new Ball(100, 100, random(-10,10), random(-10,10), (float)i/N*256); //ballオブジェクト生成
  }
  background(255);
}
/////メインループ
...
class Ball{  //「ボールクラス」の定義
  ...
  int hue;  // 色指定
  Ball(float x0, float y0, float vx0, float vy0, float col){  //コンストラクタ
