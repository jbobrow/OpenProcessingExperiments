...
int c=100;
int d=-100;
int e=100, f=-100, g=100, h=-100,m=100, n=-100,o=100,p=-100,r=-1000,s=100,t=-1000,k=100;
int i=0, j=0;
...
void setup() {
  ...
  player.loop();
  minim = new Minim(this);
  bouncing = minim.loadSample("bouncing.mp3", 512);
...
void draw() {
  ...
  if (keyCode==ENTER) {
    ...
    g=480;
    h=230;
    text("SCORE: Block one time +10", r, s);
    r=500;
    s=260;
    text("Lose one time  -30", t, k);
    t=580;
    k=290;
     }
     

    if (mouseX-60<=x && mouseX+60>=x && 20<y && y<30 &&ySpeed<0 ) {
      ySpeed = -ySpeed;
      score = score + 10;
      bouncing.trigger();
    }
    if (y<=405 && y>=395 && mouseX-80<=x && mouseX+60>=x && ySpeed>0 ) {
      ySpeed=-ySpeed;
      score = score + 10;
      bouncing.trigger();
    }
    if (x>=15 && x<=30 && y<=mouseY+60 && y>=mouseY-60 && xSpeed<0) {
      xSpeed= -xSpeed;
      score = score + 10;
      bouncing.trigger();
    }
    if (x>=690 && x<=700 && y<=mouseY+60 && y>=mouseY-60 &&xSpeed>0) {
      xSpeed= -xSpeed;
      score = score + 10;
      bouncing.trigger();
    }
