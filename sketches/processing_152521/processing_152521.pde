...
void setup() {
 ...
 //lets create the array objects and initialise some variables.
 
  SizeOfRectangle= (width)/5;
  NumOfSquares=5;
  currentSquare=0;
 
 
  mainColor = new color[NumOfSquares];
  complimentaryColor = new color[NumOfSquares];
  ...
  for (int i=0; i<NumOfSquares; i++) {
    ...
    mainColor[i]= color(0, 0, 100);
 // returns a random float bewteen 0 and 1
    complimentaryColor[i] = color(0, 0, 100);
...
void DrawSquare(){
  ...
  if (isLocked) {
    mainColor[currentSquare] = color(hueValue, satValue, brightValue);
    complimentaryColor[currentSquare] = color(calculateCompHue(hueValue), satValue, brightValue);
  ...
  //create squares and inner squares and fill with alternate colours as selected
  
   for (int i = 0; i < NumOfSquares; i++) {   
    fill(mainColor[i]);
    rect(i* SizeOfRectangle, height-SizeOfRectangle, SizeOfRectangle, SizeOfRectangle);
    fill(complimentaryColor[i]);
    rect(i * SizeOfRectangle + SizeOfRectangle/4, height-SizeOfRectangle+SizeOfRectangle/4, SizeOfRectangle/2,         SizeOfRectangle/2);
  } 
...
void mouseReleased() {
  isLocked = false;
  //check current square number and increment or reset as necessary
   if (currentSquare<=4) { 
