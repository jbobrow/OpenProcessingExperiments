  ...
  for (int j=0; j<numV-1; j++) {
    for (int i=0; i<numH; i++) {
      if (i*j!=(numH-1)*(numV-2)) {
        ...
        beginShape(QUADS); 
        vertex(pos[circleNum].x, pos[circleNum].y);
        vertex(pos[(i+1)%numH+j*numH].x, pos[(i+1)%numH+j*numH].y);
        vertex(pos[(i+1)%numH+(j+1)%numV*numH].x,pos[(i+1)%numH+(j+1)%numV*numH].y);
        vertex(pos[i+(j+1)%numV*numH].x, pos[i+(j+1)%numV*numH].y));
        endShape();      
        //quad(pos[circleNum].x, pos[circleNum].y,
        //pos[(i+1)%numH+j*numH].x, pos[(i+1)%numH+j*numH].y, 
        //pos[(i+1)%numH+(j+1)%numV*numH].x, pos[(i+1)%numH+(j+1)%numV*numH].y,
        //pos[i+(j+1)%numV*numH].x, pos[i+(j+1)%numV*numH].y);
      }
    }
  }
  stroke(255, 103, 76);
