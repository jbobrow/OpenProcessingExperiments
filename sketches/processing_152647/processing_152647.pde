...
// The color-showing squares
int number = 8; //number of squares
int size = 100; // size of the squares
int gap = width/number;
color innerSquare[];
color outerSquare[];


float mouseWheelSensitivity = 2.5;
...
void setup() {
    ...
    (
      ...
      { 
          if (evt.getWheelRotation()>0) {
            saturation+=mouseWheelSensitivity; 
          }
          else{
            saturation-=mouseWheelSensitivity; 
          }
  ...
  innerSquare =  new color[number];
  outerSquare = new color[number];
  
  // initializing the vectors
  for (int i=0; i<number; i++){
     innerSquare[i] = color(0,0,0);
     outerSquare[i] = color(0,0,0);
  }
}
...
void draw() {
  ...
  fill( complementryHue, saturation, brightValue );
  ellipse(comHandX, comHandY, 40, 40);
  
  innerSquare[number-1] = color( complementryHue, saturation, brightValue );
  outerSquare[number-1] = color( hueValue, saturation, brightValue );
  
  // drawing the color-showing squares
  for (int i=0;i<number;i++){
    fill(outerSquare[i]);
    rect(i*size+gap, 10, size, size);
    fill(innerSquare[i]);
    rect(i*size+gap, 10, size/2, size/2);
 ...
 * 
 * and also, change the color of the squares
 */
void mouseReleased(){
  isLocked = false;
  for (int i=0;i<number;i++){
    outerSquare[i] = i<(number-1)? outerSquare[i+1] : color(0,0,0);
    innerSquare[i] = i<(number-1)? innerSquare[i+1] : color(0,0,0);
  }
}  
