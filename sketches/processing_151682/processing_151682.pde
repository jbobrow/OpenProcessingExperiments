...
void draw() {
  background(107, 141, 227);
  noStroke();

  int num = 10  ;
  int margin = 80;

  float gutter = 25; //distance between each cell
  ...
  for (int i=0; i<num; i++) {
    for (int j=0; j<num; j++) {
      ...
      if ( circleNumber % 2==0) {
        movingCircle(tx, ty, cellsize, circleNumber * TWO_PI * millis() / 600000.0);
      } else {
        movingCircle2(tx, ty, cellsize, circleNumber * TWO_PI * millis() / 600000.0);
      }
    }
  }
}//end of draw 
