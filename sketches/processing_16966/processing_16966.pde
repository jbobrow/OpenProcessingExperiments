...
drawSpiral(width/2,height/2 - 20,2 + mouseX/100, 20, PI/2,3,3);
drawSpiralT(width/2,height/2 - 20,2 + mouseX/100, 20, PI/2,3,3);

//drawSpiral(width-120,height/2,2 + mouseX/100, 20, PI/2,4,2);
//drawSpiralT(width/2-120,height/2,2 + mouseX/100, 20, PI/2,4,2);

drawSpiral(width/2,height- 60,2 + mouseX/100, 20, PI/2,5,2);
drawSpiralT(width/2,height - 60,2 + mouseX/100, 20, PI/2,5,2);
...
void drawSpiralT(float x, float y, int turns, float len, float angle, int depth,int depth2){
   ...
   if(depth > 0){
      ...
      if( depth2 > 0) {
      drawSpiralT(nextX ,nextY,turns  ,len,mouseY*angle/100 ,depth,depth2 -1);}
      //drawSpiral(nextX,nextY,-turns,len/1.1,-angle,depth-1);
      
      drawSpiralT(nextX,nextY,turns,len/1.7,nextAngle ,depth-1,depth2);
   }
}
