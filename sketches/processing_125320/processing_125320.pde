...
int _0point = 12; /* pontos fixos */
int _1point = 8; /* pontos com apoio variável */
int _2point = 12; /* pontos com apoio duplo */
...
float p0Spacing =-0.5;/* quanto mais próximo de 0.5, mais próximos os p0 */
float delay = 0; /* o delay para os pontos seguirem */
Boolean scroll = true; /* se é para a tela ir para o lado */
Boolean fade = true; /* se é para a tela escurecer o que ja passou */
int fadeSpeed = 8; /* velocidade de fade */
float stkWeight = 1; /* stroke Weight */
float opacity = 360;
Boolean rndSizes = false; /* se as esferas podem ter tamanhos aleatórios */

int sizeX = 1024; /* largura da tela */
int sizeY = 600; /* altura da tela */
...
void setup()
{
    ...
    background(0);
    size(sizeX,sizeY);//use P2D for more speed, no browser support.
    strokeWeight(1);
    stroke(255); 
    colorMode(HSB);
    int i=0;
