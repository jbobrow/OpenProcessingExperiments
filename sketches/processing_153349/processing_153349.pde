...
void draw() {
  ...
  // STEP 4: display the mousePosition string at the current mouse location
    if(mouseX >= width - textWidth(mousePosition)){
  textPositionX = width - textWidth(mousePosition);}
  else {textPositionX = mouseX;}
  if(mouseY <= textAscent()){
    textPositionY = textAscent();}
    else{textPositionY = mouseY;}
      
    text(mousePosition, textPositionX, textPositionY);
