...
public class Ball
{
  ...
  Ball(int X, int Y, int filly, float veloX, float veloY)
  {
    velX = veloX;
    velY = veloY;
    x = X;
    y = Y;
    rectangle = new Rectangle(width, height, hasStroke, strokeColor, hasFill, filly);
  ...
  void updatePosition() {
    ...
    if (alive == true)
      ...
      if (y>=gameFrameHeight-height) alive = false;
      xcentre = x+width/2;
      ycentre = y+height/2;
