...
class Leaf {
 ...
 // float radius;
  ...
  void draw () {
    fill(218,165,32);
    curve(Lcpx1+position.x, Lcpy1+position.y, position.x+50, position.y+50, position.x, position.y, Lcpx2+position.x, Lcpy2+position.y);
    curve(Rcpx1+position.x, Rcpy1+position.y, position.x+50, position.y+50, position.x, position.y, Rcpx2+position.x, Rcpy2+position.y);
    line (position.x+50, position.y+50, position.x-20, position.y-20);
    line (position.x+8, position.y+30, position.x+10, position.y+10);
    line (position.x+24, position.y+40, position.x+25, position.y+25);
    line (position.x+30, position.y+15, position.x+14, position.y+14);
    line (position.x+40, position.y+30, position.x+30, position.y+30);
  }
...
class PointMass {
  ...
  void updateInteractions () {
    ...
    if (mousePressed) {
      ...
      else if (mouseButton == RIGHT) { // if the right mouse button is clicking, we tear the cloth by removing links
