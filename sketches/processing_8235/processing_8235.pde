...
// if ( k== int(mix[0])) forward = false; //omit 
  ...
  upb(mix[1]);
   ...
   if ( k== int(mix[2])) {
    ...
    mouseX= px;mouseY= py; 
   // image(q,0,0);
   arraycopy(q.pixels,back.pixels);
   }
  if ( k== int(mix[3])) { forward = false;
  mouseX= px;mouseY= py;
    upb(mix[3]);
     arraycopy(w.pixels,back.pixels);
  }
   if ( k== int(mix[4])) { forward = false;
   mouseX= px;mouseY= py;
     upb(mix[4]);
      arraycopy(e.pixels,back.pixels);
   }
  if ( k== int(mix[5])) { forward = false;
  mouseX= px;mouseY= py;
    upb(mix[5]);
     arraycopy(r.pixels,back.pixels);
  }
   if ( k== int(mix[6])) {
     forward = false;
    mouseX= px;mouseY= py; 
      upb(mix[6]);
       arraycopy(t.pixels,back.pixels);
   }
  if ( k== int(mix[7])) { 
    forward = false;
   mouseX= px;mouseY= py; 
     upb(mix[7]);
      arraycopy(y.pixels,back.pixels);
  }
//  case mix[1]: 
//    forward = false;
//    break;
