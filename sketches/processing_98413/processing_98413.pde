...
import ddf.minim.spi.*;  // Posem les variables de so
...
void setup () {
 ...
 for(int i=0; i<numAstros; i++) { // Es donen valors al moviment de l'astronauta
  ...
  minim = new Minim(this);  // Adjuntem el so
  So = minim.loadFile("http://www.sonidosmp3gratis.com/sounds/001094906_prev.mp3");
  So2 = minim.loadFile("http://www.sonidosmp3gratis.com/sounds/astronaut.mp3");
...
if((posX[i]<30)||(posX[i]>width-30)){   // Es condicionen les restriccions de l'astronauta amb els limits de la finestra 
  ...
  PosY = PosY+VelY;
   ...
   if((PosX<300)||(PosX>width+250)){  // Es condicionen les restriccions del coet amb els limits de la finestra  
    VelX = -VelX;
     So.cue(0);  // Es condiciona que cada cop q el coet toqui amb el limit de la finestra soni un so de laser
     So.play();
   } 
