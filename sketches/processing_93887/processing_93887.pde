...
float ux;
float uy;
float htimer, mousetimer, mousemovex;
float bfiller= 255;
float bfiller2 = 200;
float redpos, bluepos;
 float amp;
 float start;
 float end;
PFont font;
boolean maindna = true;
...
void setup(){
  ...
  smooth();
  background(0);
  font = loadFont("CenturyGothic-100.vlw");
  textFont (font);
  redpos = width/2;
 bluepos = width/2;
 amp=height/8;
...
void draw(){
  ...
  //initiate classes////////////////////////////////////////////////////////
    ...
    dnaswirl dswirl = new dnaswirl(redpos,bluepos, amp, 0, start,8, end,15, height);//here change  (positionblue, postion red, amplitude, starting angle, start,start2, end,end2, height)
...
if ((mousemovex==mouseX)&&(htimer>=5000)){ //needs to come last so rectangle can be drawn over everything also needs to reset all areas.
...
if( bfiller2>= 200){
  ...
  noStroke();
    fill(bfiller, bfiller-100);
    textAlign(CENTER);
   
    textSize(50);
  text("Telomeres", width/2, height/2);
