...
float ux; //speedx
float uy; //speedy
float maxs = 70; //max particle size
float mins = 30;  //min particle size
int i,j = 0; //counters
int hm = 0; //how many ellipses
...
void nocolision(){
  //for(int b=0; b<4; b++){
    if (dist(x[i],y[i],x[j],y[j])<+(s[i]+s[j])/2){
      float plus = -dist(x[i],y[i],x[j],y[j]) +(s[i]+s[j])/2+1;//20;
      float xk = 0;
