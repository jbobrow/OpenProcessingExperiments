  ...
  for (int i=0; i<num; i++) {
    for (int j=0; j<num; j++) {
     ++circleNumber;
      ...
      if (circleNumber%2==0)
      {
        movingCircle(tx, ty, cellsize, circleNumber * PI * millis() / 60000.0);
      }else{
...
void movingCircle(float x, float y, float size, float angle) {
  ...
  float t=abs(cos(radians(frameCount*2))*size);
  float c=abs(sin(radians(frameCount))*255);
 
  // calculate endpoint of the line
  float tempX = x + (size/3 ) * sin(angle);
  float tempY = y + (size / 3) * cos(angle);
