...
void setup() {
  size(200, 200);
  myPic = loadImage("web.jpg");
  for (int b =0 ; b < bug.length;b++) {
    bug[b]= new Bug();
  }
  spdr = new Spider(new PVector(width/2, height/2), 2.0, 0.1);
  smooth();
}
...
class Bug {
  ...
  float x = random(0, myPic.width);
  float y = random(0, myPic.height);
...
class Spider {
  ...
  void render() {
    // Draw a spider rotated in the direction of velocity
