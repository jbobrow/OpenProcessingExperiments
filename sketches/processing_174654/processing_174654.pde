...
PVector[][] patch;
int RESOLUTION = 5; // patch resolution
void setup() {
  size(600, 600);
   stroke (#432B0D);
...
void draw() { 
  ...
  randomSeed(3);
  // design a patch
  PVector lBottom = new PVector(300, height-200); // left bottom 
  PVector mBottom = new PVector(width/2, height); // middle bottom 
  PVector rBottom = new PVector(width-200, height-200); // right bottom 
  PVector lMiddle = new PVector(0, height/2); // left middle
  PVector mMiddle = new PVector(width/3, height/3); // middle middle 
  PVector rMiddle = new PVector(width, height/2); // right middle
  PVector lTop = new PVector(300, 300); // left top
  PVector mTop = new PVector(width/3, 0); // middle top
