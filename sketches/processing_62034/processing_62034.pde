...
public void setup() {
  ...
  vel = new PVector(7, 8);
  icePos= new PVector((int)(.5*width),300);
  iceVel=new PVector(6,7);
  radius=10;
  compScore=0;
  playerScore=0;
  rectlength=80;
...
public void changeIceVel(int x, int y) {
  iceVel=new PVector(x,y);
}
...
public void draw() {
  if (gameOvered==false && gameStarted==false) {
    ...
    //creating and setting up the iceball
    fill(0,0,255);
    noStroke();
    ellipseMode(CENTER);
    ellipse(icePos.x, icePos.y, radius/1.5, radius/1.5);
    icePos.add(iceVel);
    ...
    //iceball bouncing back from the top and bottom walls
    if ( icePos.y + radius >= height ) {
      iceVel.y = -iceVel.y;
    }
    if ( icePos.y + radius <= 0 ) {
      iceVel.y = -iceVel.y;
    }
  ...
  //iceball bounces back from the player paddle and right walls
    if (icePos.x >=688&& icePos.y<=mouseY+40 && icePos.y >=mouseY-40) {
      if ( icePos.y < mouseY) {
        icePos.x=685;
        iceVel.x = -iceVel.x; 
        iceVel.y = -iceVel.y;
