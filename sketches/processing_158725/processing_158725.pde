...
/* @pjs preload="masson4e2012.jpg,masson4e2009.jpg,masson4e1951.jpg";*/
...
void setup() {
  size(684, 544);// pour le web, le setup doit ^çetre hard codé
  lengthPixels=684*544; //Résulution de l'image.
  //Initialisation du loading l'image.
  img[0] = loadImage("masson4e2012.jpg");
  img[1] = loadImage("masson4e2009.jpg");
  img[2] = loadImage("masson4e1951.jpg");
...
void draw() {
  ...
  if (!pause) { //Si l'image n'est pas sur pause.
    ...
    assoupFacteur=pow(anneeDiff+1.5, -0.9)/(baseParAn)*2; //calcul du facteur d'assouplissement (le fade) entre le noise et l'image cible
     // println( "anneeCourante:"+anneeCourante+"  anneeCible:"+anneeCible+"  imageHackWebShow:" + imageHackWebShow +"  indexCible:" + indexCible + "  anneeDiff:"+anneeDiff+ " assoupFacteur:" + assoupFacteur); //debug
