...
void draw() {
  ...
  for (int i=0; i<num; i++) { // column in y
    for (int j=0; j<num; j++) { // row in x
      ...
      movingCircle2(tx, ty, cellsize, -circleNumber * TWO_PI * millis() / 60000.0); //negative value to circleNumber variable to change the direction
    }
  }
}//end of draw 
...
void movingCircle2(float x, float y, float size, float angle) {
  ...
  strokeWeight(1);
  fill(140, 180);
  rect(x, y, size, size); // rectangle. Change Ellipse command to Rect command
