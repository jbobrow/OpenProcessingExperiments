...
void draw() {
  ...
  fill(135,7,37);
  text("Pulsa espacio para lanzar una bola", (width/2)-105, 70);
...
class Particle
{
  ...
  //DETECCIÓN COLISIÓN PARTÍCULA-PARTÍCULA
  void collision(Particle p)
  {
    ...
    if(dist < (this.radio+p.radio) )
    {
      ...
      // necesitamos vectores paralelos al vector 'd' de módulo 'v1' y/o 'v2'
      // por tanto multiplicamos 'v' por un vector unitario 'unitD' en la dirección de 'd'
...
class Tablero {
  ...
  Tablero()
  {
    ...
    //Planos inclinados
    Plano inclinadoIzq = new Plano((width-35)/2-150,height-190,(width-55)/2-100,height-150, true);
    Plano inclinadoDer = new Plano((width-185)/2+150,height-150,(width-205)/2+200,height-190, true);
