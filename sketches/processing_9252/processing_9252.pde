    ...
    */
    ...
    if ( labelVisible ){
      ...
      if( sliderValueVisible ) 
        text(nf(int(sliderValue), 3), int(x+w/2 - 5) + sliderValueMarginLeft, int(y+h/2 + labelSize/3.0) + sliderValueMarginTop);
    }
  } // end void build
...
class GUI_TextField extends GUI_Control{
  ...
  void keyPressed(){
    ...
    if ( !(key == CODED || key == BACKSPACE || key == ENTER || key == RETURN || key == TAB || key == DELETE)){
