...
class APunkt {
  ...
  void update() {
    if (schrank)
    {
      spos.x=pitch+ idx*pitch;
      spos.y=pitch+ idy*pitch;
    }
    else
    {
      spos.x =width/4*3-0.5*gr;
      spos.y=pitch+idschrank*pitch;
    }
    ...
    if (p.checkOver(mouseX, mouseY)) {
      ...
      else
      {
        ...
        for (int j=0; j<myPunkte.length; j++) {
          if(myPunkte[j].idschrank > p.idschrank)myPunkte[j].idschrank--;
        }
        schrankcount--;
      }
    }
...
void bereiteVor() {
 ...
 myPunkte[2].col2=100;
  ...
  myPunkte[15].bild=loadImage("2m.jpg");
  myPunkte[15].idx=1;
  myPunkte[15].idy=8;
  
  myPunkte[16].bild=loadImage("3m.jpg");
  myPunkte[16].idx=2;
  myPunkte[16].idy=8;
  
  myPunkte[17].bild=loadImage("4m.jpg");
  myPunkte[17].idx=3;
  myPunkte[17].idy=8;
