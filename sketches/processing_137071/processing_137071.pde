...
Minim minim;
AudioPlayer gameOver, wall, paddle, topbottom;
...
void setup() {
  ...
  gameOver = minim.loadFile("powerdown07.mp3");
  wall = minim.loadFile("button03b.mp3");
  paddle = minim.loadFile("select04.mp3");
  topbottom = minim.loadFile("poka02.mp3");
}
...
void draw() {
  ...
  /* pushStyle();
   while (xVelocity < 0) {
   fill(255+(int)xVelocity, 255+(int)xVelocity, 255);
   }
   while (xVelocity > 0) {
   fill(255, 255, 255-(int)xVelocity);
   }
  ...
  if (yBall > height - 12.5 || yBall < 12.5) {
    ...
    //topbottom.rewind();
    rally2++;
    tbwallR = (int)random(200, 255);
    tbwallG = (int)random(100, 255);
    tbwallB = (int)random(100, 255);
    //tbAlpha = 255;
