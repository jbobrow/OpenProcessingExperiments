...
// Carnival:since I say rhat this painting refers to a scene of carnival and music concert, I make a person 
//on the left above be a music conductor. the so called"music heart" on the right above is where all the
//melody comes from. All the music parts will be conducted an move with the sway and command of the conductor.
...
void draw(){
 ...
 beginShape();
 vertex(279,110);
 bezierVertex(mouseX-386,mouseY-2,mouseX-391,mouseY-9,mouseX-392,mouseY-23);
 bezierVertex(mouseX-383,mouseY-48,mouseX-382,mouseY-61,mouseX-390,mouseY-67);
 endShape(); 
 
 noFill(); //right hand
 stroke(0);
 bezier(310,111,mouseX-280,mouseY-8,mouseX-280,mouseY-27,mouseX-290,mouseY-70);
 ...
 pushMatrix();
 translate(434,102);
 float spin6=PI*(mouseX+mouseY)/100;
 rotate(spin6);
 ...
 pushMatrix();
 translate(110,74);
 float spin5=PI*(mouseX+mouseY)/200;
 rotate(spin5);
