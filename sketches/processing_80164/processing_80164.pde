...
void startEnGameOverScherm() {
  if (startGame == false) {
    background(0);
    textAlign(CENTER);
    fill(255);
    text("Highscore "+highscore, w * breedteKarel / 2, (h * (breedteKarel / 8))* 1);
    text("Press ENTER button to play", w * breedteKarel / 2, h * ((breedteKarel/ 4)));
    textFont(visitor, 48);
    text("- START GAME -", w * breedteKarel / 2, h * breedteKarel / 2);
    textFont(visitor, 30);
    text("Pres 'P' to pause the game", w * breedteKarel / 2, h * breedteKarel);
    text("Gather as much balls as possible in 60 seconds", w * breedteKarel / 2, h * (breedteKarel / 3));
    fill(#FF3B3B);
    text("Grab the RED balls for extra time!", w * breedteKarel / 2, h * ((breedteKarel / 4) * 3));
    textAlign(RIGHT);
  }

  //Als het spel voorbij is, reset en laat text zien game over
  if (gameover == true && startGame == true) {
    background(0);
    textAlign(CENTER);
    fill(255);
    textFont(visitor, 48);
    text("- GAME OVER -", w * breedteKarel / 2, h * breedteKarel / 2);
    textFont(visitor, 35);
    text("Hit ENTER to play again!", w * breedteKarel / 2, h * ((breedteKarel/ 4) * 3));
    textAlign(CENTER);
    textFont(visitor, 60);
    text("Score "+score, w * breedteKarel / 2, (h * (breedteKarel / 4))* 1);
    textFont(visitor, 55);
    text("Highscore "+highscore, w * breedteKarel / 2, (h * (breedteKarel / 8))* 1);
  }
  // als de tijd op is laat de score zien
  if ((timerStartProgramma/1000) > eindeTijd) {
    gameover = true;
  }
}
