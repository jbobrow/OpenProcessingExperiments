...
//create 2 arrays to hold the colours and number is the numer of options
float[] mainColor;
float[] compColor;
int num;
...
// counter
//this is the number times the mouse is clicked
int counter;
...
void setup() {
  ...
  counter=0;
  //initialise array
  mainColor= new float[num];
  compColor=new float[num];
  
  for (int i=0;i<num; i++){
    mainColor[i]=color(0,0,100);
    compColor[i]=color(0,0,100);
;
  }
...
void draw() {
  ...
  //drwa square of the colour
    mainColor[counter]=hueValue;
  //  rect(50,600,40,40);
  // draw the boxes
    fill(0,100,brightValue);
  for(int i=0;i<num; i++){
    fill(mainColor[i],100,brightValue);
    rect(50+(i*70),700,80,80);
  }
  ...
  compColor[counter]=complementryHue;
  // draw a squae with the comp colour
  //rect(50+(counter*20),600,20,20);
  fill(0,100,brightValue);
   for(int i=0;i<num; i++){
    fill(compColor[i],100,brightValue);
    rect(40+(i*70),700,20,20);
  }
