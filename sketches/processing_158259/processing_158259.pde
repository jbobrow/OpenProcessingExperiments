...
void setup(){
  ...
  cursorSSlider = new GSlider("Cursor Size",560,800,150,10,.2,color(106,148,212));
}
...
void draw(){
  ...
  rect(-10,750,760,110);
  tesselType.refresh();
  cursorSSlider.refresh();
  cursorWidth = cursorSSlider.getPos()*50;
  textAlign(LEFT, TOP);
  fill(250);
  textSize(10);
  text("Tessella v0.91 by Guillaume Riesen",560,830);
  rect(280,770,240,60);
  fill(100);
  textSize(12);
...
class Rope{
  ...
  void optimize(){
    for(int i=0;i<pointList.size()-1;i++){
      float dx = pointList.get(i+1).getX()-pointList.get(i).getX();
      float dy = pointList.get(i+1).getY()-pointList.get(i).getY();
      float distance = sqrt(sq(dx)+sq(dy));
      if(distance<4){
        pointList.remove(i+1);
      }else if(distance>8){
        pointList.add(i+1,new tuple(pointList.get(i).getX()+(5/distance)*dx,pointList.get(i).getY()+(4/distance)*dy));
      }
    }
  }
