...
float heart(float beat){        
   ...
   if ((bigCounter/Nshapes)%bF==0){beat=bigBeat;}                  // if counter is multiple, write data. 
   if ((smallCounter/Nshapes)%sF==0){beat=smallBeat;}              // counter/number of shapes because called from for
   else beat=0;
