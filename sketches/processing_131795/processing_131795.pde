...
// JuanJu Rojas, código modificado el 2 de febrero 2014, basado en código de mwachowski
...
int shapeRadius = 12;  //radio modificado JuanJu ROjas
int shapeQty = 75;  //Cantidad modificada JuanJu Rojas
 
float distanceToDrawLine = 140; //distancia modificada JuanJu ROjas
...
void setup() {
  size(900, 450); //tamaño de canvas modificado JuanJu Rojas
  colorMode(RGB, 255);
  //noCursor();
  smooth();
  ...
  for (int i=0; i < shapeQty; i++) {
    ...
    shapeAngle[i] = (int)random(250,360);
     
    //sets shape speed
    shapeSpeed[i] = random(0.25,5);
  }
}
