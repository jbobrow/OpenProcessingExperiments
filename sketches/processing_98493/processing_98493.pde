...
void setup()
{
  ...
  player = new Player();
  asteroids = new enemyAsteroids[20];
...
//Increases the score based on player's movement
//If the player doesn't move, the score doesn't increase.
...
/*
 Collision detection function for asteroids
...
//A simple class to represent the enemy asteroids
class enemyAsteroids {
  ...
  void move() {
    ...
    y += speedY;   
    // bounce
    if ( x < 0 || x > width )  speedX *= -3;
    if ( y < 0 || y > height ) speedY *= -3;
  }
