...
void draw() {
  ...
  decorateBackground(int(frameCount%1000>500));
...
void mouseDragged() {
  ...
  if (brightnessSlider.dragged && abs(mouseY-brightnessSlider.y)<10 && abs(mouseX-(brightnessSlider.x+brightnessSlider.w/2))<brightnessSlider.w/2) {
    int mycol=(int)colorSliders.get(selectedCs).col;
...
void decorateBackground(int mode) {
  ...
  case 0:
    ...
    break;
    case 1:
    if(colorStrips.size()==0 || frameCount%250==0)preparedeco();   
    noStroke();
    int reached=0;
    for(int n=0;n<colorStrips.size();n++){
      fill(colorSliders.get((Integer)colorsList.get(n)).col);
      rect(0,reached,width,(Integer)colorStrips.get(n));
      reached+=(Integer)colorStrips.get(n); 
    }
    break;
  }
}
...
class ColorSlider {
  ...
  void display() {
    col=pixelToColor(x-cwx+cwr, y-cwy+cwr);
    col=color(hue((int)col), saturation((int)col), bright);
    sliderWheel.beginDraw();
