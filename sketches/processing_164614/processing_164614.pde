...
//draw los triangulos de las particulas
void draw() {
  background(0);
...
class Particle {
  ...
  Particle() {
    ...
    vel = new PVector(random(1, 3), random(1, 2));
    acc = new PVector(random(-0.1, 0.1), random(0.1, 0.4));
    thres = 75;
    areaThres = 2000;
    c = color(random(200), 255, 255);
  }
  ...
  void displayConnection(Particle p) {
    ...
    if (dst < thres) {
      ...
      stroke(avgHue, saturation, random(64), alpha);
  ...
  void displayTriangle(Particle p1, Particle p2) {
    ...
    if (area < areaThres && dst1<thres && dst2<thres && dst3<thres) {
      noStroke();
      float avgHue = (hue(c)+hue(p1.c)+hue(p2.c))*0.51;
