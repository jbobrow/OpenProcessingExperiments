...
ArrayList particles;
ArrayList falldown;
ArrayList falling; //inserted AW

void setup(){
  size(360, 360);
  particles = new ArrayList<Particle>();
  
  falling = new ArrayList<Particle>();  //inserted AW
  
  
  // creates a grid of green "particles"
...
void draw(){
  ...
  for (int i=0; i<falling.size(); i++) {
   Particle p = (Particle)falling.get(i);
   p.run();
  }
...
// creates a second arraylist with falling particles
