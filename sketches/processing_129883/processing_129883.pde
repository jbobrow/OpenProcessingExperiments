...
class Pea {
  ...
  void seek(PVector _n, float _m) {
    ...
    n.mult(_m + mass); // Factor mass of both objects into acc
  ...
  void display() {
    ...
    // Drawing the pea twice insures that it
    // won't just dissapear when wrapping
    line(loc.x, loc.y, pploc.x, pploc.y);
    wrap();
    line(loc.x, loc.y, pploc.x, pploc.y);
    
    // Update previous coordinates for path mapping
    pploc.set(ploc);
    ploc.set(loc);
  }
