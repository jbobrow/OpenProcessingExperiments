...
void setup() {
  ...
  nvCirc = new CircleText(width/2, height/2, "");
  noLoop();
}
...
void draw() {
  ...
  for(int i = 0; i < limit; ++i){
    nvCirc.drawSymbolsByRadius(symmb[int(random(symmb.length))], i*20*1.6+100);
  }
    redraw();
}
...
void keyPressed() {
  ...
  case 'r':
  case 'R': { limit = (int)random(min, max); }
  default : { redraw(); }
  }
}
