...
float ballSpeedX = 5;     // speed of the ball horizontally
float ballSpeedY = 5;     // speed of the ball vertically
int paddleHeight = 30;  // height of the paddle
int paddleColor;        // color of the paddlei
boolean button = false;

void setup() {
  size(800, 600);   // canvas size
  noCursor();       // hides the mouse
  ballX = width/3;  // start the ball in the
...
void checkCollide() {
  ...
  if (ballX < 0 || ballX > width) {
    ballSpeedX = ballSpeedX * -1.1; // switch X direction, how fast the ballX would move
  }
