...
/*Tarea Pythagoras tree
 * Recursive Hypotenuse
 * Author Herbert Spencer
 * February 2014
 * Tarea de Mariana Mora, hackeando el codigo de 
 * Herbert Spencer, se variaron los colores y la forma del movimiento
 */

float ang;  // this is the global angle
int levels; // this is the amount of tree levels or how many time we eill call the recursive function

void setup() {
  size(700, 700);
  noStroke();
  smooth();
  levels = 12;       // start with something small
...
void hypotenuse(float x1, float y1, float x2, float y2, int level) {
  ...
  pushMatrix();
  {
    ...
    // draw the square
    fill(255,20); // nice color with transparency
