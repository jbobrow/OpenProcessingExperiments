...
//
float innerR = 50; // inner
float outerR = 130; // outer
float outerR2 = outerR * 2; // limit of the handle's "pull" range
...
void setup() {
  ...
  rectMode(CENTER);
   ...
   colorHandleX = width/2+250;
   colorHandleY = height/2;
   swatchNum = 0;
    
   currentColour =new float[6];
   complimentColour =new float[6];
   swatchSat = new float[6];
   swatchBright = new float[6];

  for (int i=0; i<6; i++) {
       currentColour[i] = 0;
       complimentColour[i] = 180;
       swatchSat[i] = 100;
       swatchBright[i] = 0;
   }
...
void draw() {  
  ...
  for (int i=0; i<6; i++) {
       noStroke();
       fill(currentColour[i], swatchSat[i], swatchBright[i]);
       rect(50, i*100+50, 100, 100);
       fill(complimentColour[i],  swatchSat[i], swatchBright[i]);
       rect(50, i*100+50, 50, 50);
   }
  ...
  for (int i=0; i<=100; i++) {
    float angle = radians(3.6*i-90); // 10 x 36 degree steps
    fill(3.6*i, 100, 100);
...
void mouseReleased() {
  isLocked = false;
  currentColour[swatchNum] = hueValue;
  complimentColour[swatchNum] = complementryHue;
  swatchBright[swatchNum] = brightValue;
  if (swatchNum<5){
      swatchNum++;}
  else{
      swatchNum=0;}
