...
// Written by Ruben Aguirre 
// Ashe and arrow property of Riot Games.
...
void mousePressed() {                       // arrow moves to where the mouse is clicked.
  ...
  if (endX<200) {                                // Lots of if statements to make sure the arrow looks correct when shot at an angle.
    angle = -PI/3;
  }
  else if(endX<300){
    angle=-PI/4;
  ...
  if(endY>600 && endX<500){
    angle =-PI/2;
    }
  else if (endY>600 && endX>500){
    angle = PI/2;
}
}
