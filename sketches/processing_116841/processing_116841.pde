...
// Miro clock, with the directions of sunrise,sunset, moonrise, moonset to indicate the time, the hours, 
// Daylight sky and dark night are shown.
// the turning star refer to the seconds.
// Su Hsiu Pai
//suhsiupai"gmail.com
...
void draw() {
  ...
  //set turning centre point of moon according change time
  if (h==22) {
    drawMoon(548, 235);
    drawSun(250, 761);
  }
  ...
  /*
// draw the middle star
   tr = dist(_mStarX, _mStarY, _sunX, _sunY);
   tx = tr*cos(_mStarA+ta)+_sunX;
   ty = tr*sin(_mStarA+ta)+_sunY;
   drawOneStar(tx, ty, 33, 52);
   // draw the small star
   tr = dist(_sStarX, _sStarY, _sunX, _sunY);
   tx = tr*cos(_sStarA+ta)+_sunX;
   ty = tr*sin(_sStarA+ta)+_sunY;
   drawOneStar(tx, ty, 28, 29);
   // end tweak
   
   
   // new function added by schien@mail.ncku.edu.tw
...
void drawMoon(float x, float y) {
  ...
  //center at (308.5,199.5)

  float cX = x;
  float cY = y;
  vertex(cX+137.5, cY-187.5);
  bezierVertex(cX+137.5, cY-187.5, cX-129.5, cY-97.5, cX-137.5, cY-3.5);
  bezierVertex(cX-137.5, cY-3.5, cX-131.5, cY+166.5, cX+21.5, cY+187.5);
  bezierVertex(cX+21.5, cY+187.5, cX+17.5, cY+96.5, cX+22.5, cY+1.5);
  bezierVertex(cX+22.5, cY+1.5, cX+100.5, cY-171.5, cX+137.5, cY-187.5);
  endShape();
}
...
void drawstar(float sX, float sY)
{ //draw lines (Right star)
  ...
  translate(400, 500);
  rotate(-PI/s*30);
  line(sX-104, sY, sX+80, sY);
  line(sX, sY-76, sX+3, sY+100);
  line(sX-80, sY-100, sX+92, sY+120);
  line(sX-100, sY+90, sX+90, sY-80);
