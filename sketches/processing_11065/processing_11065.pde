...
public class Avoider extends BoundedBox{
  ...
  public Avoider(int startX, int startY, int startS,int speed, int topX, int topY, int bottomX, int bottomY){
    super(startX,startY,startS,topX,topY,bottomX,bottomY);
    speedLimit = speed;
    type = 6;
  }
  ...
  public void act(ArrayList bots){
    ...
    if (newX > x)
      vX-=2*(100/distance);//(int)(distance)/5;
    if (newX < x)
      vX+=2*(100/distance);//(int)(distance)/5;
    if (newY > y)
      vY-=2*(100/distance);//(int)(distance)/5;
    if (newY < y)
      vY+=2*(100/distance);//(int)(distance)/5;
    //println(vX);
    //println(vY);
