...
class Agent {
  PVector p, pOld;
  float zNoise, zNoiseVelocity = 0.1;
  float stepSize, angle, randomizer;
  color col;

  Agent() {
    p = new PVector(random(height),random(height));
    pOld = new PVector(p.x,p.y);
    randomizer = random(5);
    stepSize = 1+randomizer*10;
    
    // colors
    if (randomizer < 5) col = color((int)random(170,190), 70, (int)random(0,100));
    else col = color((int)random(40,60), 70, (int)random(0,100));
    
    // init zNoise
    setNoiseSticking(0.1);
  }
