    ...
    if (s.longitude != 0 && s.latitude != 0) {
      ...
      if (s.tempF < 9999.0) {
        if (bFuzzy) {
          ...
          noStroke();
          if (bAll) ellipse(x,y, 7,7);
          else ellipse(x,y, 11,11);
        } else {
...
void drawScale() {
  for (int i=0; i<140; i++) {
    stroke(i,255,130);
    line(width-25,i+140, width-10,i+140);
  }
  textAlign(RIGHT,CENTER);
  fill(0,255,150);
  text(nfc(maxTemp,1)+"°",width-5,130);
  fill(140,180,255);
  text(nfc(minTemp,1)+"°",width-5,286);
}
...
void gatherData() {
  ...
  if (!bNOAAConnect) {
    ...
    if (stations.size() > 0) {
      ...
      text(currChild+" of "+childCnt, 5, height-68);
    }
