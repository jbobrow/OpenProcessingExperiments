...
/*LeapDrawing by Sonia Martin
 ...
 References
 -help from Ms. Freed
 -LeapMotion help and advice from Pauline
 Websites:
...
Shapes gameLearnShapes=new Shapes();
...
int shown = 0;
int oldSwipe = 0;
boolean gameModeBegin = false;
boolean learnModeBegin = false;
...
void setup() {
  ...
  titleTextIncorrect = cp5.addTextlabel("label8")
    .setText("Screen tap to try again!")
      .setPosition((width/2)-30, height-120)
        .setColorValue(color(200, 20, 20))
          .setFont(createFont("Candara", 40))
            ;
  titleTextIncorrect.hide();
  ...
  titleTextRhombus = cp5.addTextlabel("label4")
    .setText("Rhombus")
      .setPosition((width/2)-50, height-70)
        .setColorValue(color(10))
          .setFont(createFont("Candara", 40))
            ;
...
void leapOnSwipeGesture(SwipeGesture g, int state) {
  ...
  case 2: // Update
    break;
  case 3: // Stop
    if (gameModeBegin || learnModeBegin) {
      swipe = true;
    }
...
void gameMode() {
  ...
  if (swipe) {
   ...
   // println("swipe2 " + swipe2);
    titleSubtextGame.hide();
    titleTextGame.hide();
    
    println("swipe2:" + swipe2);
   println("oldSwipe" + oldSwipe); 
    if (swipe2>oldSwipe+8) {
      println("greateroldswipe");
      //submit:
...
//        }
    ...
    //  if (tooBig == false){
      if (distance<=200) {
        ...
        points.clear();
        tooBig = false;
        //If user has finished going through all shapes, reset and go to the home page.
        if (shown>4) {
          //high score
          reset();
        }
...
void reset() {
  ...
  titleTextIncorrect.hide();
  gameModeBegin = false;
  learnModeBegin = false;
  points.clear();
 
  draw();
}
 
float distance(int shown, int i) {
  //circle:
  if (shown == 0) {
    distance = dist(points.get(i).x, points.get(i).y, width/2, (height/2)-30);
  //  println(distance);
  }
  return distance;
}
  ...
  void show(int temp) {
    ...
    if (counter >1) {
      ...
      for (int i=0; i<counter; i++) {
        ...
        //If it has recorded more than one position, draw a line from the previous position to the next position, creating the path onscreen of the finger.
        if (i>0) {
          line(points.get(i-1).x, points.get(i-1).y, points.get(i).x, points.get(i).y);
        }
    ...
    if (temp == 1) {
      line(width/3, (height/3)*2, 2*(width/3), (height/3)*2);
      line(2*(width/3), (height/3)*2, width/2, height/4);
      line(width/2, height/4, width/3, (height/3)*2);
    ...
    if (temp == 2) {
      ...
      line(width/2, 2*(height/3), 2*(width/3), 2.5*(height/6));
      line(2*(width/3), 2.5*(height/6), width/2, height/6);
      line(width/2, height/6, width/3, 2.5*(height/6));
      line(width/3, 2.5*(height/6), width/2, 2*(height/3));
    }
    if (temp == 3) {
    ...
    if (temp == 4) {
      ...
      line(width/5, height/2, 3*(width/4), height/2);
      line(3*(width/4), height/2, 4*(width/5), height/3);
      line(4*(width/5), height/3, width/4, height/3);
      line(width/4, height/3, width/5, height/2);
    }
  }
}
