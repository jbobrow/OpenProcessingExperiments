...
void drawmatrix(){
      strokeWeight(1);
  for (int px = width/2 -250; px <= width/2 + 250; px += matrixstep){
    for (int py = height/2 -250; py <= height/2 + 250; py += matrixstep){
      float dista = distance(new PVector(px, py), new PVector(width/2, height/2));
      float mdist = 150;
      for (int i = Balls.size()-1; i >= 0; i--){
        Ball a = (Ball) Balls.get(i);
          float nd = distance(a.position, new PVector(px, py));
            if ( nd < mdist){
              mdist = nd;
            }
      }
      float a = grav_all_field(new PVector(px-matrixstep/2, py)).mag()*100;
      float b = grav_all_field(new PVector(px+matrixstep/2, py)).mag()*100;
      float c = grav_all_field(new PVector(px, py-matrixstep/2)).mag()*100;
      float d = grav_all_field(new PVector(px, py+matrixstep/2)).mag()*100;
      if ((dista<100 || mdist < 45) && (a < 100 && b < 100 && c < 100 && d < 100)){
        stroke(0, dista, 255-dista);
        line(px-matrixstep/2, py, 25-a, px+matrixstep/2, py, 25-b);
        line(px, py-matrixstep/2, 25-c, px, py+matrixstep/2, 25-d);
      }
    }
  }
      strokeWeight(1);
