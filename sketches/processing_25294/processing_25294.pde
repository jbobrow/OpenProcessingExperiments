...
int rayon = 22;  //distance entre le point de départ d'un arc et son extrémité  
...
void setup(){  
  ...
  strokeWeight(0);  //définit la taille du trait  
...
void draw(){  
  ...
  fill(random(255), random(255), random(255), random(255));  //couleur aléatoire  
    ...
    for ( int x = 20 ; x <400 ; x+=20 ) {  //première boucle (largeur: x)  

      for ( int y = 45 ; y <400 ; y+=100 ) {  //deuxième boucle (hauteur: y)  

       arc(x, y, sz, sz, 0, PI);  //arc en bas à droit  
