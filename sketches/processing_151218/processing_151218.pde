...
void draw() {
  ...
  for (int i=0; i<num; i++) {
    for (int j=0; j<num; j++) {
      ...
      if (evenodd == 0) {  
        thing1(tx, ty, cellsize, circleNumber * TWO_PI * millis() / 60000.0);
      } else {
        thing2(tx, ty, cellsize, circleNumber * TWO_PI * millis() / 60000.0);
      }
...
void thing2(float x, float y, float size, float angle) {
  ...
  // calculate endpoint of the line
  float tempX = x + (size / 2) * cos(angle); // switchy-
  float tempY = y + (size / 2) * sin(angle); // switch
