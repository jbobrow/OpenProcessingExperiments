...
void draw_() {
  ...
  if (t<t1){
    ...
    if(t<p1){
       ...
       color col=lerpColor(#000000,lerpColor(#00B9FF,#FA00FF,.5),map(t,t1,p1,0,1));
       fill(col);
      ...
      for(float i=-n;i<=n;i++){
        for(float j=-n;j<=n;j++){
          for(float k=-n;k<=n;k++){
            ...
            float mul=.4;
            strokeWeight(map(sin(map(t,p1,p2,0,PI/2.)),0,1,100,8));
            color col=lerpColor(#00B9FF,#FA00FF,map(i*j*k,-pow(n,3),pow(n,3),0,1));
            stroke(col);
            pushMatrix();
      ...
      for(float i=-n;i<=n;i++){
        for(float j=-n;j<=n;j++){
          for(float k=-n;k<=n;k++){
            ...
            float mul=.4;
            strokeWeight(map(cos(map(t,p4,p5,0,PI/2.)),1,0,8,100));
            color col=lerpColor(#00B9FF,#FA00FF,map(i*j*k,-pow(n,3),pow(n,3),0,1));
            stroke(col);
            pushMatrix();
       ...
       noStroke();
       //fill(lerpColor(#00B9FF,#FA00FF,.5));
       fill(lerpColor(#00B9FF,#FA00FF,map(cos(map(t,p5,1,0,6.*PI)),1,-1,0,1)));
