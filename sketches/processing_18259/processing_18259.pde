...
void setup()
{
  ...
  titleFont = createFont("Verdana", 21);
  textFont(plotFont); 
    size(1000, 600);
...
void draw()
{
  ...
  for(int a = 0; a < 10; a+=1)
  {
    ...
    if(get(mouseX, mouseY)==(#222222 + 20*a))
    {
      ...
      image(img, 300, 100, 300, ((300.0/img.width)*img.height) );
      if(blurb != null)
      image(blurb, 550, 150, 400, ((400.0/blurb.width)*blurb.height) );
      //image(title, 50, 50, 600, ((600.0/title.width)*title.height));
        textFont(plotFont2);
        fill(#5555BB);
        text(speakers[a], 700 ,100);
        fill(#8888FF);
        text(institutions[a], 700, 130 );
        fill(255);
        textFont(titleFont);
        text("National Symposium for the Advancement of Women in Science: Speaker Biographies", 50, 50);
