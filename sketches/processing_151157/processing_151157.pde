...
void setup() {
  ...
  for (int i=0; i<num; i++) {
    x[i] = gap * (i + 1);      // y is constant for each so can be calculated once
...
void draw() {
  ...
  for (int i=0; i<num; i++) 
  {
    ...
    y[i] = width/2 + sin(radians(frameCount*speed[i] ) + phase[i])* 200;
    //x[i] = width/2 + sin(radians(frameCount*speed[i] ) + phase[i])* random(200);
    //y[i] = width/2 + sin(radians(frameCount*speed[i] ) + phase[i])* random(200);
