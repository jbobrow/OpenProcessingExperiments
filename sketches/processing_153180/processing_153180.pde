...
// arrays for cols
color[] colarray = {color(10), color(10), color(10), color(10), color(10)}; // colour
color[] comparray = {color(100), color(100), color(100), color(100), color(100)};  // complimentary colour
...
void draw() {
  ...
  endShape(CLOSE);
    ...
    // draw rectangles, and inner ones
  for (int i=0; i<=4; i++) {
  fill(colarray[i]);
  rect(i * 80 + 200, 720, 80, 80);
    fill(comparray[i]);
  rect(i * 80 + 220, 740, 40, 40);
  ...
  text("saturation is " + satValue, 5,height-100);
...
// trying this
  // compvalue = calculateCompHue(float hueValue);
/*
 * colorHandleUpdate
 *
...
void colorHandleUpdate() {
  ...
  if (isLocked) {
    ...
    // change float variable from dist to distance
    // meant it works in javascript
...
void keyPressed() {
  ...
  //if ( keyPressed ) { thanks to Jackie Palman
...
void mouseReleased() {
  isLocked = false;
   // set arrays of colours
   colarray [colcount] = color(hueValue, satValue, brightValue);   
   comparray[colcount] = color(complementryHue, satValue, brightValue);
   colcount += 1;
  colcount = colcount % 5;
