...
//variable count
//if varCount is 5000, then start or stop or restart; can maybe recall setup
//change color
//maybe add additional images
...
void setup() {
  ...
  background(255);
  smooth();
  img = loadImage("Untitled-1.gif"); //load the image
...
void draw() {
  for (int i=0; i<num; i++) { //returns the size of the array
    ...
    //if (frameCount%10 == 1){//if framecount % 100, then change color; or write a function that initializes the sketch and then reinitalize it
...
class Scanner { //setting up the class; template for the object
  ...
  void scan() { //this is a method of the object, the scanner
    ...
    if (img.get(int(x), int(y)) != color(2)) { //if scanner is on a white pixel
