 ...
 *
 * modified 07/06/14 by D Anderson
 * similar to w3_04_01 except colours progress slowly
 */

int tops=3;      // number of tops
...
//    gn[i] = random(0,255);
//    bl[i] = random(0,255);
    rd[i] = rd[i] < 255.0 ? rd[i]+1.0 : 0.0;
    gn[i] = gn[i] < 255.0 ? gn[i]+1.0 : 0.0;
    bl[i] = bl[i] < 255.0 ? bl[i]+1.0 : 0.0;
