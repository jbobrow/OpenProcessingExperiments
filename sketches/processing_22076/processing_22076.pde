...
void setup() {
  size(600, 800); //size should be multiple of img width and height
  smooth();
...
void draw() {
  ...
  float mouseXFactor = map(mouseX, 30,width, 0.05,1);
  float mouseYFactor = map(mouseY, 20,height, 0.05,1);
  ...
  for (int gridX = 0; gridX < img.width; gridX++) {
    for (int gridY = 0; gridY < img.height; gridY++) {
      ...
      case 4:
        ...
        stroke(40);
        float w4 = map(greyscale, 0,300, 10,0);
