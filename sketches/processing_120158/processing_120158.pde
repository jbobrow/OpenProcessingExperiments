...
AnimatedSpriteObject puma;
AnimatedSpriteObject fireball;
 
void setup()
{
  size(500,500);
   
 puma = new AnimatedSpriteObject("puma.png");
 puma.x = width/2;
  puma.y = height/2;
  puma.frameWidth = 600;
  puma.frameHeight = 270;
  puma.startFrame = 1;
  puma.endFrame = 5;
 puma.frameSpeed = 6.0; // 6fps
   
  fireball =  new AnimatedSpriteObject("fireball.png");
  fireball.x = 400;
  fireball.y = 400;
 fireball.frameSpeed = 30.0; // 30fps
  fireball.frameWidth = 64;
  fireball.frameHeight = 64;
  fireball.startFrame = 0; 
  fireball.endFrame = 24;
  fireball.scaleX = 2.0;
  fireball.scaleY = 5.0;
}
...
void keyReleased()
{
  ...
  if (key == CODED)
  {
    if (keyCode == LEFT)
    {
     puma.startFrame = 7;
      puma.endFrame = 11;
     puma.currentFrame = puma.startFrame;
    }
    if (keyCode == RIGHT)
    {
      // or use the helper function that does the same thing.
      // start, end, looping
     puma.setAnimation(19, 23, true);
    }
    if (keyCode == UP)
    {
      puma.setAnimation(13, 17, true);
    }
    if (keyCode == DOWN)
    {
      puma.startFrame = 1;
