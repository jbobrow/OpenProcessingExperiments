...
void draw() {
  ...
  int counter = 0;   
   ...
   while (radius < radiusMax) {
    ...
    float size =  cellSize * abs(log(1+atan(angle))) *  abs(sin(frameCount/20 * HALF_PI)); 
    movingCircle(x, y, size, counter,radius); 
    
    float x = mx + radius * sin(angle-PI/2);
    float y = my + radius * cos(angle+PI/2);
    float size =  cellSize * abs(log(0+atan(angle))) *  abs(cos(frameCount/20 * HALF_PI)); 
    movingCircle(x, y, size, counter,radius); 
    
    float x = mx + radius * sin(angle-PI);
    float y = my + radius * cos(angle+PI);
    float size =  cellSize * abs(log(0+atan(angle))) *  abs(sin(frameCount/20 * HALF_PI)); 
    movingCircle(x, y, size, counter, radius); 
    
    float x = mx + radius * sin(angle-3*PI/2);
    float y = my + radius * cos(angle+3*PI/2);
    float size =  cellSize * abs(log(1+atan(angle))) *  abs(cos(frameCount/20 * HALF_PI)); 
    movingCircle(x, y, size, counter, radius); 
    angle += angleStep; 
    radius += radiusStep;
    cellSize += sizeStep;
...
float h=map(radius*size,0,width*25,0,360)
  ...
  fill(h, map(size,0,1,150,255), map(size,0,20,50,255),map(size,0,20,255,0));
  noStroke();
