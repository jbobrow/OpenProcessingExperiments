...
class Tumbler {
  ...
  void display() {
    ...
    float shakeamount = 0;
    if(!lockedinplace && y < sweetspot + (TUMBLER_SWEETSPOT_BUFFER - (DIFFICULTY * DFACTOR)) ) {
      float distance = abs((sweetspot - y));
      shakeamount = map(distance, 0, (TUMBLER_SWEETSPOT_BUFFER - (DIFFICULTY * DFACTOR)) * 2, 5, 0);
    ...
    else if(y > sweetspot - TUMBLER_SWEETSPOT_BUFFER - (DIFFICULTY * DFACTOR)) {
      checkSweetspot();
    }
  ...
  void checkSweetspot(){
    if(y < sweetspot + TUMBLER_SWEETSPOT_BUFFER - (DIFFICULTY * DFACTOR) &&
       y > sweetspot - TUMBLER_SWEETSPOT_BUFFER - (DIFFICULTY * DFACTOR) ) {
      lockedinplace = true; 
    }
    resetting = true;
    setMoving(false);
