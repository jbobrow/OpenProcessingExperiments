...
void draw() {
  ...
  for (int i=0; i<num; i++) {
    ...
    case 3:
      ...
      col=defineColor(xArray[i], yVirt);
      break;
    }
    //noStroke();
    fill(col, 150);
    rect(0, yArray[i]-ballDiameter/4, width,ballDiameter/2);
    rect(xArray[i]-ballDiameter/4, 0, ballDiameter/2, height);
    fill(col);
    stroke(255);
    strokeWeight(2);
