...
//int ENEMYMAX=50;
Player P;
Projectile[] projectiles=new Projectile[PROJECTILEMAX];
  ...
  case 1:
    if (key == CODED) {
      ...
      case 'd':
        ...
        break;
      case ' ':
        enemies.add(new Enemy(1, width/2, height/2));
        break;
      }
    }
...
class Enemy { //I should extend this in the future 
  int state; //speed and damage are state dependent
  float x, y, vX, vY;
  boolean visible;
  ...
  Enemy(int newState, float initX, float initY) {
    state=newState;
    x=initX; 
    y=initY;
    visible=true;
  ...
  void update(float pX, float pY) {
    ...
    case 4:
      speed=4;
      float theta=get_angle(x, y, pX, pY);
      vX=get_movement_x(theta, speed);
      vY=get_movement_y(theta, speed);
      break;
    }
