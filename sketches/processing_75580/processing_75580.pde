...
void makecreature(int localX, int localY, int localZ, int jumpspeed, int localalp, float localrot, float localsize, int localseed, int biting, int walking, int jumping, int side, boolean left) {
  pushMatrix();
    ...
    if (side == 0) {
      ...
      if (biting > 0) {
        makebite(localX,localY,localalp,biting);
      }
    }
    else {
      //Show back
...
void makebite(int localX, int localY, int localalp, int biting) {
  stroke(0,localalp);
  //black mouth
  fill(0,localalp);
  //Adjust as mouth moves
  rect(localX-256,localY,8,256-128*smoothtrans(float(biting)/bitetime));
  //white teeth
  fill(255,localalp);
  pushMatrix();
    translate(0,-64*smoothtrans(float(biting)/bitetime));
    triangle(localX-288,localY+128,localX-256,localY+128,localX-288,localY+64);
    translate(0,128*smoothtrans(float(biting)/bitetime));
    triangle(localX-288,localY-128,localX-256,localY-128,localX-288,localY-64);
  popMatrix();
}
