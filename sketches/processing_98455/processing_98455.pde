...
// Proposta: Planta ou fachada com texturas / Entrega:07/05/2013
// Praça
...
int jstart=0;
int jend =400;
for (int g=7; g<400; g=g+7){ 
  stroke (0,255,0);
  strokeWeight (1/3);
line (g, jstart,g, jend);
line (jstart,g,jend,g);

noStroke();
smooth ();
fill (35,35,142); //cor do lago (Azul-Marinho)
ellipse (200,100, 148,148); //parte superior do lago
ellipse (200,300,150,150); //parte inferior do lago
ellipse (200,200,100,100); // meio do lago

fill (#107927); //cor do acabamento da grama, sobre o lago
ellipse (155,200,70,70); //acabamento da direita
ellipse (245, 200, 70,70); //acabamento da esquerda
...
fill (235,199,158); // espaço de convivência, cor Bronze Claro
ellipse (70,200,120,120); // espaço de convivência da direita
ellipse (330,200,120,120); //espaço de convivência da esquerda

//Vegetação do lado direito, da área de convivência
stroke (0);
strokeWeight (1);
fill (35,142,35); // árvore Verde Floresta
ellipse (20,230,20,20);
fill (0,255,127); //árvore Verde Primavera (opaco)
...
ellipse (20,20,20,20); //arredondamento da curva 1
rect (20,10,350,15); // para direita
ellipse (370,20,20,20); // arredondamento da curva 2
...
// condições da textura em linhas, imitando tábuas de madeira
int ystart=180;
int yend =220;
...
smooth ();
noStroke();
fill (168,168,168); //cor do corpo do pato (cinza)
...
fill (255,165,0); //cor do bico do pato (laranja)
...
PFont fonte;
  ...
  //pessoa atravessando a ponte
  
  int p=180;
  for (p=0; p<350; p=p+10);
  fill (0);
  ellipse (p,200,5,5);
  p++;
  }
