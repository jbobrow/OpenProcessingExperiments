...
class IssueSpot {
  ...
  IssueSpot(int _id, int[] _updateTimeHistory, String[] _statusHistory) {
    issueId = _id;
    x = xInCircle(cx, r);
    y = yInCircle(x, cx, cy, r);
    statusUpdateHistory = _updateTimeHistory;
    statusNameHistory = _statusHistory;
  }
  
  float xInCircle(float _centerX, float _radius){
    return random(_centerX - _radius, _centerX + _radius);
  }
  
  float yInCircle(float _x, float _centerX, float _centerY, float _radius){
    return _centerY + oneOrMinusOne() * random(root(pow(_radius,2) - pow(_x - _centerX, 2))); 
  }
