...
void setup() {
    size(600, 440, P3D); // Canvas size (width = 600, height = 600)
    frameRate(8);   // Frames per second (?)
}
  
void draw() {
    background(59);    // Cover with white background
    lights();          // Turn on ambient lights
    ...
    void display() {
        ...
        for (int pos_x = 0; pos_x < w; pos_x++) {
            for (int pos_y = 0; pos_y < h; pos_y++) {
                ...
                pushMatrix();
                translate(x + pos_x * l, y + pos_y * l, 0);
                box(l);
                popMatrix();
            }
        }
        ...
        /*
        translate(0, 0, 0); // Set back to (0,0) for drawing all canvas
