...
//  R.A. Robertson 2011.10 "Holy Smoke"  ~ www.rariora.org ~
...
Ball[] myBall;
int ballAmount = 16;  // Line: n > 1; Curve: n > 3; Bez: ((n > 0) * 3) + 1 
    ...
    for (int j = 0; j < i; j++) {  // Display proximity indicators.
      if (i != j) {
        ...
        if (distBalls <= distance) {  // Use this block for proximal lines.
          lineWeight = 70/distBalls;
          stroke(lineWeight * 50, lineWeight, lineWeight);
...
/*
Notes:
Application of "Particle Chain" study.
 */             
