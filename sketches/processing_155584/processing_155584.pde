...
class Bubble {
  ...
  void reproduceBubble(Bubble b) {
    ...
    if (d<pfRadius && d<b.pfRadius && pfRadius>b.pfRadius && abs(piCharge)==abs(b.piCharge) ) { // bubles are reproducing
      if (millis() - pfDateOfBirth>pfMaturityAge) {
        //if (millis() - pfDateOfBirth > pfMaturityAge && pfRadius>pfMaturityRadius) {
        ...
        if (rR>pfBirthRadius) {
          ...
          if (iFreNum!=-1) {
            ...
            boolean alive=true;
            float mouthRadius=pfMouthRadius * random(0.99, 1.01);
            float birthRadius=pfBirthRadius * random(0.99, 1.01);
            float maturityRadius= pfMaturityRadius * random(0.99, 1.01);
            float maturityAge= pfMaturityAge * random(0.99, 1.01); // maturity time in minutes
            float sexRadius=pfSexRadius * random(0.99, 1.01);
            float fLifExp = pfLifExp * random(0.99, 1.01);

            //Bubble newBubble = new Bubble(iFreNum, x, y, sX, sY, radius, bodyColor, charge, clicked, clickedX, clickedY, alive, mouthRadius, birthRadius, maturityRadius, maturityAge, sexRadius, fLifExp);
            ...
            bubble[iFreNum].pfLifExp = fLifExp;
