...
float outerR2 = outerR * 1.2; // limit of the handle's "pull" range
...
void draw() {
  ...
  for (int i=0; i<=20; i++) {
    float angle = radians(18*i-90); // 10 x 36 degree steps

    //   Hue   Sat  Brightness
    fill(18*i, 100, 100);  // change the colour as we're building the quads
  ...
  // call dotline function defined below
...
void colorHandleUpdate() {
  ...
  if (isLocked) {
    ...
    float radius = constrain(distance, outerR-200, outerR2-30); 
    ...
    brightValue = map(radius, outerR-100, outerR2-30, 0, 100);
...
void keyReleased() {
  ...
  if (keyCode == UP && saturation <= 100) {
    saturation += 10;
  }

  // down arrow -- decrease saturation
  if ( keyCode == DOWN && saturation >=1) {
    saturation -= 10;
  }
