...
//the color of the ball is controlled by this variable
//note the color is initially black
color colorUpdate = (0);
boolean stopColorUpdate = false;
...
//This function will update the color randomly if there is a colission
void updateColor() {
    if (objectX > width - objectWidth || objectX == 0) {
        colorUpdate = color((int) random(156,256), (int) random(156,256), (int) random(156,256));
    }
    else if (objectY > height - objectHeight || objectY == 0) {
        colorUpdate = color((int) random(156,256), (int) random(156,256), (int) random(156,256));
    }
    else{
...
void stopUpdateColor() {
    if (speedY < .01 && speedY > -.01 && speedX < .01 && speedX > -.01) {
        stopColorUpdate = true;
    }
}
