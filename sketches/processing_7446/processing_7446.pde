	...
	public static void main(String[] args) {
		PApplet.main(new String[] { "--present", "GridSketch" });
	}
	...
	void update() {
		...
		for (int x = 0; x < gridWidth; x++) {
			for (int y = 0; y < gridHeight; y++) {
				...
				VectorType vector = scale(node.getPosition(), new VectorType((float) mouseX, (float) mouseY),
						0.2f);
				vector = zoom(node.getPosition(), new VectorType((float) mouseX, (float) mouseY),0.985f,
						1.2f);
				node.warp = vector;
			}
		}
	...
	private VectorType zoom(VectorType position, VectorType center, float logFactor, float zoom){
