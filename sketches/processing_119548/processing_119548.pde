...
void setup()
{
  ...
  tingle = new SpriteObject("tingle.png");

  tingle.w = playerSize * 4;
  tingle.h = playerSize * 5;
  //tingle.s = 0.05;
}
  ...
  if (collisionImage.width > 0)
  {
    ...
    for (int y = 0; y < collisionImage.height; y += 1)
    {
      for (int x = 0; x < collisionImage.width ; x += 1)
      {
        ...
        tingle.x = nextX;
        tingle.y = playerY;
        if (red(pixelColor) < 128 && tingle.isOverlappingRect(platformX, platformY, 10, 10) == true)
        {
          // moving left and character is currently on the right side of the wall
          if (playerVelocityX < 0 && playerX - (tingle.w * 0.5) >= platformX + 10)
          {
            playerVelocityX = 0;
          }
          // moving right and character is currently on the left side of the wall
          if (playerVelocityX > 0 && playerX + (tingle.w * 0.5) < platformX)
          {
            playerVelocityX = 0;
          }
        }
        ...
        {
          fill(255, 0, 0);
           if (playerVelocityY > 0 && playerY - (tingle.h * 0.5)  < platformY)
          {
            playerVelocityY = 0;
            tempOnGround = true;
          }
