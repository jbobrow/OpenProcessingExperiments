...
PImage bg;               // バックグラウンド
PImage[] textures;       // テクスチャ
final int MAX_H = 1000;  // 輝度の最大値
...
class AuroraEffect implements Scene {
  ...
  Scene update() {
    ...
    // 二重のオーロラを描画
    renderAurora(noiseMap1, time % textures.length, -300);
    loadPixels();
    for(int i = 0; i < pixels.length; i++) {
    ...
    updatePixels();
    background(0xFF000000);
    renderAurora(noiseMap2, (time + textures.length * 9 / 10) % textures.length, 0);
    ...
    glow.resize(width/ 10, height / 10);
    glow.filter(BLUR);
  ...
  private void renderAurora(float[][] noiseMap, int texNum, float zoffset) {
...
// テクスチャを一気に生成
void createTextures() {
  if(textures == null) textures = new PImage[MAX_H];
  colorMode(HSB, MAX_H, 100, 100, 100);
  for(int n = 0; n < textures.length; n++) {
    textures[n] = createImage(360, 100, RGB);
  
    for(int i = 0; i < textures[n].height; i++) {
      float tmp = abs(i - 0.5 * textures[n].height) / (0.5 * textures[n].height);
      float alphaRate2 = 1.0 - tmp * tmp;
      
      for(int j = 0; j < textures[n].width; j++) {
        float alphaRate1 = 1.0 - abs(j - 0.5 * textures[n].width ) / (0.5 * textures[n].width);
        int c = color((j + n) % MAX_H, 40, 60);
        ...
        textures[n].pixels[i * textures[n].width + j] = 0xFF << 24 | r << 16 | g << 8 | b;
      }
    }
  }
}
