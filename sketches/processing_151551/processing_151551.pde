...
void movingCircle2(float x, float y, float size, int offset) {
...
void movingCircle(float x, float y, float size, int offset) {
  ...
  drawShape(x-size/2, y-size/2, circleSize, int(lerp(3, circlePeriod, circleAge)));
}

void drawShape(float px, float py, float size, int sides) {
  float angle = TWO_PI/sides;

  beginShape();
  for (int i =0; i < sides; i++) {
    float x = px + size*cos(angle*i);
    float y = py + size*sin(angle*i);
...
void keyReleased() {
  ...
  if ( keyCode == LEFT && frame_rate_value > 3) {
    frame_rate_value--;
  }
...
void mouseWheel(MouseEvent event) {
  ...
  println(e);
  if (e > 0 && frame_rate_value < 120) {
    frame_rate_value++;
    println("Current frame Rate is: " + frame_rate_value);
  } 
  else if (e < 0 && frame_rate_value > 3) {
    frame_rate_value--;
    println("Current frame Rate is: " + frame_rate_value);
  }
}
...
void mouseClicked() {
  frame_rate_value = int(map(mouseX, 0, width, 3, 120));
  println("Current frame Rate is: " + frame_rate_value);
}
