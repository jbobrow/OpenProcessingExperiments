...
TIME_INCREMENT = 0.4;
OUT_PAUSE = false;
FRAMES = PI * 6 / TIME_INCREMENT;

X_MIN = -20; X_MAX = 20;
Y_MIN = -20; Y_MAX = 20;

color rgb(x, y, t) {
  float d1 = dist(u, v, 50, 50) / WIDTH * 9.5 +.01;             // top-left star
  float d2 = dist(u, v, WIDTH/2, HEIGHT/2) / WIDTH * 9.5 +.01;  // central-following star
  float d3 = dist(50, 50, WIDTH/2, HEIGHT/2) / WIDTH +.01;      // stars fusion

  float pattern = pow(cos(x-t/6) * sin(y-t/2) +1, -1/d2);

  float luma = (pattern) * 127;
  float d = d1*d2*d3 / 3;
  return color(luma/d, luma/d - 125, luma/d - 125);
} 
// End of user formula @@@
