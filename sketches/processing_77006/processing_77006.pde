...
void setup() {
    ...
    noStroke(); 
    setUpNormalization();
    f = createFont("Baskerville", 16, true);
...
void draw() {
   ...
   smooth();
    ...
    float ypos = height - obama * 3.; // 100 points in poll, 400 pixels = height
    float xpos = (index1 + interp) / obamaFL.length * width;
  
   fill(random(255),random(120,255),255);
   rect(xpos-175, 560, 250, 40);

   
textSize(60);
text("♥",xpos, ypos, ypos/2, ypos/2);
...
textSize(60);
text("♥",xpos, ypos, ypos*3, ypos*3);
rect(xpos-10,600, 200, 40);
