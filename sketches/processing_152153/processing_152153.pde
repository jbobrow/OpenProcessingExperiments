...
void draw() {
  ...
  for (int i=0; i<=num; i++) {
    for (int j=0; j<=num; j++) {
      ...
      movingCircleShadow(tx, ty, cellsize, circleNumber);
      movingCircle(tx, ty, cellsize, circleNumber);
    }
  }
} //end of draw 

void movingCircleShadow(float x, float y, float size, int circleNum){
  float finalAngle;
  finalAngle = frameCount + circleNum;

  //the rotating angle for each tempX and tempY postion is affected by frameRate and angle;  
  float tempX = x-20 + (size / 2) * sin(PI / frame_rate_value * finalAngle);
  float tempY = y-20 + (size / 2) * cos(PI / frame_rate_value * finalAngle);
  
  int randomColour = int(random(100,255));
...
void movingCircle(float x, float y, float size, int circleNum) {
  ...
  //inner ellipse
  noStroke();
  fill(0,100, randomColour);
  ellipse(tempX, tempY, size/5, size/5);
  
  //outer ellipse
  noFill();
  strokeWeight(10);
  stroke(0, 100, randomColour);
  ellipse(tempX, tempY, size, size);
}
