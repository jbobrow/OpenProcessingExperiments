...
void setup() 
{
  ...
  smooth();
  noStroke();
  ret1 = loadShape("ret1.svg");
  ret2= loadShape("ret2.svg");
  cor1 = loadShape("cor1.svg");
  cor2= loadShape("cor2.svg");
  cors= loadShape("cors.svg");
  cort= loadShape("cort.svg");
  corch= loadShape("corch.svg");
  corv= loadShape("corv.svg");
  corca= loadShape("corca.svg");
  correc= loadShape("correc.svg");
  corlef=  loadShape("corlef.svg");
  corques=  loadShape("corques.svg");
  corrigh=  loadShape("corrigh.svg");
  corzig=  loadShape("corzig.svg");
  corx= loadShape("corx.svg");
...
void draw(){
  ...
  recorder.update();
  recorder.draw();
    float pupilX = map(mouseX, 0, width, 198, 250); //left pupil 
    float pupilA = map(mouseX, 0, width, 377, 440);  //right pupil
    float corX = map(mouseX, 0, width, 195, 255); //left pupil 
    float corA = map(mouseX, 0, width, 370, 445);  //right pupil
    float pupilY = map(mouseY, 0, height, 210, 265); 
    float corY = map(mouseY, 0, height, 205, 270);
    ellipse(220, 235, 150, 150); //left pupil 
    ellipse(410, 235, 150, 150); //right pupil 
    shape(ret1, pupilX, pupilY, 70, 70); //left pupil 
    shape(ret2, pupilA, pupilY, 70, 70); //right pupil
  ...
  if( result != null)
  {
    ...
    if (result.Name.equals("star")) {
      shape(cors, corX, corY, 70, 70); //left pupil 
      shape(cors, corA, corY, 70, 70); //left pupil 
    } 
    else if(result.Name.equals("circle")){
    shape(cor1, corX, corY, 70, 70); //left pupil 
    shape(cor1, corA, corY, 70, 70); //left pupil 
    }
    else if(result.Name.equals("triangle") || result.Name.equals("delete")){
    shape(cort, corX, corY, 70, 70); //left pupil 
    shape(cort, corA, corY, 70, 70); //left pupil 
    }
    else if(result.Name.equals("check")){
    shape(corch, corX, corY, 70, 70); //left pupil 
    shape(corch, corA, corY, 70, 70); //left pupil 
    }
    else if(result.Name.equals("v")){
    shape(corv, corX, corY, 70, 70); //left pupil 
    shape(corv, corA, corY, 70, 70); //left pupil 
    }
    else if(result.Name.equals("caret")){
    shape(corca, corX, corY, 70, 70); //left pupil 
    shape(corca, corA, corY, 70, 70); //left pupil 
    }  
    else if(result.Name.equals("rectangle")){
    shape(correc, corX, corY, 70, 70); //left pupil 
    shape(correc, corA, corY, 70, 70); //left pupil 
    }  
    else if(result.Name.equals("question") || result.Name.equals("- none - ")){
    shape(corques, corX, corY, 70, 70); //left pupil 
    shape(corques, corA, corY, 70, 70); //left pupil 
    }  
    else if(result.Name.equals("left square bracket") || result.Name.equals("left curly brace")){
    shape(corlef, corX, corY, 70, 70); //left pupil 
    shape(corlef, corA, corY, 70, 70); //left pupil 
    }  
    else if(result.Name.equals("right square bracket") || result.Name.equals("right curly brace")){
    shape(corrigh, corX, corY, 70, 70); //left pupil 
    shape(corrigh, corA, corY, 70, 70); //left pupil 
    }  
    else if(result.Name.equals("pigtail")){
    shape(corzig, corX, corY, 70, 70); //left pupil 
    shape(corzig, corA, corY, 70, 70); //left pupil 
    }  
    else if(result.Name.equals("x")){
    shape(corx, corX, corY, 70, 70); //left pupil 
    shape(corx, corA, corY, 70, 70); //left pupil 
    } 
    else if(result.Name.equals("arrow")){
    shape(cor1, corX, corY, 70, 70); //left pupil 
    shape(cor1, corA, corY, 70, 70); //left pupil 
    }  
  } else{
