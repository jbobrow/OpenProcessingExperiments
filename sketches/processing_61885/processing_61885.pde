...
Vec3D speed      = new Vec3D(random(-100,100),random(-100,100),0);
...
//THE 'OTHER' INSTANCE HAS A GLOABAL VARIABLE CALLED LOC WHICH WE ACCESS BY USING '.'    
  ...
  if(dis>0 && dis<160){//SAYING, IF THE DISTANCE IS GREATER THAN ZERO AND LESS THAN 40 WE DO SOMETHING
...
//THE 'OTHER' INSTANCE HAS A GLOABAL VARIABLE CALLED LOC WHICH WE ACCESS BY USING '.'    
  ...
  if(dis>0 && dis<180){//SAYING, IF THE DISTANCE IS GREATER THAN ZERO AND LESS THAN 180 WE DO SOMETHING
