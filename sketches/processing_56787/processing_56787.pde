...
int w = 30;                              //  ----- another primitive value for the rectangle size. An integer type.
...
void setup() {
  ...
  for (int i = w/2; i < width; i = i + 5) {
    for (int j = w/2; j < height-100; j = j + 2) {
      ...
      G = 80;
      B = n;
      fill(R, G, B,50);                     //  ----- my color is changing for each sequence into the loop, so for each rectangle  
      rotate(TWO_PI * j*1200);
