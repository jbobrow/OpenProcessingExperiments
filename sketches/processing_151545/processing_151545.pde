...
void draw() {
  ...
  for (int i=0; i<num; i++) {
    for (int j=0; j<num; j++) {
      ...
      //      movingCircleTmp(tx, ty, cellsize, circleNumber);
      movingCircle(tx, ty, cellsize, circleNumber);
    }
  }
} //end of draw 

void movingCircleTmp(float x, float y, float size, int circleNum) {
...
void movingCircle(float x, float y, float size, int circleNum) {
  ...
  noStroke();
  fill(255, 0, 0);
  rect(tempX-5, tempY-5, size/5, size/5);
  stroke(255, 0, 0);
  noFill();
  stroke(255, 0, 0);
  line(x-5, y-5, tempX-5, tempY-5);
