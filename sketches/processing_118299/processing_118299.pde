...
File Read Write created 
As Well As reading from Json file
Also loading and image.
...
void draw()
{
  ...
  PImage expendibles;
  expendibles = loadImage("status.jpeg");
  //expendibles.width = 250;
  //expendibles.height = 250;
  image(expendibles,50,450);
  
  int startAt = 425;
  textSize(12);
  
  JSONArray tweets = loadJSONArray("raw.json");
  for(int i =0; i < 10;i++)
  {
    
    JSONObject aTweet = tweets.getJSONObject(i);
    //String date = aTweet.getString("created_at");
    String text = aTweet.getString("text");
    startAt = startAt + 45;
    text(" Tweet: " + text, 200, startAt );
  ...
  if(keyPressed == true)
  {
   ...
   int sec = second(); 
   int milisec = millis();
   String timeStamp = month + "-" + day + "-" + year + "-" + hour + "-" +min + "-" + sec + "-" + milisec;
   String[] list = split(timeStamp, ' ');
   text("Last Updated: " + timeStamp, 500, 220);
 ...
 if (timer.isFinished())
 {
   ...
   text("Last Updated: " + lines[0], 500, 200);
