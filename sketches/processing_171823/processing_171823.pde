      ...
      for(int j=0; j<myCells.length;j++){
        if(myCells[j].isseed == true){
          ...
          d = PVector.dist(myCells[i].location, myCells[j].location);
            //if the activation distance is lower, the velocity
            //of the spreading is hugely reduced
            if(d < 8){
              myCells[i].isseed = true;
              //a variety of strokes is very interesting
              //the line generated is a consequence of the 
              //fact that a cell became a seed. So the attributes
              //of the line must be here
              strokeWeight(random(0.05,3));
