...
class Boid3 {
  ...
  void run(ArrayList<Boid3> Boid3s) {
    flock(Boid3s);
    update();
    borders();
    render();
  ...
  // drei Regeln:
  void flock(ArrayList<Boid3> Boid3s) {
    PVector sep = separate(Boid3s);   // Separation
    PVector ali = align(Boid3s);      // Alignment
    PVector coh = cohesion(Boid3s);   // Cohesion
    // Kräfte
    sep.mult(1.5);
    ali.mult(1.0);
    coh.mult(1.0);
    ...
    applyForce(ali);
    applyForce(coh);
    PVector separateForce = separate(Boid3s);
 ...
 stroke(0);
  ...
  // Separation
  PVector separate (ArrayList<Boid3> Boid3s) {
    float desiredseparation = 25.0f;
    PVector steer = new PVector(0, 0, 0);
    int count = 0;
    // check alle Boid3s auf distanz
    for (Boid3 other : Boid3s) {
      float d = PVector.dist(location, other.location);

      if ((d > 0) && (d < desiredseparation)) {
        // errechne die neue richtung, weg vom Boid3
        PVector diff = PVector.sub(location, other.location);
        diff.normalize();
  ...
  // Alignment
  PVector align (ArrayList<Boid3> Boid3s) {
    float neighbordist = 50;
    PVector sum = new PVector(0, 0);
    int count = 0;
    for (Boid3 other : Boid3s) {
  ...
  PVector cohesion (ArrayList<Boid3> Boid3s) {
    float neighbordist = 50;
