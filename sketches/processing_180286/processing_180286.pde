...
void setup() {  
  ...
  lastConcentration = env.get((int)x,(int)y);
  concentration = env.get((int)x,(int)y);
  gradient = 0;
}
...
void getConcentrationAndGradient() {
    ...
    gradient += (concentration - lastConcentration - gradient)/10.0;
}

void drawGradient () {
  int c = 50 + (int)(255.0*gradient); 
  fill (c,0,c/2);
  rect(0,300,300,600);
}
