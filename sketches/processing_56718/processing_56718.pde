...
class Allies
{
  ...
  void Fighting()
  {
    Allies allies1 = (Allies) PlayerFighter.get(checkingFights);
    for (int a=0; a<OpposingFighter.size()-1; a++)
    {
      Enemies enemy = (Enemies) OpposingFighter.get(a);
      //if (abs(allies1.mouseXpoint-enemy.coordinateX)==1 || abs(allies1.mouseYpoint-enemy.coordinateY)==1)
      if (allies1.startX-enemy.coordinateX==-1)
      {
        println("good");        
        fighting=true;
        costume=leftCostume;
        for (int g=startupAmount; g<PlayerFighter.size();g++)
        {
          Allies allies2 = (Allies) PlayerFighter.get(g);
          if (allies2.assignNumCoordinate==allies1.assignNumCoordinate)
          {
            PlayerFighter.remove(g);
          }
        }
      }
      if (allies1.startX-enemy.coordinateX==1)
      ...
      if (allies1.startY-enemy.coordinateY==-1)
      {
        println("active");
        fighting=true;
