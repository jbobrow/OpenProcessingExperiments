...
void setup() {
  ...
  frame_rate_value = 12; // plus la valeur est grande plus c'est smooth et va lentement
  rectMode(CENTER);
  colorMode(HSB);
  background(0);
}
...
void movingCircle(float x, float y, float size, int offset) {
  ...
  float circleSize = size * 2.5 * sin(circleAge * TWO_PI);

  strokeWeight(1);
  stroke(255, lerp(255, 0, circleAge));
  fill(random(0, 255), random(0, 255), random(0, 255), lerp(255, 0, circleAge));
...
void mousePressed() {
  ...
  if (mouseX >= width/2 && frame_rate_value > 2) { /* ici si la valeur
   est laissée à 120 il n'y
   a rien qui se passe
   puisque la valeur a été 
   définie à 12 (voir plus haut)*/
    frame_rate_value--;
  }
