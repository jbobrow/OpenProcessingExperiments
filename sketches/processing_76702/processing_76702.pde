...
/* OpenProcessing Tweak of *@*http://www.openprocessing.org/sketch/1263*@* */
...
// Philip Sparks, ISTA 301 Assignment
...
void draw()
{
  for (int x = 0; x < width; x++) {
    for (int y = 0; y < height; y++) {
      ...
      set(x,y,color(c[x][y][q],0.5,a[x][y][q]));
    }
  }
  if (p == 0) {
    p = 1; q = 0;
