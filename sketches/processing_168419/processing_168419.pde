...
/* OpenProcessing Tweak of *@*http://www.openprocessing.org/sketch/167142*@* */
...
void setup() {
  ...
  for (int i = 0; i < 1000; i++) {
    flock.addBoid(new Boid(new Vector3D(width/2,height/2),5.0f,1.0f));
  }
  smooth();
}
...
class Boid {
  ...
  void flock(ArrayList boids) {
    ...
    float mod = in.left.get(32 )*10;
    sep.mult(mod +20.0f);
    // float mod = in.left.get(32 )*2000;
    ali.mult(mod*300 + 10.0f);
    coh.mult(165.0f);
  ...
  void update() {
    ...
    acc.setXYZ(.0001, .0001, 0);//random(-.0001,.001));
  }
  ...
  void render() {
    ...
    fill(in.left.get(32) *7000,in.left.get(32) * random(8400),in.left.get(32) * random(4400),10);
    noStroke();
    pushMatrix();
    translate(loc.x, loc.y);
    rotate(theta);
    float mod = in.left.get(32 )*300;
    rect(r, r,  2+(mod),  2+(mod));
