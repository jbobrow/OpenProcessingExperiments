...
void draw() {
  ...
  for (int i=0; i<num; i++) {
    for (int j=0; j<num; j++) {
      ...
      if(circleNumber % 2 == 1 )       // if circlemunber has remainder after dividing by 2
      movingCircle(tx, ty, cellsize, circleNumber * TWO_PI * millis() / 60000.0); // draw circles 
      else                        
      movingCircle2(tx, ty, cellsize, circleNumber * TWO_PI * millis() / 60000.0); // otherwise draw squares
    }
...
void movingCircle2(float x, float y, float size, float angle) { // my function to make squares
