...
/* OpenProcessing Tweak of *@*http://www.openprocessing.org/sketch/150753*@* */
 ...
 *
 *  Deux faisceaux de points en correspondance homographique donnent
 *  des couples de points et donc des droites qui enveloppent les coniques
...
strokeWeight(4);
  ligneplus(frames[0], frames[1], color(255, 255, 0));
  ligneplus(frames[0], frames[2], color(255, 255, 0));
  ligneplus(frames[4], frames[3], color(255, 255, 0));
  ligneplus(frames[3], frames[5], color(255, 255, 0));
  ligneplus(frames[0], frames[4], color(0,0,255));
  ligneplus(frames[1], frames[3], color(0,0,255));
  ligneplus(frames[1], frames[5], color(0,0,255));
  ligneplus(frames[2], frames[4], color(0,0,255));
  ligneplus(palpha, pbeta, color(0, 255, 0));
  strokeWeight(1);
...
class Faisceau {
  ...
  void actualiser() {
    for (int i=1; i<nPoint-1; i++) {
      ...
      float lang=TWO_PI*i/nPoint+temps*0.1;
...
class UnPoint {
  ...
  void actualiser() {
   ...
   PVector Mimage=inter2Lines(frames[3],frames[5],frames[2],ax);
    ligneplus(position,Mimage, color(255,0,0));
}
