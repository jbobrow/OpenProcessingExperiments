...
//ROBB GODSHAw rsgodsha 2012 http://robb.cc
//assignment 6
//Computing for the arts with processing
 
//I used some code from Golan's EMSII course.
//I referenced the bounce tutorial on Processing.org
 
int upp = 0;
float xa = -4, ya = 204, xb = 44, yb = 196, xc = 108, xd = 164, xe = 220;
float yAttitude = 1, xAttitude =1;
float xX = 0, yY=0;
float velX= 3, velY=4;
float  randoRainbo;
...
void wrap() {
  if (frameCount == 1) {
    randoRainbo= random(1, 1.1);
  };
  rainbow(randoRainbo*frameCount%width, 0);//calls the rainbow
  rainbow(randoRainbo*(frameCount%width)-width, 0);
}
...
void bounce() {
  ...
  yY = yY +( velY * yAttitude );
  xX = xX + ( velX * xAttitude );
 
  if (xX > width-rad || xX < rad) {
    xAttitude *= -1;
  }
  if (yY > height-rad || yY < rad) {
    yAttitude *= -1;
  }
 
 
  initialCat(xX, yY);//calls the cat
}
...
void rainbow(float locX, float locY) {//function for rainbow
  translate(locX, locY);
  stroke(239, 0, 0);//RED
  strokeWeight(3*8);
  strokeCap(ROUND);
...
void initialCat(float locX, float locY) {
  translate(locX-width/2, locY-height/2);
