...
void movingCircle(float x, float y, float size, int circleNum) {
  ...
  noStroke();
 float ran_val = random(-20,20);
 float ran_valSQ = random(-20,20);
 float ran_val_SIDE = random(0.25, 0.5);
  fill(random(200));
  rect(tempX + ran_val, tempY + ran_val, size*1.5, size/random(4,6));
  rect(tempX - ran_valSQ, tempY + ran_valSQ, size*ran_val_SIDE, size*ran_val_SIDE);
  stroke(0);
  noFill();
  stroke(0);
...
void keyReleased() {
  ...
  if ( keyCode == LEFT && frame_rate_value > 10) {
    frame_rate_value--;
  }
