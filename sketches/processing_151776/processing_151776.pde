...
void draw() {
 ...
 float posX = map(mouseX,0,width/2,width/2, width);
...
void movingCircle(float x, float y, float size, int offset) {
  ...
  float posY = map(mouseY,0,height/2,height/2, height);
   // right arrow -- increase frame_rate_value
  if (mouseY >= height/2 && circleColour < 250) {
    circleColour++;
  }

  // left arrow -- decrease frame_rate_value
  if (mouseY <= height/2 && circleColour > 0) {
    circleColour--;
  }

  strokeWeight(2);
  stroke(circleColour,0,0, lerp(255, 0, circleAge));
