...
public class Ball {
  ...
  void updatePosition() {
    ...
    if(x<=0 || x>=gameFrameWidth/2-width){
      velX = -velX;
      x = constrain(x, 0, gameFrameWidth/2-width);
    }
   ...
   //COLLISION WITH MORE BRICKS
     backgroundRefreshes=false;
    if (result == 0) {
      for (int i=0; i<morebricks.length; i++){
         if(morebricks[i].imAlive){
           int res = checkCollisionWithRectangle(morebricks[i].rectangle);
           if (res != 0){
             morebricks[i].die();
           backgroundRefreshes = true;
             break;
