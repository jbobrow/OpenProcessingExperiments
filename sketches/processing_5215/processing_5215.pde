...
int nMovers = 1; //from 'Interactive Roots'
...
float velocityScaling = 0.3; //value changed and made constant by me
...
{ 
  ...
  for(int i=0; i<movers.length; i++) 
    movers[i] = new CircleMover(200, 200); //value changed and made constant by me
} 
...
{ 
  ...
  return color((int)random(20,74),(int)random(0,2),(int)random(0,106),alpha);
 // I changed the random colour values above so that the colours would only vary between shades of purple
}
