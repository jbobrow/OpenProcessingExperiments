...
//O trabalho Palacio do Itamaraty Definitivo de Daniel S. GalvÃ£o estÃ¡ licenciado com uma LicenÃ§a Creative Commons - AtribuiÃ§Ã£o 4.0 Internacional.

//Daniel S. GalvÃ£o AU UTFPR S53
...
void setup() {
  ...
  //estrelas
  estrelax = new float[estrelas];
  estrelay = new float[estrelas];
  for (int i=0; i<estrelas; i++)
  {
    estrelax[i] = floor(random(width));
    estrelay[i] = floor(random(height));
  }
}
int lua;
int ceu;
...
void draw() {
  ...
  for (int a=0;a<100;a++)
  {
    point(estrelax[a], estrelay[a]);
    estrelay[a]=estrelay[a]+random(0, 5);
    estrelax[a]=estrelax[a]+random(0, 5 );

    if (estrelay[a]>height)
    {
      estrelax[a] = floor(random(width*1));
      estrelay[a] = floor(random(height));
    }
  }
