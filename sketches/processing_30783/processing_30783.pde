...
//Original at http://www.openprocessing.org/visuals/?visualID=30231
int[][] ice;
...
void setup(){
  ...
  for(int x = 0; x < palette.length; x++) {
    ...
    palette[x] = color(x/3, 15, constrain(x*3, 30, 255));
  }
...
void draw() {
  ...
  for(int x = 0; x < width; x++)
  {
    ice[x][height-1] = int(random(0,190)) ;
  }
  ...
  for (int y = 0; y < height; y++) {
    for(int x = 0; x < width; x++) {
      ...
      ice[x][y] =
          ...
          + ice[calc1[x]][calc2[y]]
          + ice[calc4[x]][calc2[y]]
          + ice[calc1[x]][calc5[y]]) << 5) / 129;
 
      // Output everything to screen using our palette colors
      pixels[counter] = palette[ice[x][y]];
