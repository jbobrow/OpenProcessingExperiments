...
making5,making6,making7,making8,making9,making10,fonsmenu,arc, jaula, primarykey,salamandradreta,sagrafa,cloud1,cloud2,cloud3,sun,personatgefinal,hemfetunjoc,personatgeadal, personatgeabaix, personatgedreta, personatgeesquerra, fonsjocfinal,guerrerJoc3;

Personatge_Laberint hero;
PFont font;
Nube nuvol1, nuvol2, nuvol3;
PersonatgeJoc3 a;
Punt mosca;
String numero, numeroUsuari, numeroauxiliar,numero;
  ...
  void colisio(){
    if((dist(ataca.x,ataca.y,xpos,ypos-50))<dianaJoc1.width/2){
      ...
      if(colisiona){
        ...
        if(recompte==110){
          //          800              10            500
          if(punts >= 800 || pasjoc == 10 && punts >=500){
            pantalla = 7;
            blabla.cue(0);
            blabla.play();
          }
...
void pfinal(){
  ...
  textSize(60);
  textAlign(CENTER);
  text(punts,455-50,100);
  rectMode(CORNER);
  rect(width/2-50,height/4*3-15,100,30);
  textSize(20);
  textAlign(LEFT);
  fill(20);
  text(frases[idiomaa][9],width/2-40,height/4*3+8);
  if(mousePressed && mouseX>width/2-50 && mouseX < width/2+50 && mouseY > height/4*3-15 && mouseY < height/4*3+15){
    setup();
  }
}
void inici(){
...
void introduccio(){
  ...
  stroke(0);
  strokeWeight(2);
  rect(480,160,315,150,20);
  fill(0);
  textSize (25);
  textAlign(LEFT);
  text (frases[idiomaa][5],490,188);
...
void PantallaJocFletxes(){
  ...
  image(fonsJoc1,0,0);
  if(infojoc1 == false){////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    rect(14,80,180,50,50);
    fill(0);
    textAlign(LEFT);
    textSize(18);
    text(frases[idiomaa][7],34,110);
  }
...
void Joc2(){
 ...
 image(fonsJoc2,0,0); 
 if(infojoc2==false){/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   rect(500,75,260,60,50);
   fill(0);
   textAlign(LEFT);
   textSize(18);
   text(frases[idiomaa][8],530,105);
 ...
 if (hero.ypos==254 && hero.xpos==width/2+10){
   ...
   if(colisiona || hero.xpos==width/2+10){
    ...
    if(recompte == 200){
      //        1600             20           1000
      if(punts>=1600 || pasjoc == 20 && punts>=1000){
       pantalla=9; 
      }else{
      pantallaJoc2 = 1;
      hero=new Personatge_Laberint(width/2+10,550,2,2); 
