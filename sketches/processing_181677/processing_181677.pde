...

{
  ...
  if (term[n]== 4)  // " if the nth term of pi is 4"
  {
    stroke(255, 132, 0); // draw in Orange
  }

  if (term[n]== 5) // " if the nth term of pi is 5"
  {
    stroke(0); // draw in Black
  }

  if (term[n]== 6) // " if the nth term of pi is 6"
  {
    stroke(255, 0, 0); // draw in Red
  }

  if (term[n]== 7) // " if the nth term of pi is 7"
  {
    stroke(250, 242, 2); // draw in Yellow
  }

  if (term[n]== 8) // " if the nth term of pi is 8"
  {
    stroke(148, 2, 245); // draw in Purple
  }

  if (term[n]== 9) // " if the nth term of pi is 9"
  {
