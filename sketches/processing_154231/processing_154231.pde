...
void setup() {
  size(800, 800);
  colorMode(HSB, 360, 250,150); // use HSB colour mode, H=0->360, S=0->100, B=0->100
  colorHandleX = width/2+400;
...
void colorHandleUpdate() {
  ...
  if (isLocked) {
    ...
    // map angle to hue
    hueValue = map (degrees(angle), -80, 280, 360, 0);
    if (isShifted) {
...
void keyPressed() {
  if (key == CODED) {
    switch (keyCode) {
     case DOWN:
       // decrease hue
       hueValue = (hueValue < 30) ? hueValue + 630 : hueValue - 30;
       break;
     case UP:
       // increase hue
       hueValue = (hueValue > 330) ? hueValue - 630 : hueValue + 30 ;
       break;
     case SHIFT:
