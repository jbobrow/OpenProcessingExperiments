...
class Slice {
  ...
  Slice(int xiniTemp, int xSizeTemp, PImage imgTemp) {
    ...
    xSize = xSizeTemp;
    foto = imgTemp;
    imgSlice = createImage(xSize,foto.height,ARGB);
    vel = 0;
    xVel = 0;
    ...
    sign = sign/abs(sign);
    seed = random(0,100);
    xPos = 0.5*(width-img.width) + xini + xVel + sign*noise(seed)*foto.width/3;
  ...
  void renueva() {
    ...
    xPos = 0.5*(width-foto.width) + xini + xVel + sign*noise(seed + iter)*foto.width/3;
    vel*=0.95;
    t+=1;
  }
