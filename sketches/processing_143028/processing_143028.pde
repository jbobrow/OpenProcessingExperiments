...
Squasher squasher;
Sprite[] sprites= new Sprite[15]; //object name[] name of array
...
void setup(){
  ...
  for(int i=0; i<sprites.length; i++){
    sprites[i]=new Sprite(random(45,850),random(580,1000)); }
}
...
void draw() {
  ...
  //draw a picnic blanket
  // draw the white of the picnic blanket
  fill(0,0,99);
  rect(0,0,width,height/12);
  
  //draw the red squares of the picnic basket
  for (int i = 4; i < width; i+=25) {
  for (int j = 0; j < height/15; j+=25) {
    fill(0,99,99); //red
    rect(i,j,15,15); //i and j function as x and y, diamtete of 15
  }
}
  ...
  void display(){
    noStroke();
    fill(0,99,99); // make the squasher red
    ellipse(x,y,r*3,r*3); // x and y (which is now mouseX and mouseY) diamter and diamter
  }
