...
int[][][] cell = new int[Ncell_x + 2][Ncell_y + 2][Ccell + 1];  // ç²�å­�ã�¤ã�³ã��ã��ã�¯ã�¹æ ¼ç´�é��å��
int[][] celledge_x = new int[Ncell_x + 2][Ncell_y + 2];  // ã�»ã�«ã��ç«¯ã��ã�©ã��ã��ã�®ã��ã�©ã�°
int[][] celledge_y = new int[Ncell_x + 2][Ncell_y + 2];  // ã�»ã�«ã��ç«¯ã��ã�©ã��ã��ã�®ã��ã�©ã�°
int overflowcount = 1;
/////////////
/// display parameters
int dispX = 720;
...
void displayParticles(){
  ...
  for(int i = 0; i < M; i++){
    ...
    fill(h, 1, 1);
    ellipse((x[i] + bi_x) * sc_x, ( -y[i] + bi_y) * sc_y, s/2, s/2);
    //rect((x[i] + bi_x) * sc_x, ( -y[i] + bi_y) * sc_y, s, s);
  }
}
////////////////////////////////////////////////////////////////////
void displayLines(){
  int nx = 4 * (int)(L_x / h_W);
  int ny = 4 * (int)(L_y / h_W);
  strokeWeight(1);
  stroke(1);
  for(int i = 0; i <= nx; i++){
    float x = i * h_W * sc_x;
...
int findCell_x(int i){
  ...
  if(icx < 1){ icx = 1; }                     //  0-measureå�°é�·å¯¾ç­�
  else if(icx > Ncell_x){ icx = Ncell_x; }    //  0-measureå�°é�·å¯¾ç­�
  return icx;
}
int findCell_y(int i){
  int icy = floor( (y[i] + L_y) / dy_cell ) + 1;  
  if(icy < 1){ icy = 1; }                     // 0-measureå�°é�·å¯¾ç­�
  else if(icy > Ncell_y){ icy = Ncell_y; }    // 0-measureå�°é�·å¯¾ç­�
  return icy;
}
////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////
void periodicSettingCells(){ ////å�¨æ��ç��å¢�ç��æ�¡ä»¶
