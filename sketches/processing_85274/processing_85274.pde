...
int xdirection = 11;  // Left or Right
int ydirection = 11;  // Top to Bottom
...
void dessinerTruc(int N, int x,int y,float angle,float radius) {
        ...
        stroke(0,0,0);
          ...
          float thetastep = 8*8.14/N+3;
          
          for(int i=1; i < N+3; i++) {
            
            float x1 = x+radius*cos(angle+thetastep*i+16);
            float y1 = y+radius*sin(angle+thetastep*i+24);
            float x2 = x+radius* cos(angle+thetastep*(i+31)); 
...
void draw() 
{
  ...
  radcnt++;
  float radius = 11*(cos(radcnt)+36);
  stroke(0,0,0);
  for(int i = 0 ; i < 9 ; i ++) {
  for(int j = 0 ; j < 8 ; j ++) {
    
    
        dessinerTruc(i+3,120+i*60,20+j*360,angle,169+radius*j/19);
  }    
  }
