...
// Declare
Circle [ ] circleLots = new Circle [60]; // This array will allow 60 ellipse. 20 is the size of the array.  



void setup() {            // Void because we are not returning anything.
  
  size(900,600);          // Create window 600x600
  smooth();               //Draws all geometry with smooth edges.

  
  
 // Initailize
 for( int i = 0; i < 60; i ++) {                             // Building class. Size of the array. Use this to generate different elements (ellipse). Up to 20 at the one time.
...
void draw() {
  ...
  fill( random(255), random(255), random(255));     // This is generating a random colour for when the mouse was clicked.
   ...
   }                                                // As i had another plan fot mouse clicking. I left this here incase i change my mind. 
  

  // Call Function
  for (int i = 0; i < 60; i ++) {   // if i is less than 20 increase the value
 ...
 if(y > height){
  ...
  void display() {                                // When you click the mouse, the size of the ellipse will change.
    ...
    ellipse(x,y,40,40); // Create an elippse      // Click and you will create an ellipse 40x40
       ...
       rect(x,y,50,50);                        // Let go and you will create a bunch of rect's 50x50
    }
