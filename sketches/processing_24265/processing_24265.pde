...
Creature creatureThree;
Creature creatureFour;

void setup () {
  size (800,800);
  smooth();
  creatureOne=new Creature();
  creatureTwo=new Creature();
 creatureThree=new Creature();
}

void draw(){
background (234,192,199);
  
creatureOne.show();
creatureOne.move(1);//creature will continuously bounce
creatureOne.mouth(150);
creatureTwo.show(); //need to display creature two before you can move it, thats why wasn't working before
creatureTwo.move(3);//creature will bounce faster each time it hits the defined boundary, eventualy disappears
creatureThree.show();
creatureThree.mouth(220);
} 
