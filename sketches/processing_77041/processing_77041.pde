...
void draw() {
    ...
    float random_colorA = random(0,150);
    float random_colorB = random(0,250);
    
    int index1 = (int)time % obamaFL.length;    
    int index2 = (index1 + 1) % obamaFL.length; 
    float interp = time - (int)time;     
    ...
    float ypos = height - obama * 8.; 
    float xpos = (index1 + interp) / obamaFL.length * width;
    

 
    fill(mouseX % 150, mouseY % 250, 255,20);
    stroke(0,100);

    //rectMode(CENTER);
    //bezier(80, 300, xpos,ypos, xpos +20  , ypos+20, 820,300 ); 
   
    bezier(xpos, 350, xpos,ypos, xpos + offset  , ypos + offset, xpos,350); 
   // fill(0, 0, 255);
    stroke(0,0,255,30);
    line(xpos,ypos, xpos,350);
    
    

    float romney = romneyFL[index1] + interp * (romneyFL[index2] - romneyFL[index1]);
    romney = normalizePrez(romney);
    ypos = height - romney * 8.; 
    
    fill(255, mouseX % 250, mouseY % 150,20);
    stroke( 0,100);
    bezier(xpos, 350, xpos,ypos, xpos + offset  , ypos + offset, xpos,350 );
    //bezier(80, 300, xpos,ypos, xpos +20  , ypos+20, 820,300 ); 
    //fill(255, 0, 0);
...
//    rect(0,348,900,4);
    ...
    if (mouseX >= 80 && mouseX<160 && mouseY>= 300 && mouseY <= 400 ) 
    {
         fill(255,10); 
    } else {
         fill(0,80);       
    ...
    noStroke();   
    ellipse(120,350,80,80);
    
    if (mouseX >= 300 && mouseX<380 && mouseY>= 300 && mouseY <= 400 ) 
    {
         fill(255,10); 
    } else {
         fill(0,80);       
    ...
    noStroke();   
    ellipse(320,350,80,80);
    
    if (mouseX >= 470 && mouseX<560 && mouseY>= 300 && mouseY <= 400 ) 
    {
         fill(255,10); 
    } else {
         fill(0,80);       
    ...
    noStroke();   
    ellipse(520,350,80,80);
    
    if (mouseX >= 680 && mouseX<760 && mouseY>= 300 && mouseY <= 400 ) 
    {
         fill(255,10); 
    } else {
         fill(0,80);       
    ...
    textFont(f, 13);
    fill(255,0,0);
    text("271-DAY", 120, 355);
    text("192-DAY", 320, 355);
    text("85-DAY", 520, 355);
    text("43-DAY", 720, 355);
...
float normalizePrez(float _percent) {
  ...
  return (_percent - lowestResult) / spread * 85.0;
}
