...
// convert (x,y) in range 0 ~ (RESOLUTION-1) to (u,v) in 0~1 scale float u = (float)x / (RESOLUTION-1);
...
beginShape(QUAD_STRIP); // try POINTS for (int ix=0; ix<RESOLUTION; ix++) {
  for (int ix=0; ix<RESOLUTION; ix++) {
fill(random(255));
...
void draw() {
  background(0);
  randomSeed(100 * (mouseX));
// design a patch
PVector lBottom = new PVector(mouseX, mouseY); // left bottom PVector mBottom = new PVector(width/2, height); // middle bottom PVector rBottom = new PVector(width-100, height-100); // right bottom PVector lMiddle = new PVector(0, height/2); // left middle
...
PVector mMiddle = new PVector(mouseX, mouseY); 
PVector rMiddle = new PVector(width, height/2); // right middle// middle middle PVector rMiddle = new PVector(width, height/2); // right middle
