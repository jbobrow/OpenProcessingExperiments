  ...
  void collide() {
    ...
    for (int i = 0; i < numDots; i++) {
      if(i != id) {
        ...
        float minDist = others[i].diameter/2 + diameter;//толщина линий
        if (distance < minDist) { 
          ...
          velocityX = -map(cos(angle), -1, 1, -1.5, 1.5);//струи влево-вправо
          velocityY = -map(sin(angle), -1, 1, -11., 1.5);//струи вверх-вниз
