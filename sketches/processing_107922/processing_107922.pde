...
/*********************************************************************
 ...
 Programmer: Jason Barles / Interaction Designer
 
 Date: 14/Aug/2013
 
 Description: This program generates random lines so that the screen
 is filled with colourful patterns. The user can click on the screen
 to add extra lines.

 This code is a modified version of code by Lachlan David Sunderland 
 called Smarter Dumb Agent.
...
void setup() {
  ...
  x[0] = random(width/2);
  y[0] = random(height/2);
  c[0] = color(random(30, 330), random(30, 330), random(30, 330));
...
void draw() {
  for (int i = 0; i < x.length; i++) {
    //update vector
    p[i].x = p[i].x + random(-0.2, 0.2);
    p[i].y = p[i].y + random(-0.2, 0.2);
    // constrain the vector 
    p[i].x = constrain (p[i].x, -1, 1);
    p[i].y = constrain (p[i].y, -1, 1);
