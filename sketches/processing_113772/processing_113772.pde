...
void draw() {
  ...
  for (int i=0;i<n;i++) {
    ...
    noStroke();
    fill(190);
    ellipse(968-100, 18, 15, 15);
    //fill(195);
    ellipse(978-100, 18, 15, 15);
    //fill(205);
    ellipse(962-100, 25, 15, 15);
    //fill(210);
    ellipse(982-100, 25, 15, 15);
    //fill(200);
    ellipse(972-100, 29, 15, 15);
    ...
    if (!(mouseX>= 855 && mouseY>=10 && mouseX<=890 && mouseY<=37)) { //Si mouse posición X
      if (cualjb[i]==0) {
    ...
    if (rate_jbgde[i] > 1) {// rate = 0
      rate_jbgde[i] = 0;
    }
  }
   
  if ((mouseX>= 855 && mouseY>=10 && mouseX<=890 && mouseY<=37)) {
...
void keyPressed() {
  if (key == ' ') {
    save("Yelibins - "+day()+"."+month()+"."+year()+"_"+hour()+"."+minute()+"."+second()+".jpeg");
  }
}
//lluvia
class Gotas {
