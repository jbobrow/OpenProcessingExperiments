...
/* OpenProcessing Tweak of *@*http://www.openprocessing.org/sketch/62787*@* */
...
void draw() {
 ...
 directionalLight(255, 255,255, -1, -1, -0.1); 
  ...
  directionalLight(255, 255,255, -0.3+0.2*sin(millis()*0.002)/10, -0.3+0.1*cos(millis()*0.002)/10, -0.2); 
float ct=-3*cos((millis()%10000)*0.00005*TWO_PI);
  for (int x=5;x<photo.width;x+=10) {
    for (int y=5;y<photo.height;y+=10) { 
      pushMatrix();
      haut=30*sq(sin(0.015*(x-wid)*ct))+30*sq(cos(0.015*(y-hei)*ct));
