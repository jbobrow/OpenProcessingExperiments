...
String playerText = "";
String howToPlay = "http://i589.photobucket.com/albums/ss335/rumblefighter87/022465ad-3a9d-4093-b745-967df661a0fd.png";
...
String gameOverImage = "http://i589.photobucket.com/albums/ss335/rumblefighter87/d83146bd-eeb7-4d51-8884-157b259cc41d.jpg";
String robotUrl = "http://i589.photobucket.com/albums/ss335/rumblefighter87/d87e9ce0-77cc-4158-94fb-f5b165aba245.png";
String shotsUrl = "http://i589.photobucket.com/albums/ss335/rumblefighter87/60f48d0e-0883-44bf-bdf2-19fdf10ca6ef.png";
...
void setup() {
  ...
  robotWally = new dodgePlayer(width/2-60, height-130, 1, loadImage(robotUrl), 3);
  ...
  if ( URF && !startUpScreen ) {
    if ( setRounds == true ) {
      ...
      if ( oneRideRobot || twoRideRobot ) {
        ...
        if ( mousePressed ) {
          image(shots, mouseX-21, mouseY-16);
          if ( two.headShot(mouseX, mouseY) && !twoRideRobot) {
            playerOneWon++;
            roundOver = true;
            cursor();
          } else if ( one.headShot(mouseX, mouseY) && !oneRideRobot) {
            playerTwoWon++;
            roundOver = true;
            cursor();
          }
        } else if ( oneRideRobot && robotHealth <= 0 ) {
          playerTwoWon++;
          roundOver = true;
          cursor();
        } else if ( twoRideRobot && robotHealth <= 0 ) {
          playerOneWon++;
          roundOver = true;
          cursor();
        }
...
void isGameOver() {
  ...
  if ( playerOneWon+playerTwoWon > maxRound-moreWins ) {
...
void ballCollision() {
  ...
  if ( ball.speed < 0 && ball.collision(one.x, one.y) && ONEVSONE) {
    one.out();
    roundOver = true;
    playerTwoWon++;
  }
  if ( ball.speed > 0 && ball.collision(two.x, two.y) && ONEVSONE ) {
    two.out();
    roundOver = true;
    playerOneWon++;
  }

  if ( one.playerType == 3 || two.playerType == 3 ) { // if they are riding robot
    if ( (ball.collisionWally(one.x, one.y) && !playerOneHasBall ) || ( !playerTwoHasBall && ball.collisionWally(two.x, two.y) ) ) {
      println(robotHealth);
      robotHealth -= 10;
    }
  }
...
void resetMatch() {
  ...
  if ( roundOver ) {
    if ( URF ) {
    ...
    } else if ( numRound != 0 && ONEVSONE ) { // randomize player's position starting from round 2
      ...
      ball = new dodgeBall(width/2, height/2, 0, loadImage(dodgeBallUrl));
    } else if ( numRound == 0 && ONEVSONE) { // round 1
      one = new dodgePlayer(20, 165, 3, loadImage(playerOneImage), 1);
      two = new dodgePlayer(545, 165, 3, loadImage(playerTwoImage), 2);
      ball = new dodgeBall(width/2, height/2, 0, loadImage(dodgeBallUrl));
    }
...
void menuButton() {
  fill ( 92, 227, 236 );
  rect(0, 370, 90, 50, 10, 10, 10, 10);
  textSize(15);
  fill(76, 140, 250);
  text(" Main Menu ", 4, 400);
  if ( mouseX > 0 && mouseX < 90 && mouseY > 370 && mouseY < 440 ) {
    fill(255, 255, 0);
    rect(0, 370, 90, 50, 10, 10, 10, 10);
    textSize(15);
    fill(76, 140, 250);
...
void setNumRounds() {
  ...
  textSize(25);
  fill(0);
  text("How Many Rounds? (Click ME!!) ", width/6+15, height/3+30);
...
void checkField() {
  ...
  if ( two.x > 545 ) { // no else if statement because it creates a visual bug.
    two.x = 545;
  }

  if ( ball.x > 545 && ONEVSONE ) {
    ball.speed = 0;
    ball.x -= 130;
  }

  if ( ball.x < 20 && ONEVSONE ) {
    ball.speed = 0;
    ball.x += 130;
  }


  if ( URF && (ball.y > height-100 || (ball.collisionWally(one.x, one.y) && !playerOneHasBall ) || ( !playerTwoHasBall && ball.collisionWally(two.x, two.y) ) ) ) {
    ball.speed = 0;
    if ( rapidFire ) {
      ball.speed = 25;
    }
...
void keyPressed() {
  if ( key == 'j' || key == 'J' ) {
    ...
    if ( robotWally.collisionRobot(one.x, one.y) && !playerOneHasBall && !twoRideRobot ) {
      one = robotWally;
      oneRideRobot = true;
      robotHealth = 1000;
    }
  ...
  if ( key == '1' ) {
    ...
    if ( robotWally.collisionRobot(two.x, two.y) && !playerTwoHasBall && !oneRideRobot ) {
      two = robotWally;
      twoRideRobot = true;
      robotHealth = 1000;
    }
...
void keyReleased() {
  if ( key == 'w' || key == 'W') {
    ...
    two.moveRight = false;
  } else if (key == 'j' && playerOneHasBall && URF) {
    rapidFire = false;
  } else if ( key == '1' && playerTwoHasBall && URF) {
    rapidFire = false;
  }
}
...
class dodgePlayer {
  ...
  void healthBar() {
    ...
    if ( robotHealth < 250 ) {
      fill(255, 0, 0);
    } else if ( robotHealth < 500 ) {
      fill(255, 200, 0);
    } else if ( robotHealth < 750 ) {
      fill(255, 255, 0);
    } else {
      fill(0, 255, 0);
    }
  ...
  void display() {
    move();
    if ( playerType == 3 && (oneRideRobot || twoRideRobot) ) {
      healthBar();
    } 
  ...
  boolean collisionRobot(int h, int v) {
    return ( h > x-imagePlayer.width+110 && h < x + imagePlayer.width-40 && v > y - imagePlayer.height+90 && v < y + imagePlayer.height-75 );
  }
...
class dodgeBall {
  ...
  boolean collisionWally(int h, int v) {
    return ( h > x - imageBall.width-40 && h < x + imageBall.width-30 && v > y - imageBall.height-5 && v < y + imageBall.height-5 );
  }
}
