 ...
 // rotateY(-.5); 
  ...
  for(int x=0; x<w-skip; x+=skip) {
    ...
    for(int y=0; y<h-skip; y+=skip) {
      ...
      PVector v5 = depthToWorld(x,y+skip,rawDepth3);
      PVector v6 = depthToWorld(x+skip,y+skip,rawDepth4);
     ...
     // translate(v.x*factor,v.y*factor,factor-v.z*factor);
      if(rawDepth > 1 && rawDepth < 700 && rawDepth2 > 1 && rawDepth2 < 700 && rawDepth3 > 1 && rawDepth3 < 700 &&rawDepth4 > 1 && rawDepth4 < 700) {
        //fill(250,215,215);
        noFill();
       // beginShape();
        vertex (v.x*factor, v.y*factor, v.z*factor);
        vertex (v2.x*factor, v2.y*factor,v2.z*factor);
        vertex (v6.x*factor, v6.y*factor,v6.z*factor);
        vertex (v5.x*factor, v5.y*factor, v5.z*factor);
        vertex (v.x*factor, v.y*factor, v.z*factor);
       // endShape ();
