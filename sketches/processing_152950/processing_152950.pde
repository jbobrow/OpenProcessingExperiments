...
// arrays to store primary & secondary colors with which to fill each paintbox 
float [][] boxPri;
float [][] boxSec;
float [][] boxTer;
int activeColor = 0;
...
void setup() {
  ...
  colorMode(HSB, 360, 100, 100); // use HSB colour mode, H=0->360, S=0->100, B=0->100
    ...
    //set up the empty paintboxes
    boxPri = new float [boxes][3];
    boxSec = new float [boxes][3];
    boxTer = new float [boxes][3];
...
void draw() {
  ...
  //draw itten's color wheel with a QUAD_STRIP
  noStroke();
  beginShape(QUAD_STRIP);
  for (int i=0; i<=40; i++) {
    float angle = radians(9*i-90); // 40 x 9 degree steps
  ...
  // color handle Position Update
  colorHandleUpdate();
  //draw dotted line from center to colorhandle
  dotLine(width/2, height/2, colorHandleX, colorHandleY, 30); 
  //draw color handle
  stroke(125);
  fill(255);
  ...
  ellipse(colorHandleX, colorHandleY, handleSize/2, handleSize/2 );
  ...
  ellipse(comHandX, comHandY, 20, 20);
  //caclulate tertiary hue
  float terHue = ((hueValue+complementryHue)/3)%360;     

  if (isLocked) {
    // set up for primary paintbox color
    boxPri [activeColor][0] = hueValue;
    boxPri [activeColor][1] = satValue;
    boxPri [activeColor][2] = brightValue;
    // set up for secondary paintbox color
    boxSec [activeColor][0] = complementryHue;
    boxSec [activeColor][1] = satValue;
    boxSec [activeColor][2] = brightValue;
    // set up for tertiary paintbox color
    boxTer [activeColor][0] = (hueValue+complementryHue)/2;
    boxTer [activeColor][1] = satValue;
    boxTer [activeColor][2] = brightValue;
  }
  for (int i=0; i<boxes; i++){
     paintBox(100+(i*100),650,100,100,i);
  }
}

/*
 * paint boxes : Draw three circles and fill primary, secondary, and tertiary colors
 */
 void paintBox(float x, float y, float w, float h, int boxes){
  noStroke();
  ellipseMode(RADIUS);
  //biggest circle = primary color
  fill(boxPri [boxes][0], boxPri [boxes][1], boxPri [boxes][2]); 
  ellipse(x,y,w/2,h/2);
  //medium-sized circle = secondary color
  fill(boxSec [boxes][0], boxSec [boxes][1], boxSec [boxes][2]);  
  ellipse(x,y+h/6,w/3,h/3);
  //smallest circle = tertiary color
  fill(boxTer [boxes][0], boxTer [boxes][1], boxTer [boxes][2]);
  ellipse(x,y+h/3, w/6, h/6);
 }
...
void colorHandleUpdate() {
  ...
  if (isLocked) {
    ...
    //THIS IS WHERE THE COLOR PICKER PARKS NEW VALUES
    // map location to hue
    hueValue = map (degrees(angle), -180, 180, 360, 0); 
    // map distance from outer edge of the wheel to brightness or saturation
    if (keyPressed) {
...
boolean isWithinCircle(float x, float y, float r) {
  float distCenter = dist(mouseX, mouseY, x, y);
  return (distCenter <= r);
}
