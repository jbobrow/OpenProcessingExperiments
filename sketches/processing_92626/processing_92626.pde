...
class Boid {
  ...
  void render() {
    ...
    stroke(int(useColor)*pic.pixels[constrain(int(loc.x)+int(loc.y)*pic.width,0,-1+pic.width*pic.height)],100);
  ...
  // Wraparound
  void borders() {
    if (loc.x < -r) loc0.x=loc.x = pic.width+r;
    if (loc.y < -r) loc0.y=loc.y = pic.height+r;
    if (loc.x > pic.width+r) loc0.x=loc.x = -r;
    if (loc.y > pic.height+r) loc0.y=loc.y = -r;
  }
