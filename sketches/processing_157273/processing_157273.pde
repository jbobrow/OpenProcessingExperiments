...
void touching(int _i){
    for (int j = 0; j < nBalls; j++ ) {
      if ( j != _i ) {
        float d = dist(posX[_i], posY[_i], posX[j], posY[j]);
        if (d < letterSize[j]/2 + letterSize[_i]/2) {
          posX[_i] = posX[_i] - speedX[_i] * 2;
          posY[_i] = posY[_i] - speedY[_i] *2 ;
          speedX[_i] = -speedX[_i];// * 0.4;
          speedY[_i] = -speedY[_i];// * 0.4;

          posX[j] = posX[j] - speedX[j] * 2;
          posY[j] = posY[j] - speedY[j] * 2;
          speedX[j] = -speedX[j];// * 0.4;
          speedY[j] = -speedY[j];// * 0.4;
        }
      }
    }
    
    speedX[_i] -= speedX[_i] * 0.01;
    speedY[_i] -= speedY[_i] * 0.01;
}
