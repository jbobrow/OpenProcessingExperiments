  ...
  if (noObjectClicked==true) {
    for (int i=0; i<giNum; ++i) {
      if (bubble[i].sX()-bubble[i].sRadius()<mouseX && mouseX<bubble[i].sX()+bubble[i].sRadius() && bubble[i].sY()-bubble[i].sRadius()<mouseY && mouseY<bubble[i].sY()+bubble[i].sRadius()) {
        ...
        bubble[i].pvCli.set(bubble[i].pvPos.x-convertToRealX(mouseX), bubble[i].pvPos.y-convertToRealY(mouseY));
...
int getFreeBubbleNumber() {
  ...
  while (j==-1 && i<giNum-1) {
    i++;
    if (bubble[i].pbAlive==false) {
      j=i;
//println(" getFreeBubbleNumber="+j +" giNum="+giNum +" population()="+population() +" sumOfMass()="+sumOfMass() );     
    }
  }
  return j;
}
...
void createBubble(int i) {
  ...
  boolean alive=true;
  float mouthRadius=(1+random(-0.05, 0.05))*radius*0.1;
  float birthRadius=(1+random(-0.05, 0.05))*minRadius;
  float maturityRadius=(1+random(-0.05, 0.05))*(maxRadius+minRadius)/2;
  float maturityAge=(1+random(-0.05, 0.05))*5; // maturity time in minutes
  float sexRadius=(1+random(-0.05, 0.05))*0.8;

  bubble[i]= new Bubble(i, x, y, sX, sY, radius, bodyColor, charge, clicked, clickedX, clickedY, alive, mouthRadius, birthRadius, maturityRadius, maturityAge, sexRadius);
}
...
void reportParameters() {
  println("   PHYSICAL par: frames="+frames, " giNum="+giNum, " maxRadius="+maxRadius, " minRadius="+minRadius, " maxInitialSpeed="+maxInitialSpeed);
...
class Bubble {
  ...
  Bubble(int num_, float x_, float y_, float sX_, float sY_, float radius_, color bodyColor_, int charge_, boolean clicked_, float clickedX_, float clickedY_, boolean alive_, float mouthRadius_, float birthRadius_, float maturityRadius_, float maturityAge_, float sexRadius_ ) {
    piNum=num_; 
    pvPos.x=x_; 
    pvPos.y=y_; 
    pvVel.x=sX_; 
  ...
  void interact(Bubble[] b) {
    if (pbAlive) {
      ...
      // and Human Forces
      if (pbClicked==true) {
        vIncVel.set(-pvVel.x+convertToRealX(mouseX)-convertToRealX(pmouseX), -pvVel.y+convertToRealY(mouseY)-convertToRealY(pmouseY));
      }
  ...
  void feedBubble(Bubble b) {
    if (abs(piCharge)!=abs(b.piCharge) ) {
      ...
      if (pfRadius>b.pfRadius) {
        if (d<b.pfRadius) { // I am eating bubble 'b'
          eat(this, b);
        }
      }
      if (pfRadius<b.pfRadius) {
  ...
  void feedBubble7(Bubble b) {
    if (abs(piCharge)!=abs(b.piCharge) ) {
      float d=distance(b);
      if (pfRadius>b.pfRadius) {
        if (d<b.pfRadius) { // I am eating bubble 'b'
          float maxBite = min(b.pfRadius, pfMouthRadius);
          float bite = sqrt(sq(pfRadius)+sq(maxBite)) - pfRadius;
          pfRadius += bite;
          //println("EATING piNum="+piNum +" pfRadius="+pfRadius +" pfMoutRadius="+pfMouthRadius );
          //b.pfRadius-=bite;
        }
      }
      if (pfRadius<b.pfRadius) {
        if (d<b.pfRadius) { // bubble 'b' is eating me
          float maxBite = min(pfRadius, b.pfMouthRadius);
          float bite = sqrt(sq(pfRadius)-sq(maxBite)) - pfRadius;
          pfRadius+=bite;
          //println("EATED  piNum="+piNum +" pfRadius="+pfRadius +" b.pfMoutRadius="+b.pfMouthRadius );
          //b.pfRadius-=bite;
          if (pfRadius<=b.pfMouthRadius) {
            pbAlive=false;
          }
          if (pfRadius<=0) {
            populationChanged=true;
  ...
  void reproduceBubble(Bubble b) {
    float d=distance(b);
    if (d<pfRadius && d<b.pfRadius && pfRadius>b.pfRadius && abs(piCharge)==abs(b.piCharge) ) { // bubles are reproducing
      if (millis() - pfDateOfBirth>pfMaturityAge*1000) {
        //if (millis() - pfDateOfBirth > pfMaturityAge*1000 && pfRadius>pfMaturityRadius) {
        //println("millis()="+millis() +" pfDateOfBirth="+pfDateOfBirth +" pfMaturityAge="+pfMaturityAge +" pRadius="+pfRadius +" pfMaturityRadius="+pfMaturityRadius );
        ...
        if (rR>pfBirthRadius) {
          ...
          if (iFreNum!=-1) {
            ...
            float x=pvPos.x + cos(alpha) * pfSexRadius;
            float y=pvPos.y + sin(alpha) * pfSexRadius;

            float sX=pvVel.x;
            float sY=pvVel.y;
            float radius=random(0.95, 1.05)*pfBirthRadius;
            color bodyColor=pcBodyColor;
            float r = random(-1, 1);
            int charge=piCharge * r/abs(r);
            boolean clicked=false;
            float clickedX=0;
            float clickedY=0;

            boolean alive=true;
            float mouthRadius=pfMouthRadius * (1+random(-0.01, 0.01));
            float birthRadius=pfBirthRadius * (1+random(-0.01, 0.01));
            float maturityRadius= pfMaturityRadius * (1+random(-0.01, 0.01));
            float maturityAge= pfMaturityAge * (1+random(-0.01, 0.01)); // maturity time in minutes
            float sexRadius=pfSexRadius * (1+random(-0.01, 0.01));

            Bubble newBubble = new Bubble(iFreNum, x, y, sX, sY, radius, bodyColor, charge, clicked, clickedX, clickedY, alive, mouthRadius, birthRadius, maturityRadius, maturityAge, sexRadius);

            //bubble[iFreNum].piNum = giNum-1;
            bubble[iFreNum].pvPos.x = x;
            bubble[iFreNum].pvPos.y = y;
            bubble[iFreNum].pvVel.x = sX;
            bubble[iFreNum].pvVel.y = sY;
            bubble[iFreNum].pfRadius = radius;
            bubble[iFreNum].pcBodyColor = bodyColor;
            bubble[iFreNum].piCharge = charge;
            bubble[iFreNum].pbClicked = false;
            //bubble[iFreNum].pvCli.set(0,0);
            bubble[iFreNum].pbAlive = true;
            bubble[iFreNum].pfMouthRadius = mouthRadius;
            bubble[iFreNum].pfBirthRadius = birthRadius;
            bubble[iFreNum].pfMaturityRadius = maturityRadius;
            bubble[iFreNum].pfMaturityAge = maturityAge;
            bubble[iFreNum].pfSexRadius = sexRadius;
            //println(" bubble[iFreNum].piNum="+bubble[iFreNum].piNum +" iFreNum="+iFreNum +" bubble[iFreNum].pbAlive="+bubble[iFreNum].pbAlive);

            pfRadius=sqrt(sq(pfRadius)-sq(radius));
          }
        }
      }
    }
  ...
  float sRadius() {
    return convertToGraphX(pfRadius);
  }
  float sMouthRadius() {
    return convertToGraphX(pfMouthRadius);
  }
  float sBirthRadius() {
    return convertToGraphX(pfBirthRadius);
  }
  float sMaturityRadius() {
    return convertToGraphX(pfMaturityRadius);
  }
  float sX() {
    return convertToGraphX(pvPos.x);
  }
  float sSX() {
  ...
  float sY() {
    return convertToGraphY(pvPos.y);
  }
  float sSY() {
