...
void movingCircle(float x, float y, float size, int circleNum) {
  ...
  float tempX = x + (size) * sin(PI / frame_rate_value * finalAngle);
  float tempY = y + (size) * cos(PI / frame_rate_value * finalAngle);

  noStroke();
    
     if (tempY<250 && tempY>200){
    
    fill(255,0,0,random(30,180));     }
     else
     {
    fill(255,random(30,180));
     }
