...
void setup() {
  ...
  gameState = false;
  isColliding = false;
  background = loadImage("http://i1371.photobucket.com/albums/ag297/kiet_nguyen4/outer-space-stars_zpsc7c3acea.jpeg?t=1400717616");
  spaceship = loadImage("http://i1371.photobucket.com/albums/ag297/kiet_nguyen4/p-trans_zpse0c71090.gif");
  a = new Asteroid();
  imageMode(CENTER);
  clicked = false;
  actualTime = 0;
...
void draw() {
  ...
  if (gameState == false) {
    ...
    if (clicked) {  
      ...
      int hunds = (int)(timeOnScreen%1000/10);
      fill(255);
...
class Asteroid {
  ...
  public Asteroid() { 
    ...
    if (loc.x >= 0 && loc.x <= width && loc.y >= height) { // going up   300,600
      loc.y = random(height+25000, height+10000000);
    }  
    else if (loc.x >= width && loc.y >= 0 && loc.y <= height) { // going left   600,300
      loc.x = random(width+25000, width+10000000);
    } 
