...
void initialize() {
  ...
  the_others[0] = random_cross_screen_line();
    PVector circle_intersection = the_others[0].circle_intersection(width/2, height/2, 250);
...
void draw() {
  ...
  if (lines_created != NUMBER_OF_OTHERS) {
    the_others[lines_created] = random_line_from_existing(the_others[(int)random(lines_created)]);
    
    //println("Random: " + the_others[lines_created].to_s());
    
    PVector circle_intersection = the_others[lines_created].circle_intersection(width/2, height/2, 250);
    if (circle_intersection != null) {
    ...
    //println("Amended: " + the_others[lines_created].to_s());
    if (distance_between_points(the_others[lines_created].get_start(),the_others[lines_created].get_end()) > 15) {
      the_others[lines_created].draw();
      lines_created++;
    }
  }
