...
public class Ball{
  ...
  public void move(Ball current, float xPos, float yPos){
    x = xPos;
    y = yPos;
    if((y + (bSize/2) >= height || y - (bSize/2) <= 0) && (dirY > 10 || dirY < -10) && bSize == 50) split(current);
    if((x + (bSize/2) >= width || x - (bSize/2) <= 0) && (dirX > 10 || dirX < -10) && bSize == 50) split(current);
  ...
  public Ball combine(Ball current, Ball other){
    ...
    float newSize = current.getWidth() + 5;
    pile.remove(current);
    pile.remove(other);
