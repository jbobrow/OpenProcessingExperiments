...
void setup()
{
  String url = URLforSite("Sheffield");
  ...
  println("Rain in 2012");
  
 
  println("max "+ max(mmrain));
  println("min "+ min(mmrain));
  println("");
  ...
  float[] degCtmax = float(tmax);
  println("Maximum temperature in 2012");
  
  
  println("max "+ max(degCtmax));
  
  println("");

  String[] tmin = subsetValues(data, "yyyy", "2012", "tmin");
  float[] degCtmin = float(tmin);
  println("Minimum temperature in 2012");
  
  
  println("min "+ min(degCtmin));
