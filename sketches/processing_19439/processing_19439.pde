...
void setup() {
  ...
  angerCap = 37;
  storeBugs[0] = new Bug(-width, -height);
}
...
void draw() {
  ...
  if (angry >= 36 && angry < angerCap) { // super saiyin
   // background(colorCap, colorCap - 30, colorCap - colorCap);
    noStroke();
  ...
  }else if (angry >= angerCap) { // start over again
    eyeColor = 0;
   ...
   //show current bugs in array
      for(int i = 0; i < storeBugs.length; i++) {
       storeBugs[i].make();
       storeBugs[i].move();
     }
   }
 ...
 void mousePressed() {
   ...
   if (angry > 0 && angry < angerCap) {
     Bug temp = new Bug(mouseX, mouseY);
     storeBugs = (Bug[]) append(storeBugs, temp);
   }
