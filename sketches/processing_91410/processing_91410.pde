...
void setup() {  
  ...
  SDbackground= loadImage("SDbackground.jpg"); 
 
  scooby = loadImage("scoobydoo_0.png"); 
 
  scoobySnack= loadImage("Scooby_Snacks.png"); 
 
  monster= loadImage("monster1.gif"); 
 
  size(972, 618); // sets the size of the sketch 
...
//  pMoveY=random(-RBSpeed, RBSpeed); // sets the white ellipse speed in the y 
  ...
  rY=random(height-radius+50); // sets the starting position of the red ellipse in the y 
 
  gX=750; 
 
  //gX=random(width-radius); // sets the starting position of the green ellipse in the x 
 
  gY=random(height-radius+50); // sets the starting position of the green ellipse in the y 
...
void draw() { 
  ...
  image(scoobySnack,rX-27,rY-21,100,70); 
...
//  gX+=gMoveX; // these 2 lines move the green ellipse 
 ...
 // fill(0, 255, 0); 
  ...
  image(monster,gX-30,gY-20,300,250); 
...
//} 
  ...
  image(scooby,pX-45,pY-43,200,152); 
