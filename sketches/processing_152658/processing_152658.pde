...
float attachedIncrement;// how much a cell grows when attached to others
float detachedIncrement;// how much a cell shrinks when isolated
...
void initialiseColony() {
  for (int i=0; i < num-1; i++) {
    ...
    ballRadiusStealRatio = 0.05;
    
    attachedIncrement = 0.01;
    detachedIncrement = 0.01;
    
    distIncrement[i] = random(0.01,0.05);
  } 
}
...
void draw() {
 ...
 background(#B2CF3E);
  ...
  for (int i=0; i < num; i++) {
    ...
    // and also slow the circle down if overlaps were detected, and speed it up otherwise
    if (isOverlapping == true) { 
      
      fill(255,255,255,64);  
      ballRadius[i] += attachedIncrement*random(0.5,2); //Circle grows a little if attached
      distIncrement[i] = distIncrement[i] > 0.01 ? distIncrement[i] /2 : 0.01; 
    }
    else {
      fill(255,0); 
      ballRadius[i] -= detachedIncrement; //Circle
      distIncrement[i] += 0.003;
    }
    
    //Randomly break joined circles off, otherwise it gets quite static
    if (isOverlapping == true) { 
      if ((int) random(num) == 0) {
        distIncrement[i] = 1;
      }
   ...
   // ballRadiusStealRatio = map(sin(frameCount/100),-1,1,0.01,0.3);
