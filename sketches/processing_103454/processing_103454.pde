  ...
  if(t==0) {
    ...
    for (int j=0;j<inside.length;j++) {
      for (int k=0;k<inside[0].length;k++) {
        // distance between rectangles and moving point
        float dx2=50+50*k-x;
        float dy2=50+50*j-y;
    ...
    if (y>height) {
      ...
      text("PRESS SPACE TO RESTART GAME", 300, 400);
      text("CLICK TO RESTART LEVEL", 300, 500);
    }
