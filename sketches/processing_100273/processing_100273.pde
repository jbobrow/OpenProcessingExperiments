...
boolean mouseRelease = true; // The ball will always retract to it's starting position, when this boolean statement is set on true
void setup(){
  loop();
  size(450,450);
  background(0);
...
void mousePressed(){ // When mouse is clicked, it moves and does not retracts back to the starting coordinate because it keeps on bouncing until it reaches it's maximum bounce
  mouseRelease = false;
}
