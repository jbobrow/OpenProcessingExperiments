...
void draw()
{
  ...
  //figuras
  rect(x1pos+100,80,size,size);
  if ((xpos < 133)&&(xpos > 0)){
    xspeed = 1; //velocidad de este segmento
    fill(220,0,280,100); //color de la figura en este segmento
  }
  ...
  ellipse(x1pos,mouseY+100,60,60);
  if ((x1pos < 133)&&(x1pos > 0)){
    x1speed = 0.5;
    fill(250,200,150,100);
  }
  if ((x1pos < 200) && (x1pos > 133)){
    x1speed = x1speed +2;
    fill(200,070,050,100);
  }
  if ((xpos > 266)&&(xpos < 400)){
    x1speed = x1speed +1;
    fill(280,90,100,100);
  }
  triangle(x1pos,mouseY,60,y1pos,130,y1pos);
  if ((ypos <=200) && (ypos >=0)) {
    yspeed = 1.5;
    fill(200,0,120,100);
  }
  ...
  if ((y1pos >= 200) && (y1pos <= height)){
    y1speed = 6;
    fill(100,250,050,100);
  }
  rect (x1pos, y3pos, 60,60);
  if ((y3pos<=200) && (y3pos>=0)) {
    y3speed= 1;
    fill(250,120,250,100);
  }
