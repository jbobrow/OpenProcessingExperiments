...
//3/12/13
int line = 200;
float[] lX = new float[line];
float[] lY = new float[line];
float[] lX2 = new float[line];
float[] lY2 = new float[line];
float[] lSpeed = new float[line];
float gravity = .5;
...
void draw () {
  ...
  for (int i = 0; i < line; i++) {
    stroke(51,203, map(lY[i], 0, height, 20, 200));
    line ( 0,0, lX[i], lY[i]);
  ...
  for (int i = 0; i < line; i++) {
    stroke(0,40, map(lY[i], 0, height, 20, 200));
