...
/**
 * <p>319 Scholes Exhibition Map v1</p>
 *
 * <p>Usage:<ul>
...
void draw() {
  ...
  if (selectedParticle!=null) {
    fill(3, 138, 37);
    ellipse(selectedParticle.x, selectedParticle.y, 10, 10);
  }
}
...
void initPhysics() {
  ...
  for(String tableID : tables.keySet()) {
    ...
    VerletParticle2D p=new LabeledParticle(random(width),random(height),"e: "+tableID);
    physics.addParticle(p);
    // add repulsive force to space out clusters
    physics.addBehavior(new AttractionBehavior(p,200,-1));
    ...
    for(String name : tables.get(tableID)) {
      ...
      physics.addBehavior(new AttractionBehavior(n,200,-0.5));
      // connect to cluster centre (table particle)
      physics.addSpring(new VerletSpring2D(n,p,150,0.01));
    }
  }
}
