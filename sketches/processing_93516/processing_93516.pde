...
void draw(){
    // création de variables pour les couleurs
  ...
  // création d'une variable pour les cotés des polygones
  int A;
  A = mouseX;
  
  // création d'une variable pour le rayon du polygone
  int B;
  B = mouseY;
...
float x=20;//mouseX;
float y=40;//mouseY;
float n=200;//3+random(8); // nombre de cotes du polygone
float R=40;//+random(50); // rayon du polygone 
float r=4*R/2; // rayon de l'etoile polygonale
color bleu=color(35,137,203);
color rose=color(95,24,56);
stroke(0,45);
strokeWeight(4/5);
fill(237,162,0,56);
float pisn=PI/n;
...
for(int j=0;j<=n+5;j++){
  {if (j%2==0){
  fill(bleu,56);}//Si c'est pair c'est rose et sinon c'est bleu
  else{
 fill(rose,99);}
