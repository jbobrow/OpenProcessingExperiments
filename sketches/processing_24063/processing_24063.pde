...
void setup() {
  size(600, 600, P3D);  
//  stroke(0);
  texmap = new PImage[141];
  for(int i=0;i<texmap.length;i++){
    texmap[i] = loadImage("AGEGRID_MASK-"+i+".png");
  }  
...
//  smooth();
  ...
  noStroke();
  textureMode(IMAGE);  
  texturedSphere(globeRadius, texmap[frameNum]);
//  if(frameCount%5==0){frameNum+=dir;}
  frameNum+=dir;
  if(frameNum<0){
    dir=1;
    frameNum+=dir;
  }
  if(frameNum>=texmap.length){
    dir=-1;
    frameNum+=dir;
  }
  popMatrix();  
