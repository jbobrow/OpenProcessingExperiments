...
void zeichne() {//------------------------------zeichne das netz und die Punkte
  ...
  for ( int zeile = 1; zeile < num-1; zeile++) {
    for ( int spalte = 1; spalte < num-1; spalte++) {
      ...
      //masche als vertex zeichnen und füllen......................................................................
      stroke(0);
      r=1.8*dist(knoten[zeile][spalte].x,knoten[zeile][spalte].y,knoten[zeile-1][spalte-1].x,knoten[zeile-1][spalte-1].y);//farbe hängt ab von der verzerrung des feldes
      g=1.8*dist(knoten[zeile-1][spalte].x,knoten[zeile-1][spalte].y,knoten[zeile][spalte-1].x,knoten[zeile][spalte-1].y);
      fill(r,g,b);
      beginShape();
      vertex (knoten[zeile][spalte].x,knoten[zeile][spalte].y);
      vertex (knoten[zeile-1][spalte].x,knoten[zeile-1][spalte].y);
      vertex (knoten[zeile-1][spalte-1].x,knoten[zeile-1][spalte-1].y);
      vertex (knoten[zeile][spalte-1].x,knoten[zeile][spalte-1].y);
      endShape();
    }
  }
}
