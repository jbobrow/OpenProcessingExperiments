...
//these two variables decide whether the two player controls are working or the ai is in control
boolean twoplayer = false;
boolean nottwoplayer = true;
void setup()
{
  size(1000,550,P3D);
...
void writing()
{
  //this part of the code is telling it to say the words that stay on the screen
  if (level != 0){
    textSize(25);
    fill(0);
    textAlign(CENTER);
    text("LEVEL " + level,width / 2,30);
    text("C L I C K  T H E  Q  K E Y  A N D  T H E  E  K E Y  T O " ,width / 2,height - 115);
...
//void powerup()
//{
//  int powerupx = random(int(width));
//  int powerupy = random(int(height));
//  float powerupz = random(200,200);
//  pushMatrix();
//  translate(powerupx,powerupy,powerupz);
//  stroke(255,50,50);
//  fill(120,30,30);
//  sphere(30);
//  popMatrix();
...
void aipaddle()
{
  ...
  {
    if (twoup == true && aiy > 150){
      aiy -= aispeed;
    }
    if (twodown == true && aiy < height - 150){
      aiy += aispeed;
    }
    if (twoleft == true && aix > 200){
      aix -= aispeed;
    }
    if (tworight == true && aix < width - 200){
      aix += aispeed;
    }
    if (aispeedincrease == true){
      aispeed += 1.0;
