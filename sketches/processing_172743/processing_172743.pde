...
void draw() {
 background(157,188,252); // resets the background
...
void checkCollide() {
 ...
 if (ballY < 0) {
   ballSpeedY = ballSpeedY * -1; // switch Y direction
     if (paddleWidth == 40){ // if the paddle hits 40
   paddleWidth = paddleWidth + 40; // the paddle grows 40
 ...
 if (ballY > height-paddleHeight) { // past 'goal line'
   if (ballX > mouseX && ballX < mouseX + paddleWidth) { // within paddle, the ball will hit the paddle width
     paddleWidth = paddleWidth - 40; // paddle width is 200 and will subtract 40 each time the ball hits it
      ballSpeedY = ballSpeedY * -1; // switch Y direction
       } else { // end game
