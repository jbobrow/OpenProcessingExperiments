...
// { 
      ...
      boolean circ = true; // it is common to use boolean statements with control statements to determine flow of program see next line as add on
      if (1%2!=0) circ=false; //from boolean statment we now determine that if that figure in brackets are calculated we will not get a circle
...
void movingCircle(float x, float y, float size, float angle, boolean circ) {
  ...
  } else { //this is related to the circ boolean
  rect (x, y, size, size);// gives you rectangles
  }
