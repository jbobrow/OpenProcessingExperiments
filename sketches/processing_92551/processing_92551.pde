...
void draw() {
 ...
 background(0); 
 textSize(12);
 text(disp+" colleges with enrollment greater than "+limit, 140,height-50);
 disp=0;
 text("Reset zoom",450,height-50);
   textSize(9);
 stroke(155,0,255,255);
 strokeWeight(3);
 noFill();
 ellipse (100,height-30,30,30);
 ellipse (150,height-30,30,30);
 ellipse (450,height-30,30,30);
 line(90,height-30,110,height-30);
 line(100,height-40,100,height-20);
 line(140,height-30,160,height-30);
 line(440,height-40,460,height-20);
 line(460,height-40,440,height-20);
 noStroke();
 int num=1;
...
void mousePressed() {
    if (dist(mouseX,mouseY,100,height-30)<15) {limit += 500;}
    if ((dist(mouseX,mouseY,150,height-30)<15) && (limit>0)) {limit -= 500;}
    if (dist(mouseX,mouseY,450,height-30)<15) {    
        lngmint=-175; //-135; //-175;
        lngmaxt=-50; //-60; //-50;
        latmint=5;// 16;//23; // 10;
        latmaxt=75;}//55;}//50;}//80;}
    if (mouseY<height-70) {
        ...
        if (sizer==1) {
         ...
         if (sizer==0) {   
            ...
            latmint=map(y+((mouseX-x)*(2/3)),0,height,latmax,latmin);
