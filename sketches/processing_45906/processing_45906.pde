...
void setup() {
  ...
  player = minim.loadFile("heavydrumwav1.wav");
  player.play();
...
void draw() {
  ...
  {
    float left1 =  bmusic.left.get(k) *5500;//get lift buffer size
 
    float right1 = bmusic.right.get(k) * 5500;//get rightbuffer size
    buffersizeL[0] = left1;
