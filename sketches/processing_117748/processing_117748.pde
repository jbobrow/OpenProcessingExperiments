...
void update(){    //in order to continue to the movement
  if(isseed == false){
 velocity = new PVector (random(-5,5),random(-5,5));  //cell will appear anywhere 5 unit far from its last current location   
 loc.add(velocity);
  
}}}  

cell[] mycells = new cell[3000];  // [x]  x number of cells will be in frame
...
void draw(){
 ...
 stroke(random(150,255),random(0,150),0);  // random color for the lines btw cells
