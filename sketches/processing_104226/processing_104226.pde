...
//FINAL PROJECT:deadline 28 July
//Coursera: Creative programming for Digital Media & Mobile Applications

//Copyright (c) 2013 Mohit Hingorani


MultiSlider ms;
//Maxim maxim;
//AudioPlayer rsound,gsound,bsound,asound; 
void setup()
  {
    ...
    String[] sliderNames = {"red","green","blue","alpha"};
    ms= new MultiSlider(sliderNames.length,0,255,80,310,1000,20,HORIZONTAL);
    ms.setNames(sliderNames);
    /*rsound=maxim.loadFile("pad4.wav");
    rsound.setLooping(true);
    gsound=maxim.loadFile("pad1.wav");
    gsound.setLooping(true);
    bsound=maxim.loadFile("chime.wav");
    bsound.setLooping(true);
    asound=maxim.loadFile("alpha.wav");
...
{ 
 ...
 /*
  rsound.volume(ms.get(0)/50);
  rsound.play();
  
  gsound.volume(ms.get(1)/50);
  gsound.play();

  
  bsound.volume(ms.get(2)/150);
  bsound.play();
  
  asound.volume(ms.get(3)/100);
  asound.play();
  */
