...
public class AspektWorld extends PApplet {
	...
	public ArrayList<Line> lines;
        public int mode = 0;
        public  final static int gridOnFlag = 2;
        public  final static int verticesOnFlag = 4;
        public  final static int edgesOnFlag = 1;
	...
	public void draw() {
		...
		//textMode(SCREEN);
                //fill(200);
                //text(forward_heading.x + "," + forward_heading.y + "," + forward_heading.z,300,300);
		//text(pos.x + "," + pos.y + "," + pos.z,300,400);
	...
	public void dvals() {
		//pos = new PVector(36393,11078,-58216);
		pos = new PVector(7260,2200,-11640);
                forward_heading = new PVector(33,8,-37);
		...
		make_normals();
		camera(pos.x,pos.y,pos.z, forward_heading.x, forward_heading.y, forward_heading.z, 0.0f, 1.0f, 0.0f);
	}
	...
	public void make_grid() {
		float spacing = 200;
		lines = new ArrayList<Line>((int)(2*(wd+dp)/spacing + 1));
		stroke(200,255,255,255);
		for(float i = -wd*2; i <= wd*2; i+=spacing) {
			Line nl = new Line(this, i,25000,-2*wd,i,25000,2*wd);
			lines.add(nl);
		}
		for(float i = -wd*2; i <= wd*2; i+= spacing) {
			Line nl = new Line(this, -2*wd,25000,i,2*wd,25000,i);
			lines.add(nl);
		} 
		stroke(0,0,0,255);
	}
	...
	public void mouseClicked() {
            ...
            switch(mode) {
              ...
              case(gridOnFlag):
                ...
                gridOn = 1;
                break;
              case(edgesOnFlag|verticesOnFlag):
                edgesOn = 1;
                verticesOn = 1;
                gridOn = 0;
                break;
              case(verticesOnFlag|gridOnFlag):
                edgesOn = 0;
                verticesOn = 1;
                gridOn = 1;
                break;
              case(gridOnFlag|edgesOnFlag):
                edgesOn = 1;
                verticesOn = 0;
                gridOn = 1;
                break;
              case(edgesOnFlag|verticesOnFlag|gridOnFlag):
                edgesOn = 1;
                verticesOn = 1;
                gridOn = 1;
                break;
	...
	public void setup() {
		...
		translate(pos.x,pos.y,pos.z);
		draw_grid();
		g1 = new Graph(this, Graph.Types.LAUNCH_ANIMATION_GRAPH);
		g1.draw(pos);
		mouseLookOn = 1;
