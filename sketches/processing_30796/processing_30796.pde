...
Point getTextOutlines(String text) {
  ...
  PGraphics graphics= createGraphics((int) textWidth(text), 
  (int) (g.textSize + textAscent() + textDescent()), JAVA2D);

  graphics.beginDraw();
  graphics.fill(0);
  graphics.background(255);
  graphics.textAlign(LEFT, TOP);
  graphics.textFont(g.textFont);
  graphics.textSize(g.textSize);
  graphics.text(text);
  graphics.endDraw();
...
private void recursiveFun(Rect rect) {
  ...
  int size = min(rect.h, maxh);
  textSize(size);
  int wid = (int)random(rect.w/2, 3*rect.w/4);
  while (textWidth (word) > wid && size > 0) {
    size--;
    textSize(size);
  }
  ...
  if (w>1 && size>1)
    ...
    point = new Point(0, 0);
  int h = (int) (textAscent()+textDescent()+size-point.x-point.y);
