...
float [ ] myears1650 = {9, 9, 2, 9, 10, 3, 3, 8};
...
void setup( )
{
  ...
  x = 0;
  dx = .5;
  c = color (62, 206, 202, 50);
  col = color (62, 206, 202, 90);
  stroke( 255 );
  noFill( );
}
...
void draw( )
{
  ...
  drawCircles (years50, rank50, height*.20 ); 
  drawCircles (years1650, rank1650, height*.50 );
  drawCircles (myears50, mrank50, height*.35 ); 
  drawCircles (myears1650, mrank1650, height*.65 );
...
void drawCircles(float [] y, float [] r, float h)
{
  for( int i = 0; i < y.length; i++)
 {
    if ( x < y[i])
