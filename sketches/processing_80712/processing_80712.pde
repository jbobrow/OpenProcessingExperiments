...
/* OpenProcessing Tweak of *@*http://www.openprocessing.org/sketch/60965*@* */
...
void draw() {
  ...
  if (mousePressed && mouseButton==LEFT) {
    noiseXRange = mouseX/5;
    noiseYRange = mouseY/5;
  }
  ...
  for(int meshY=0; meshY<=tileCount; meshY++) {
    ...
    for(int meshX=0; meshX<=tileCount; meshX++) {
      ...
      float y = map(meshY, 0,tileCount, -height/3,height/3);
