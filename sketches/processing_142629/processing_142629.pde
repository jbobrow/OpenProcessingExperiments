...
createSelectedThumbnailIfNotExists()
}
...
if(vx==0&&vy==0){}//donÃ�ï¿½Ã¯Â¿Â½Ã�ï¿½Ã�Â´t update cells without velocity
else{//only update cells with velocity
int plus=0;
  
float velocityAngle=(atan2(vy,vx)* (180.0f / PI)) ;// angle of cell velocity
float  testAngle = (velocityAngle > 0.0 ? velocityAngle : (360.0 + velocityAngle)); // map angle to 360Ã�ï¿½Ã¯Â¿Â½Ã�ï¿½Ã�Â°
if((testAngle>270)||(testAngle<90)){plus=180;}else plus=0;// test direction, fix angle calculation issue by adding 180Ã�ï¿½Ã¯Â¿Â½Ã�ï¿½Ã�Â° to right direction.
 ...
 velocityAngle = (velocityAngle > 0.0 ? velocityAngle : (360.0 + velocityAngle));// map angle to 360Ã�ï¿½Ã¯Â¿Â½Ã�ï¿½Ã�Â°
...
//     float   speed = sqrt( (vx)*(vx) + (vy)*(vy) );
  ...
  for(int xi = -step; xi < 2*step; xi+=step)  {    for(int yi = -step; yi < 2*step; yi+=step)    {//get all neighbors of each cell
      ...
      float neighborAngle=(atan2(yi,xi) * (180.0f / PI))+plus;// angle of middle cell to neighbor
       neighborAngle = (neighborAngle > 0.0 ? neighborAngle : (360.0 + neighborAngle));// map angle to 360Ã�ï¿½Ã¯Â¿Â½Ã�ï¿½Ã�Â°
...
//Multitouch disable for java-mode
  ...
  HTMLElement selectedThumbnail;

 void createSelectedThumbnailIfNotExists(){//only required on pages without this element 
if(!parent.document.body.getElementsByClassName('visThumbnail50 selected').length>0){  //if the Element is still empty
       selectedThumbnail= document.createElement('img');//create a new viewsElement
 selectedThumbnail.src=externals.canvas.toDataURL("image/jpeg",0.7);// take a screenshot from the sketch and place it in the imgElement
      parent.document.body.insertBefore(selectedThumbnail, parent.document.body.children[0]);//and insert it to the top of the page
     selectedThumbnail.className='visThumbnail50 selected'; //set the class name to make it re-accessible
    selectedThumbnail.style.padding="40px"; //set padding to move the text
 } else{ selectedThumbnail=parent.document.body.getElementsByClassName('visThumbnail50 selected')[0].children[0];// get the right HTMLElement
