...
//Carter Hanson 3-18-14 Ohio State Art 3001

Walker[] w = new Walker [8];  //declaring a walker array

void setup(){
  size(950,700);  //size of sketch
  background(0);  //black background
  frameRate(30);  //speed of animation
...
if(mousePressed){
 background(255);    //if mouse is pressed then background changed to white
 fill(0,102,0);      //color of rectangle
rect(mouseX,mouseY, 100,100);  //rectangle controlled by mouse, release to restart
}
}      //end draw
...
class Walker {      //declare walker class
  ...
  color c = color(100, 50);  //end random, only gets dark if spots an overlap
  ...
  void display() {
    noStroke();
    fill(random(200), 50);      //color of squares random grey scale
    rect(x,y,t,t);              //squares are displayed 
    rect(mouseX,mouseY,s,s); //square with random grey scale that is movable with mouse
 ...
 //intersect and no intersect functions for color
    ...
    fill(c);
    text("oooooo", x, y);   //display small green circles at intersections
  }
