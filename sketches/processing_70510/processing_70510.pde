...
void draw() {
  PImage backgr;
backgr = loadImage("22.jpg");
    image(backgr,-50,-50,500,650);
  
  //background(255);
  //image(backgr,0,0,560,400);
  ...
  float angle  = -(map(mouseX, 0, width, 0, Rotat_number*6.28))+PI; 
  float X_arm1, Y_arm1;
  ...
  //hand rotation
  //float Rotat_number=0.25;  // Number of rotations per mouseX/width
  float angle1 = -(map(mouseX, 0, width, 0, Rotat_number*6.28))+0.5*PI;
  float X_arm, Y_arm;
