...
void dessinerMonstre() {
  ...
  for ( int i = 0 ; i < 700 ; i = i + 1 ) {
   line (xbarb[i], ybarb[i], xbarb[i], ybarb[i]+4);
  ...
  //Narine
  fill(0);
 float cibled = random(3,9);
 float cibleg = random(3,9);
 float vitesse = 0.5;
 
 float distanced = abs(cibled-dnarinedroite);
 float distanceg = abs(cibleg-dnarinegauche);
 float distancetotal = dist( mouseX , mouseY , xnez + 11 , ynez + 6 ) ;
    ellipse(xnez + 11, ynez + 6, dnarinedroite, dnarinedroite);
    ellipse(xnez - 11, ynez + 6, dnarinegauche, dnarinegauche);
if (mousePressed || distancetotal < dnarinedroite  ) {
   mouse2 = true;
}
  if (dnarinedroite < cibled && dnarinegauche < cibleg && mouse2 == true && dnarinedroite < 9 && dnarinegauche < 9){    
   
    dnarinedroite = dnarinedroite + min(distanced, vitesse);
    dnarinegauche = dnarinegauche + min(distanceg, vitesse);
  } else {
        dnarinedroite = dnarinedroite - min(distanced,vitesse);
        dnarinegauche = dnarinegauche - min(distanceg,vitesse);
  }
  ...
  for (int i = 0 ; i < 7 ; i ++) {
    ...
    curveVertex(int(xrandentsbas1 [i]), int(yrandentsbas1 [i]));
    curveVertex(int(xrandentsbas2 [i]), int(yrandentsbas1 [i]));
    curveVertex((xdentsbas + 18), ydentsbas - 31);
    curveVertex((xdentsbas + 10), ydentsbas + 5);
    endShape();
    xdentsbas = xdentsbas + 27;
  }
  ...
  for (int i = 0 ; i < 6 ; i ++){
    ...
    curveVertex(int(xrandentshaut1 [i]), int(yrandentshaut1 [i]));
    curveVertex(int(xrandentshaut2 [i]), int(yrandentshaut1 [i]));
    curveVertex((xdentshaut + 18), ydentshaut - 31);
    curveVertex((xdentshaut + 10), ydentshaut + 5);
    endShape();
    xdentshaut =  xdentshaut + 27;
  }
...
while (taillenomb >= 1) {
  ...
  while (poils2 < 10) {    
    ...
    line (xpn[i], (ypoilsnomb + phauteur), xpn[i], (ypoilsnomb + phauteur + random(2, 4)));
    poils2 = poils2 + 1;
    i++;
  }
