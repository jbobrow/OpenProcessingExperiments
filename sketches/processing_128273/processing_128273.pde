...
void draw() {   
  ...
  for (int i = 0; i < in.bufferSize() - 1; i++)
  {
    alph = random(200, 255);
    stroke(255, alph);
    line(i, 295 + in.left.get(i)*50, i+1, 295 + in.left.get(i+1)*50);
    line(i, 295 + in.right.get(i)*50, i+1, 295 + in.right.get(i+1)*50);

    println("in.left.get(i)"+in.left.get(i));


    if (in.left.get(i)<-0.08) {
      drawImage();
    }
  }
