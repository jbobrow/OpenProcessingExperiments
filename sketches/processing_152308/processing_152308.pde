 ...
 */
 ...
 /*
Ã�Â * From the workshop at Monash University in FutureLearn
Ã�Â * Adjustemets by Pablo Rosero
Ã�Â * www.pablorosero.com
Ã�Â */
...
void movingCircle(float x, float y, float size, float angle) {
  ...
  for (float i=size; i>0; i-=20) {
    float tempX = x + (i/ 2) * sin(angle+i/20);
    float tempY = y + (i/ 2) * cos(angle+i/20);
    fill(255);
    strokeWeight(5);
    stroke(0);
