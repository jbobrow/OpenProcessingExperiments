  ...
  for (int i = 0; i < numberOfObjects; i++) {
   ...
   /* for(int blueValue=0; blueValue<255; blueValue++){
    ...
    if (objectStyles[i] == -1) {
      ...
      bezier(objectXPositions[i], objectYPositions[i],
        objectWidths[i], objectHeights[i], 50,50,50,50);
       fill(255,greenValue,255);
       greenValue++;
       ellipse(objectXPositions[i], objectYPositions[i],
        objectWidths[i], objectHeights[i]);
    }
     stroke(objectStrokes[i]);
