...
void setup( )
{
  ...
  intitColorArray( colorVals ); 
}

void initFloatArray ( float vals[], float min, float max)
{
  for (int i=0; i<vals.length; i++){
    vals[i]=random(min,max);
  }
}

void intitColorArray( color vals[] )
{
  for (int i=0; i<vals.length; i++){
    vals[i]=color(random(255),random(255),random(255));
  }
}
