...
int thickness = 5;
int dragging = 0;
FloatList inkx = new FloatList();
FloatList inky = new FloatList();
  ...
  void drawLayer(){
    ...
    for(int i=2;i<inkx.size();i=i+1){
      if(inkx.get(i)!=-1000 && inkx.get(i-1)!=-1000){
        stroke(inkColor.get(i));
        if(anchor1==activeAnchor&&inkColor.get(i)!=color(250,250,250)){
          stroke(100,0,0);
        }
    ...
    for(int j=1;j<inkx.size();j=j+1){  
      if(inkx.get(j)!=-1000 && inkx.get(j-1)!=-1000){
        stroke(inkColor.get(j));
        if(anchor2==activeAnchor&&inkColor.get(j)!=color(250,250,250)){
          stroke(100,0,0);
        }
  ...
  void addInk(float newX,float newY){
    if(newX!=-1000){
      ...
      if(activeButton.label=="Erase"){
        inkColor.append(color(250,250,250));
      }else{
        inkColor.append(color(50,50,50));
      }
    }else{//Means a new line is being started
      inkx.append(newX);
      inky.append(newY);
      inkColor.append(color(0,0,0));
    }
  }    
}
