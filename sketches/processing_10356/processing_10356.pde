...
void draw() {
  ...
  line(x5,y5,x6,y6);
    ...
    if(key == '3') {
      float sx2 = 4.0;
      float sy2 = 5.0;
      float x4 =  width-850 + sin(angle) * radius;
      float y4 = height-650 + cos(angle) * radius;
  ...
  line(x3,y3,x4,y4);
      ...
      float sx3 = 2.0;
      float sy3 = 6.0;
      float x5 =  width+600 + sin(angle) * radius;
      float y5 = height+700 + cos(angle) * radius;
        ellipse(x5, y5, 1, 1); // Draw smaller circle
      // Set the position of the large circles based on the
      // new position of the small circle
      float x4 = x5 + cos(angle * sx3) * radius-750;
      float y4 = y5 + sin(angle * sy3) * radius-950;
       //line 
  if(hh<100){
    hh+=1;
  } 
