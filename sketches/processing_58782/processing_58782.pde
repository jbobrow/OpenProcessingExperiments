...
// Average
float[][] avemat = { { 1./9., 1./9., 1./9.},
                     { 1./9., 1./9., 1./9.},
                     { 1./9., 1./9., 1./9.} };

// Sharpen
float[][] sharpmat = { { -1., -1., -1.},
                       { -1.,  9., -1.},
                       { -1., -1., -1.} };
...
PImage convolution( PImage img, float[][] mat )
{
  ...
  PImage img3 = createImage(img.width, img.height, ARGB);
  ...
  for (int ir=0; ir < irs; ir++) {
    for (int ic=0; ic < ics; ic++) {
      ...
      img3.pixels[fp] = color(rsum, gsum, bsum);
    }
  }
