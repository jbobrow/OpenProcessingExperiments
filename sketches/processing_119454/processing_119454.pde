...
void draw() {
  ...
  //cuerpo
  rect(Xrect, Yrect, szrect, szrect);
  fill(0, 0, 255);
  ellipse(Xrect, Yrect, szrect*0.3, szrect*0.3);

  //propulsor
  rect(Xrect*0.9, Yrect, szrect*0.5, szrect*0.5);
  ...
  ////////////////////////////////////////////////////////////////////////////////////////
  triangle(Xrect*0.8, Yrect+(szrect/2)/2, Xrect*0.8, Yrect-(szrect/2)/2, fuegoestable, Yrect);
  ...
  for (int i=0; i<numObst; i++) {
    if (dist(Xrect, Yrect, Xobst[i], Yobst[i]) < (szrect/2)+(szObst[i]/2)) {
      ...
      fill(255, 130, 0);
      ellipse (Xrect, Yrect, explbaja, explbaja);
    }
  }
  ...
  text ("Pulsa UP para subir", width*0.4, height*0.1);
  text ("Pulsa DOWN para bajar", width*0.4, height*0.15);
  noCursor();
}
