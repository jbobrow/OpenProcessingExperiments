...
int num = 20; // NEW NEW NEW - made this global as I require it in movingCircle()
...
void movingCircle(float x, float y, float size, int circleNum) {
  ...
  float tempX2 = x + (size / 2) * cos(PI / frame_rate_value * finalAngle); // NEW NEW NEW Swapped sin() and cos()
  float tempY2 = y + (size / 2) * sin(PI / frame_rate_value * finalAngle); // NEW NEW NEW Swapped sin() and cos()

  float diameterX = abs(tempX-tempX2) / abs((num*2)-(circleNum/2)); // NEW NEW NEW 
  float diameterY = abs(tempY-tempY2) / abs((num*2)-(circleNum/2)); // NEW NEW NEW 
  int transparency = 128; // NEW NEW NEW 
