...
void makeWax(){
  ...
  int numWarmBlobs = 1 + int(width/10.);
...
//      if(abs(Temperature[y][x] - midT) < rangeT){
       if(abs(Temperature[y][x]) > midT){
        Img.set(x,y,color(17*Temperature[y][x],255,255));
...
class Blob{
  float cx,cy;
  float r;
  float xangle,xinc,xrad;
  float yangle,yinc,yrad;
  float xorig,yorig;
  ...
  Blob(float acx,float acy,float ar){
    ...
    cy = acy;
    r = ar;
    startWobbles(radians(20) * random(-1,1), r * random(0.1,0.3), random(20) * random(-1,1),r * random(0.1,0.3) );
  }
...
class WarmBlob extends Blob {
  ...
  WarmBlob(float acx, float acy , float ar){
    super(acx,acy,ar);
    startWobbles(radians(3)*random(-1,1),ar*random(0.1,0.2),radians(3) * random(-1,1),ar*random(0.1,0.2));
...
class HotBlob extends Blob{
  ...
  void restart(boolean firstTime){
    ...
    if(firstTime) cx += random(height);
    
    vx = random(-.1,.1);
    vy = map(r,40,100,-4,-2) * random(0.8,1.2);
    startWobbles(radians(2) * random(-1,1) , r * random(0.3,0.6),radians(2) * random(-1,1), random(0.3,0.6));
  }
