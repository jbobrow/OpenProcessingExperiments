...
void setup()
{
  ...
  if(!iniciar){  
    PImage ini = loadImage("portada.jpg");
    image(ini, 0, 0, width, height);
    textAlign(CENTER);
    myFont = createFont("comic sans ms", 75);
    textFont(myFont);
    text("PONG", width/2, height/4);
    myFont = createFont("comic sans ms", 25);
    textFont(myFont);
    text("versió 2011", width/2, height/4 + 35);
    textAlign(LEFT);
    myFont = createFont("comic sans ms", 30);
    textFont(myFont);
    text("Presiona:\nI-Iniciar joc\nC-Canviar nivell\nE-Eixir", 210, height/2 + 40);
    myFont = createFont("comic sans ms", 20);
    textFont(myFont);
    textAlign(RIGHT);
    text("Jordi García González", width - 5, height - 5);
    noLoop();
  }
}
...
void marcador(int j1, int j2){
  myFont = createFont("comic sans ms", 40);
  textFont(myFont);
  text(j1, width/4, height/4);
  text(j2, width*3/4, height/4);
}
