...
void setup() {
  ...
  for(int i = -1; i < width/patternSeperationX+2; i++) {
    ...
    for (int m = 0; m< 10; m++){
      fade(1000,round(random(2000)));
      
    }*/
  }
  //// Looped again to get rid of overlapping
  for(int k = -1; k < width/patternSeperationX+2; k++) {
    for(int m = -1; m < height/patternSeperationY+2; m++) {
      pushMatrix();
      
      translate(k*patternSeperationX, m*patternSeperationY);
...
void drawSquares() {
  ...
  for (int i = 0; i < 10; i++) {
    for(int j = 0; j < 10; j++) {
      // rect (xpos,ypos, 12,12);
      if (round(random(randColour)) == 1) { 
        fill(247, 80,200, random(0, 100));
        stroke(0, 0);
        rect(xpos, ypos, sizex,sizey);
        ypos += grid;
      }
      else if (round(random(randColour)) == 2) {
         fill(247, 50,170, random(0, 100));
        stroke(0, 0);
        rect(xpos, ypos, sizex,sizey);
        ypos += grid;
      }
      else {
       fill(247, 27,144, random(0, 100));
        stroke(0, 0);
        rect(xpos, ypos,sizex,sizey);
        ypos += grid;
      }
