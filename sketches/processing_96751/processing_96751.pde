...
int[] shootX = new int[60];
int[] shootY = new int[60];
int METEOR_SIZE = 15; // initial size when it first appears
...
void draw() {
  background(13,36,50); // dark blue night sky
  ...
  if (ssTimer >= 0 && ssTimer < shootX.length) {
    shootX[shootX.length-3] = int(startX + ssDeltaX*(ssTimer));
    shootY[shootY.length-3] = int(startY + ssDeltaY*(ssTimer));
    ssTimer++;
