...
void draw() {
  ...
  // STEP 4: display the mousePosition string at the current mouse location or near at edges
if (mouseX < 695 & mouseY >25) {
      text(mousePosition, mouseX, mouseY);
 }else{
   if (mouseY < 25) {
     text (mousePosition, mouseX, mouseY +30);
   }else{ 
     text(mousePosition, mouseX-100, mouseY); // works for rhs
 }
