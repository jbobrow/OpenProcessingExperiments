...
{
  size(800, 800);
  hyrule = requestImage("hyrule.png"); 
  myTank = new Tank();
...
void draw()
{
  // Clear the screen
  background(100);
image(hyrule,0,0,width,height);
  
  myTank.draw();
  boolean bitMonster =false;
  for (int i = 0; i < bulletCount; i += 1)
  {
    bullets[i].draw();
...
void keyPressed()
{
  if(key == CODED){
    if (keyCode == UP)
    {
      ...
      myTank.sprite.setAnimation(0, 4, true);
    }
    if (keyCode == DOWN)
    {
...
class AnimatedSprite
{
  ...
  {
    this.spriteSheet = requestImage("link.png");
...
class Tank 
{
  ...
  Tank()
  {
    ...
    this.sprite = new AnimatedSprite("link.png", 24, 32);
  }
...
class Bullet
{
  ...
  Bullet()
  {
    this.img = requestImage("Hookshot.png");
  }
