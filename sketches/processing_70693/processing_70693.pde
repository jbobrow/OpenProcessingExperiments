...
void setup() {
  ...
  myThingy = new Thingy[250];
  
  for (int i = 0; i < myThingy.length; i++){
    float thingySize = random (0, 100);
    float thingyPos = random(0, 20);
    float thingyStartX = random(0, 600);
    float thingyStartY = random(0, 600);
    myThingy[i] = new Thingy(thingyPos, thingyPos, thingySize, thingySize, color(int(random(0, 255)), int(random(0, 255)), int(random(0, 255)), 150), thingyStartX, thingyStartY);
  }
}
