...
//declare our objects
Doll a, b; // declare our object
Doll[] random = new Doll[instanceNum]; //this array will create randomly sized instances to fill the screen during the instantiation
...
class Doll { // this is the class name
  ...
  //variable stuff
  //pressure points
  float pressurePointRad=20;
  //head - x value
  float headWidthPointX = brainX+diameterX*.5;
  float headWidthPointY = brainY;

  //head - y value 
  float headLengthPointX = brainX;
  float headLengthPointY = brainY+diameterY*.5;
  ...
  void drawVariables() { //this method draws a doll with pressure points, which you can use to change variables
    ...
    ellipse(brainX+diameterX*.5, brainY, pressurePointRad, pressurePointRad);// x value of head
    ellipse(brainX, brainY+diameterY*.5, pressurePointRad, pressurePointRad);// y value of head
  }
  void dragVariables() { 
