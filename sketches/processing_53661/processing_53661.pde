...
void setup () {
  size (600, 300);
  smooth ();
  //The parameters are decided here when i intialize my object
  myBear = new Bear(0, 0);
  myBear2 = new Bear(280, 0);
  myGrass = new Grass();
  myButton = new Button();
}
...
class Bear { 
  ...
  Bear(float x, float y) {
    //here i am merely stating what I want to move in my class 
    //"Bear"
    this.x = x;
    this.y = y;
    r = color(mouseX, mouseY);
  }
  void display() {
    pushMatrix(); 
    translate(x, y);
