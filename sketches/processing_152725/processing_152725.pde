...
void draw() {
 ...
 // background(255);
  ...
  proximity = map(mouseX, 0, width, 75, 350);   // influence distance
  dark = map(mouseY, 0 , height, 0, 200);
  for (int i=0; i<numArray; i++) {
    for (int j=0; j<i; j++) {
      float distance = dist(x[i], y[i], x[j], y[j]  );
      if (distance < proximity) {
        if (i%2 == 0 && j%2==0) {
          stroke(dark, dark, 255, 4);
        } 
        else if (i%3 == 0 || j%3==0) {
          stroke(255, dark, dark, 3);
        }
        else{
          stroke(dark, 255, dark, 5);
        }
