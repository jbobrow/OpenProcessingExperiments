...
void draw() {
  background(255);
  wanderWeight = noise(frameCount/200)*2;
...
class Body {
  ...
  void update() {
    ...
    //r = map(wanderWeight, 0, 2, 1, initialR);
    
    hsb.z = map(wanderWeight, 0, 2, 0, 255);
    c = color(hsb.x, hsb.y, hsb.z);
  }
  ...
  void applyBehaviors() {
    ...
    if (goHome) {
      ...
      arrive(initialLocation);
      wanderWeight = map(mouseX, 0, width, 0, 5);
    }
  }
