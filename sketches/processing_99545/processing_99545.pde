...
seminari 4
bucles i arrays
Espases que fereixen als kennys que estàn per tota la pantalla de forma ordenada.
*/
//inicialitzo els valors que utilitzare per fer la matriu i els bucles
 
 
int numSpases = 10; //en primer lloc definim el número d' espases que volem que hi agui
...
background(0); // Determinem el color del fons
...
void draw(){
  background(0); //les espases no quedaràn repetides en el fons
   ///Realitzem les funcions///
limitEspasa();
bucleKenny();
  
}
  void limitEspasa (){ //Funció qu engloba la creació de l'espasa y els límits
  for(int m = 0; m<numSpases; m++){
    ...
    //comprobem lÃ­mits X
    if((posicionsX[m]<0)||(posicionsX[m]>width-10)){ // Cada espas rebotarà dins la finestra.
      velocitatsX[m] = -velocitatsX[m];
    }
    //comprobem lÃ­mits Y
    if((posicionsY[m]<0)||(posicionsY[m]>height-100)){ //Cada espasa rebotarà dins la finestra
...
void bucleKenny(){ //Funció que engloba la funció la creació dels buncle si el Kenny
