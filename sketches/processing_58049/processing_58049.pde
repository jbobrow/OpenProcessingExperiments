...
class Flocker {
  ...
  void Fline() {
    for (int j = 0; j < FlockerPop.size(); j++) {
      ...
      if (FlockerDist < 250) {
        stroke(150, 150, 150);
        strokeWeight(1);
        line(Floc.x, Floc.y, Floc.z, F.Floc.x, F.Floc.y, F.Floc.z);
      }
    }
  }
  ...
  void display() {
    ...
    con.normalize();
    con.mult(Fsize);
    PVector origin = new PVector(50, 0, 0);
    float rot = PVector.angleBetween(con, origin);
    rotateY(-atan2(con.x, con.z));
    rotateZ(-atan2(con.y, con.z));
    line(0, con.y, con.z, con.x/(-2), con.y, -con.z);
    line(con.x/(-2), con.y, -con.z, con.x/(2), con.y, -con.z);
    line(con.x/(2), con.y, -con.z, 0, con.y, con.z);
  }
}
