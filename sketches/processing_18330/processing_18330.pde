...
void setup() {
  ...
  frameRate(20);
  colSize = (width-border-1) / float(cols);
  rowSize = (height-border-1) / float(rows);
  font = loadFont("FreeSansBold-10.vlw");
  textFont(font,10);
...
void draw() {
    ...
    for (int i = 0;i < (cols) ;i++) {
      line(i*colSize+border,border,i*colSize+border,height);
    }
    for (int i = 0;i < (rows) ;i++) {
      line(border,i*rowSize+border,width,i*rowSize+border);
    }

    // Last line of grid
    line(width-1,border,width-1,height-1);
    line(border,height-1,width-1,height-1);
