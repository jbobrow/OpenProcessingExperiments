...
/* @pjs preload="titulo.png,  pantalla4.jpg, pantalla7.jpg,granger.png,balla.png,cerdo.png,catalan.gif,castellano.gif,ingles.png,granja.png,fons.jpg,activa.png,negra.png,vaca1.png,flecha.gif"; */

/*1=IDIOMAS, 2=INPUT, 3=HISTORIA, 4=JOC, 5=GAMEOVER, 6=RANKING, 7=CREDITS*/
InputUsuari inputUsuari;
Idiomas textos = new Idiomas();

PImage img2,img3,img4,img5,img6,img7,img8,cerdo,valla,granger,catalan,castellano,ingles,granja,titulo,vidactiva,vidanegra;
...
void setup(){
  ...
  menu = maxim.loadFile( "conditions.mp3");
  aplausos = maxim.loadFile( "aplausos.mp3");
  vaca = maxim.loadFile( "vaca.mp3");
  oveja = maxim.loadFile( "oveja.mp3");
  cavallo = maxim.loadFile( "cavallo.mp3");
  cerdo = maxim.loadFile( "cerdo.mp3");
  gameover = maxim.loadFile( "gameover.mp3");enu = maxim.loadFile( "conditions.mp3");
  ///se pueden poner los que quieras solo copia y pega lo de arriba!  (y lo del principio!)
 //   juego.cue(0);    //eso es para que empieze a sonar
//    juego.play();   //eso es para que empieze a sonar
  //  juego.stop();   //eso es para que pare de sonar
  //juego.setLooping(true); //hace que se repita esa cancion en loop
  todo.comienzozetup(); 
}
void draw(){  
  todo.comienzodraw();
...
void comienzozetup(){
 cosa.hacer();
  ...
  b4 = new Boto(1000/2, height/2+180, color(#249627), 100, 30, 2, 3, "Siguiente");
  b5 = new Boto(1000/2+250, height/2+200, color(#249627), 100, 30, 3, 4, "Siguiente");
  b6 = new Boto(5,5, color(#249627), 100, 30, 4, 5, "Siguiente");
  b7 = new Boto(1000/2+200, height/2+200, color(#249627), 100, 30, 5, 6, "Siguiente");
  b8 = new Boto(1000/2-120, height/2+220, color(#249627), 100, 30, 7, 1, "Inicio");
  b9 = new Boto(1000/2+300, height/2+200, color(#249627), 100, 30, 6, 7, "No");
  b10 = new Boto(1000/2+300, height/2, color(#249627), 100, 30, 6, 4, "Si"); 
  b11 = new Boto(1000/2+300, height/2+200, color(#249627), 100, 30, 1, 7, "Crèdits"); 
  /*INPUT*/
...
class Idiomas{
 ...
 void textojuego(){
   if(numidioma==1){  
    text("Enter the answer",550-300,300+280);
   }
   if(numidioma==2){
 ...
 void rankingg(){
   if(numidioma==1){  
    text("Name",340,100+70);  
    text("Position",530,100+70);  
    text("Points",650,100+70); 
   }
   if(numidioma==2){
    text("Nombre",340,100+70);  
    text("Puesto",530,100+70);  
    text("Puntos",650,100+70); 
   }
   if(numidioma==3){  
    text("Nom",340,100+70);  
    text("Lloc",530,100+70);  
    text("Punts",650,100+70); 
   }
 }
 void textgameover(){
...
void pantalla1(){
  ...
  text("Choice your language!",370,80);

  /*animales que decoran la p1*/
  image(cerdo,70,450,100,100);
  image(cerdo,600,430,100,100);
...
Idiomas rankingtext = new Idiomas();
  ...
  if(puntuscio<=0){
    ...
    text(nombres[0],340,yPos+120);  
    text("1",530,yPos+120);  
    text(Ranking[0],650,yPos+120);    
    
    text(nombres[1],340,yPos+160); 
    text("2",530,yPos+160);
    text(Ranking[1],650,yPos+160);

    text(nombres[2],340,yPos+200);
    text("3",530,yPos+200); 
    text(Ranking[2],650,yPos+200); 

    text(nombres[3],340,yPos+240);
    text("4",530,yPos+240);
    text(Ranking[3],650,yPos+240);
        
    text(nombres[4],340,yPos+280);
    text("5",530,yPos+280);
    text(Ranking[4],650,yPos+280);
