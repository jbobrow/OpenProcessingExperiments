...
class Beast {
    ...
    void tick () {
        ...
        rotateLeft();
        ray = castRay();
        if (ray == 0){accelerate(-2);}
        if (ray == 1){accelerate(-2);}
        if (ray == 2){accelerate(3);}
    ...
    int castRay() {
        ...
        for(e = 0; e < eyesight; e++){
            ...
            for (b = 0; b < beasts.size(); b++){
                if (dist (xPos + cos(rotation) * e, yPos + sin(rotation) * e, beasts.get(b).xPos, beasts.get(b).yPos) < (radius/2 + beasts.get(b).radius/2)) {whatSaw = 2; break;}
            }
