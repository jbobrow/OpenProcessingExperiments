...
void setup() {
  ...
  for (int i =0; i<200; i++) {
    fairies[i] = new fairy(random(0, sizeX), random(0, sizeY), random(-1.5, 1.5), random(-1.5, 1.5), random(4,8), random(15,25), random(0, 20));
  }
}
...
class fairy {
  ...
  float a;//the opacity
  float r  = 190;//r component of the colour
