...
PImage house;
 ...
 int score = 0;
 int lives = 0; //score and spiders missed start at 0
 boolean play = false; //allows the game to start from the start screen
 PFont f;
  
void setup(){
size(1000, 450);
house = loadImage("MCE_348_background.jpg");
...
f = createFont("Arial",14,true); // A font to write text on the screen
  ...
  mySpider[16] = new Spider();
     ...
     myMaxim = new Maxim(this);
   player = new myMaxim.loadFile("squish_002.mp3");   //when spiders squished
   spiderHa = new myMaxim.loadFile("spider.mp3");       //when spiders get away
   myMusic = new myMaxim.loadFile("after_dark.mp3");    //background music
   youWin = new myMaxim.loadFile("grand_theme.mp3");    //when you win
   player.setLooping(false);
   spiderHa.setLooping(false);
   myMusic.setLooping(true);
   youWin.setLooping(true);
...
void fall(){
    ...
    if ((bodyY >= (height + 22)) && (bodyY <= (height + 24))){
    lives++;
    spiderHa.stop();
    spiderHa.play();
    }
