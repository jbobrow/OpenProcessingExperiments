...
// clean up original code to make the stars move like clock ticks

// added by schien@mail.ncku.edu.tw
// global variables
float _bStarX, _bStarY, _mStarX, _mStarY, _sStarX, _sStarY;
float _bStarA, _mStarA, _sStarA;
float _sunX, _sunY;
...
void setup()
{
  ...
  _sunX = (min(_bStarX, _mStarX, _sStarX)+max(_bStarX, _mStarX, _sStarX))/2;
  _sunY = (min(_bStarY, _mStarY, _sStarY)+max(_bStarY, _mStarY, _sStarY))/2;
  // find the starting angles
  _bStarA = atan2(_bStarY-_sunY, _bStarX-_sunX);
  _mStarA = atan2(_mStarY-_sunY, _mStarX-_sunX);
  _sStarA = atan2(_sStarY-_sunY, _sStarX-_sunX);
  // end tweaking
}
float xoff = 0.0;
...
xoff = xoff + .01;
  ...
  // beging tweak by schien@mail.ncku.edu.tw
  // static stars  
  // drawOneStar(234.0, 121, 44, 57);
  // drawOneStar(225, 50.7 ,28, 29);  
  // drawOneStar(456.5, 0, 33, 52);
  // stars as ticks
  float ta = PI*2*second()/60;
  // draw the big star
  float tr = dist(_bStarX, _bStarY, _sunX, _sunY);
  float tx = tr*cos(_bStarA+ta)+_sunX;
  float ty = tr*sin(_bStarA+ta)+_sunY;
  drawOneStar(tx, ty, 44, 57);
