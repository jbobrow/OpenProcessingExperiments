...
void movingCircle(float x, float y, float size, int circleNum) {
  ...
  float tempX = x + (size / 2) * tan(PI / frame_rate_value * finalAngle);
  float tempY = y + (size / 2) * cos(PI*5 / frame_rate_value * finalAngle);

  noStroke();
  fill(random(60), random(60), random(60), random(255));
  rect(tempX, tempY, random(size), random(size));
  rect(tempX, tempY, 1, size*5);
  stroke(0);
  fill(random(255), random(255));
  stroke(0, 1);
  line(x, y, random(tempX), random(tempY));
}
