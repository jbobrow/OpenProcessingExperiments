...
int boxes = 5;
float[] boxHue = new float[boxes];
float[] boxBrightness = new float[boxes];
float[] boxCompHue = new float[boxes];
...
void draw() {
  ...
  //set current box colors
  boxHue[currentBox] = hueValue;
  boxBrightness[currentBox] = brightValue;
  boxCompHue[currentBox] = complementryHue;
...
void drawBoxes()
{
  ...
  {
    noStroke();
    fill(boxHue[i], 100, boxBrightness[i]);
    float boxWidth = width/boxes;
    float boxHeight = height/10;
    rect(boxWidth*i, height-boxHeight, boxWidth*(i+1), height);

    fill(boxCompHue[i], 100, boxBrightness[i]);
    float innerBoxWidthOffset = (boxWidth - ((width/boxes)*.60))/2;
    float innerBoxHeightOffset = (boxHeight - ((height/10)*.60))/2;
    rect(boxWidth*i + innerBoxWidthOffset, height-boxHeight+ innerBoxHeightOffset, boxWidth - 2*innerBoxWidthOffset, boxHeight-2*innerBoxHeightOffset);
  }
}
...
void mousePressed() {
  if (isWithinCircle(colorHandleX, colorHandleY, handleSize)) {
    isLocked = true;
    currentBox = (boxes+(++currentBox))%boxes;
  }
}
