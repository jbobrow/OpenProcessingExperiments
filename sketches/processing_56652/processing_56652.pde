...
int sDetail = 71;  // Sphere detail setting
float rotationX = 0;
float rotationY = 0;
float velocityX = 0;
float velocityY = 0;
...
void renderGlobe() {
  ...
  scale(1.0, 3.6374/3.660, 3.6306/3.660);
  ...
  if(mousePressed) {
    if(mouseButton == LEFT){
      ...
      PushBack += mouseY-pmouseY;
      if(PushBack>0) PushBack = 0;
      if(PushBack<-height*3) PushBack = -height*3;
    }
  }
}
