...
imageMode(CENTER);
 background(255); 
  ...
  pushMatrix();
  translate(250, 250);
   WMD = WMD+((600-dist(hbc,vbc,hpc,vpc))/500);
    rotate(radians(WMD));
...
bcs = bcs + 2;
 ...
 // ********SOUND********// 
beat = (round(dist(hbc,vbc,hpc,vpc))/2);
if(wisp>0)
{wisp = wisp-5;}
if(wilt>0)
{wilt = wilt-5;}
...
if((pcs >= 0) && (pcs < 193) && (meal==round(beat/2))) 
{twelve.trigger();}
...
{two.trigger();}
if((pcs >= 448) && (pcs < 900) && (meal==round(beat/2))) 
{zero.trigger();}
