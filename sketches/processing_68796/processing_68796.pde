...
class Boid {
  ...
  Boid(Vec2D l, float ms, float mf) {
    acc = new Vec2D (0, 0);
  ...
  Vec2D steer(Vec2D target, boolean slowdown) {
    ...
    if (d > 0) {
      ...
      if ((slowdown) && (d < 100.0f)) desired.scale(maxspeed*(d/100.0f)); // This damping is somewhat arbitrary
      else desired.scale(maxspeed);
  ...
  Vec2D align (ArrayList boids) {
    ...
    if (count > 0) {
      //the longer boids are nearby, the less alighned they become
  ...
  Vec2D cohesion (ArrayList boids) {
    Vec2D sum = new Vec2D(0, 0);   // Start with empty vector to accumulate all locations
    int count = 0;
    for (int i = 0 ; i < boids.size(); i++) {
      Boid other = (Boid) boids.get(i);
      float d = loc.distanceTo(other.loc);
