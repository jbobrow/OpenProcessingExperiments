...
void draw(){
  ...
  if((mouseX > x -ballSize && mouseX < x +ballSize) && (y == screenWidth - screenWidth/10 )) {
  dirY = -dirY; 
  } 
  if(y == screenWidth - ballSize/2){
print (s);
  }
  rect(mouseX -ballSize, screenWidth - screenWidth/10 + ballSize/2, ballSize * 2, screenWidth/10); 
}
