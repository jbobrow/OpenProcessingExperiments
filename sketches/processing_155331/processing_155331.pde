...
/* OpenProcessing Tweak of *@*http://www.openprocessing.org/sketch/85437*@* */
...
void setup() {
  ...
  axisLayer=createGraphics(width, height);
  //layer for drawing the profile
  profileLayer=createGraphics(width, height);
  //layer for rendering the volume
  volumeLayer=createGraphics(width, height, OPENGL);
  axisLayer.beginDraw();
  ...
  volumeLayer.lights();
  volumeLayer.ambientLight(50,50,50);
  volumeLayer.directionalLight(20,20,20, 0, 0, -1);
  volumeLayer.smooth(8);
...
void draw() {
  ...
  if (mousePressed && drawMode==0 && mouseButton==LEFT) {    
    P.add(new PVector(mouseX, mouseY, 0));
    line(pmouseX, pmouseY, mouseX, mouseY);
    profileLayer.line(pmouseX, pmouseY, mouseX, mouseY);
  }

  if (mousePressed && drawMode==1 && mouseButton==LEFT) {//first point of the axis is placed
  ...
  if (drawMode==2 && mouseButton==LEFT) {
    background(255);
    axisLayer.beginDraw();
    axisLayer.background(255, 0);
