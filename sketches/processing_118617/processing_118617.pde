...
testMode cubes = new testMode(new color[]{80,80,180,180,130,130},trialsPerMode,false);
testMode solids = new testMode(new color[]{130,130,130,130,130,130},trialsPerMode,false);
testMode cubesSep = new testMode(new color[]{80,80,180,180,130,130},trialsPerMode,true);
testMode solidsSep = new testMode(new color[]{130,130,130,130,130,130},trialsPerMode,true);
testMode blanks = new testMode(new color[]{250,250,250,250,250,250},trialsPerMode,false);
int numModes = 5;
...
void setup(){
  ...
  testModes.add(cubesSep);
  testModes.add(solidsSep);
...
void reset(){
  ...
  cubesSep.totalTargetTimes=0;
  solidsSep.totalTargetTimes=0;
}
...
class testMode{
  ...
  boolean separation;
  testMode(color[] gs, int nTs,boolean sep){
    grays = gs;
    nTrials = nTs;
    separation = sep;
  }
...
class Hexagon{
  ...
  void drawHexagon(float x, float y,float s){
    ...
    if(activeMode.separation){
      stroke(255);
      strokeWeight(1);
      pushMatrix();
      translate(x,y);
      scale(s);
      line(-10.5*sqrt(3.0)/2.0,5.0,-10.5*sqrt(3.0)/2.0,-5.0);
      line(10.5*sqrt(3.0)/2.0,5.0,10.5*sqrt(3.0)/2.0,-5.0);
      popMatrix();
    }
  }
