...
void setup() {  //setup function called initially, only once
  ...
  makeLanguages();
  user = new Grid(10, 0, 0, width/2, color(120, 154, 110), true);
  level = new Grid(10, 0, 0, width/2, color(240, 63, 10), false);
  place = "/menu/home";
  seed = "";
  timer = {
      timeStarted : 0,
...
void draw() {  //draw function loops 
  ...
  else if(place.indexOf("/menu/difficulty") == 0) {
      background(#F2440A);
      
  }
  else if(place.indexOf("/menu/home") == 0) {
      background(#F2440A);
      user.display();
      fill(#F2B80A);
      textSize(28);
      text("Reverse Painting\n    v2.0", width/2, height/10);
      textSize(14);
      button(3*width/5, height/4, "Match The Boards", "");
      button(3*width/5, height/4+20, "", "");
  }
...
void mouseReleased() {
  whenreleased = 0;
  if(place.indexOf("/menu/home") == 0) {
    if(selected(user.xpos, user.ypos, user.side, user.side) == true){
      console.log(selected(user.xpos, user.ypos, user.side, user.side));
...
void button(int x, int y, String eltext, String action, int elemindex) {
  stroke(0);
  fill(255);
  if(selected(x, y, width/4-4, 20) && whenReleased < 2) {
    stroke(4*(60 % second()));
    fill(209, 189, 33);
    selectedElem = elemindex;
    if(whenReleased <= 1 && whenReleased >= 0) {
      doAction(action);
    }
  }
...
boolean selected(int x, int y, int w, int h) {
...
class Grid {
  ...
  Grid(int boxesPerSide, int x, int y, int w, color c, boolean click) {
    ...
    for(int i = 0; i < boxes.length; i++) {
      ...
      boxes[i][1] = green(c);
      boxes[i][2] = blue(c);
      boxes[i][3] = 0; //states 0:normal, 1:hover, 2:clicked
    }
    return this;
  }
  void paint(int[] coords, options) {
  ...
  void setPropertyAll(int metadata, int value) {
      for(int i = 0; i < boxes.length; i++) {
          this.boxes[i][metadata] = value;
      }
  }
  int getIndex(int mx, int my) { //mx = pixelX, my = pixelY
    mx -= xpos;
    my -= ypos;
