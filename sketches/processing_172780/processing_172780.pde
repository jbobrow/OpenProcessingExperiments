...
void draw() {
  ...
  text("Score = " + score,0,0,1000,100);// Score goes up when ball hits paddle
}
...
void checkCollide() {
  ...
  if (ballY > height-paddleHeight) { // past 'goal line'
    if (ballX > mouseX && ballX < mouseX + 200) { // within paddle
      ballSpeedY = ballSpeedY * -1.2; // switch Y direction and speed up after every hit from paddle
      score = score + 1;
    } else { // end game
