...
String input = "breakbeat";  // der eingegebene text
float spacing = 60;
...
void draw ()
{
  background( 202,200,193 );                        // hintergrund weiss
  ...
  { 
    if (mousePressed ==true){
      for ( int a=0;
        ...
        fill(0);
        line (10+a-x1,10+a-y1,1+a-x2,1+a-y2);

      }
      {
        stroke (random (200,255));

        line (1+x1,1+y1,1+x2,1+y2);
      ...
      fill (random(200,255));
      smooth();
      triangle (random(1,100)+x1,random(1,100)+y1,random(1,100)+x2,y2,random(1,100)+x2,random(1,100)-y2);
      line (1+x1,1+y1,1+x2,1+y2);
