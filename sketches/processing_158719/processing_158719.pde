...
/* @pjs preload="scdl2012.jpg,scdl2011.jpg,scdl2007.jpg,scdl1930.jpg,scdl1926.jpg";*/

//Déclarations des variables
//Variables des images à loader
int nbImageNew=3; //Nombre d'images récentes.
int nbImageOld=2; //Nombre d'images anciennes.
...
int fin=1900;//année de fin, ou l'image fini.
...
void setup() {
  size(764, 636);// pour le web, le setup doit ^çetre hard codé
  lengthPixels=764*636; //Résulution de l'image.
  //Initialisation du loading l'image.
  img[0] = loadImage("scdl2012.jpg");
  img[1] = loadImage("scdl2011.jpg");
  img[2] = loadImage("scdl2007.jpg");
  img[3] = loadImage("scdl1930.jpg");//scdl1930
  img[4] = loadImage("scdl1926.jpg");
  ...
  annee[3] = 1930;
  annee[4] = 1926;
  annee[5] = fin; //la dernière année, c'est la fin.
 ...
 //frameRate(2);
  if (!pause) { //Si l'image n'est pas sur pause.
    ...
    if (imageHackWebShow) {//si c'Est la dernière année
      for (int j=0; j<lengthPixels;j++) { // parcours tous les pixels.
        //    pixels[j]=assouplissement(additionModif(pixels[j], renduBlur[j]), imageHackWeb[j]); //appelle la fonction qui fait le merge entre l'image précédent, le noise, l'image cible.
      ...
      for (int j=0; j<lengthPixels;j++) { // parcours tous les pixels.
        // pixels[j]=assouplissement(additionModif(pixels[j], renduBlur[j]), img[indexCible].pixels[j]); //appelle la fonction qui fait le merge entre l'image précédent, le noise, l'image cible.
