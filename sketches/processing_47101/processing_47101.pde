...
void draw() {
  ...
  text("Ripple control", 37, 455, 120, 120);
  ...
  if ((mousePressed == true) && (mouseY>460) && (mouseY<480)  && (mouseX>130 && mouseX<380)) {
    bb = map(int(mouseX), 130, 380, 0, 200);
  }
  fill(90, 90, 90);
  ellipse(map(bb, 0, 200, 130, 380), 462, 15, 15);
  ...
  grp = RG.getText(letter, "FreeSans.ttf", 260, CENTER);
  ...
  if (mouseX<350 && mouseX>130) {
    RG.setPolygonizerLength(map(width - sp, 0, width, 20, 200));
  }
  if ((sp>350)) {
  ...
  if (points != null) {
    ...
    for (int i=0; i<points.length; i++) {      
      for (int j=0; j<10; j++) {
        ...
        ellipseMode(CENTER);
        ellipse(points[i].x, points[i].y, 10*j, 10*j);
          if (j==10){
          j=0;}
        }
...
//  println(mouseX + " , " + " , " + points.length);
  ...
  mask.textFont( f, 200); //select font
  mask.textAlign(CENTER);
  mask.text(letter, 150, 250); //draw letter
  mask.endDraw(); // end
  ...
  blend(mask, 0, 0, 300, 300, 0, 0, 400, 420, LIGHTEST);}
