...
float Bc = 70; // variable
boolean Tick = false; // conditional 
...
void draw() { 
  ...
  line(30,0,60,145);  // one of the lines on the sun
  line(40,0,100,140); // one of the lines on the sun 
  line(0,0,140,105); // one of the lines on the sun
  line(0,0,160,60); // one of the lines on the sun 
  ...
  void mousePressed () {
    if (Tick = false) // if Tick is false, tick will be true or else the tick will be false {
    Tick = true;
    } else {
      Tick = false;
    }
