...
//kl=loadImage("b.png");
  kl=createImage(200,200,ARGB);
 //size(kl.width,kl.height,P2D);
 size(200,200,P2D);
 loadPixels();
 A=new color[width*height];
  pg = createImage(width,height,ARGB);
  for (int x=0;x<pg.pixels.length;x++){
   pg.pixels[x]=0; 
  }
...
void draw(){
 ...
 // color col =A[pos];
      ...
      if( lmode ==1) dis =distx(mouseX,mouseY,x,y)/by;
...
void rendermap(){
 ...
 // int[] pix=A;
  int[] pix=new color [width*height];
arraycopyx3(get(),pix);
  i=0;
...
void arraycopyx2(PImage  a, PImage  b){
 for (int x=0;x<a.width*a.height;x++){
  b.pixels[x]=a.pixels[x];
 } 
}  

void arraycopyx3(PImage  a, color[]  b){
 for (int x=0;x<a.width*a.height;x++){
  b[x]=a.pixels[x];
 } 
}                                                    
