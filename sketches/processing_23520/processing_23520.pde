...
void drawQuadran(){
  strokeWeight(1);
  stroke(255, 160);//transparent grey shows well...
...
for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      float extraHue = ((float(mouseY) / height) *2)-1;
      float newLumi = (blue(theHSL)/255) - extraLumi;
      float newSat = (green(theHSL)/255) + extraSat;
      float newHue = (red(theHSL)/255) + extraHue;
       newLumi = constrain(newLumi, 0, 1);
       newSat = constrain(newSat, 0, 1);
       newHue = constrain(newHue, 0, 1);
       
       
      theHSL = color(newHue*255, newSat*255, blue(theHSL) );
