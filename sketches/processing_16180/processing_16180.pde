...
public class Cube{
  ...
  public Cube(float x, float y, float z, float cubeWidth, float cubeHeight, PImage cubeTex){
    dead = false;
    verts = new PVector[8];
  ...
  public void handle(PVector lightPoint){
    ...
    beginShape();//States that you are starting a new shape. 
      ...
      //tint(255-pow(PVector.dist(verts[3], lightPoint), 1.05));
      textureMode(NORMALIZED);
      texture(cubeTex);
    vertex(verts[0].x, verts[0].y, verts[0].z, 0, 1);/////////////////////////////
    vertex(verts[1].x, verts[1].y, verts[1].z, 1, 1);// FRONT FACE              //
    vertex(verts[2].x, verts[2].y, verts[2].z, 1, 0);//                         //
    vertex(verts[3].x, verts[3].y, verts[3].z, 0, 0);/////////////////////////////
    ...
    beginShape();//States that you are starting a new shape. 
      //make the brightness of the face equivalent to 255 minus the 
      //distance between the front left point closest to y=0 and the PVector passed to the method.
      //tint(255-pow(PVector.dist(verts[0], lightPoint), 1.05));
      textureMode(NORMALIZED);
      texture(cubeTex);
    vertex(verts[0].x, verts[0].y, verts[0].z, 0, 1);/////////////////////////////
    vertex(verts[3].x, verts[3].y, verts[3].z, 1, 1);// LEFT FACE               //        
    vertex(verts[7].x, verts[7].y, verts[7].z, 1, 0);//                         //
    vertex(verts[4].x, verts[4].y, verts[4].z, 0, 0);/////////////////////////////
    endShape(CLOSE);//States that you are done drawing the current shape, and you wish for the shape to be closed off, hence a polygon.
    ...
    beginShape();//States that you are starting a new shape. 
      ...
      textureMode(NORMALIZED);
      texture(cubeTex);
    vertex(verts[1].x, verts[1].y, verts[1].z, 0, 1);/////////////////////////////
    vertex(verts[5].x, verts[5].y, verts[5].z, 1, 1);// RIGHT FACE              //
    vertex(verts[6].x, verts[6].y, verts[6].z, 1, 0);//                         //
    vertex(verts[2].x, verts[2].y, verts[2].z, 0, 0);/////////////////////////////
    endShape(CLOSE);//States that you are done drawing the current shape, and you wish for the shape to be closed off, hence a polygon.
    ...
    beginShape();//States that you are starting a new shape. 
      ...
      textureMode(NORMALIZED);
      texture(cubeTex);
    vertex(verts[2].x, verts[2].y, verts[2].z, 0, 1);/////////////////////////////
    vertex(verts[3].x, verts[3].y, verts[3].z, 1, 1);// TOP FACE                //       
    vertex(verts[7].x, verts[7].y, verts[7].z, 1, 0);//                         //
    vertex(verts[6].x, verts[6].y, verts[6].z, 0, 0);/////////////////////////////
    endShape(CLOSE);//States that you are done drawing the current shape, and you wish for the shape to be closed off, hence a polygon.
    }
  }
